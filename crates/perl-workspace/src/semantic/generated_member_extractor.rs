...
//! Class::Tiny accessors, DBIx::Class declarations, and bounded
//! DBIx::QuickORM table-package synthesis.
//!
...
//! the declaration that proves the generated member.
//!
//! # Scope
//!
//! Only package-level declarations after an order-visible supported framework
//! import are recognized. QuickORM support is limited to the compile-time
//! `qorm_table` method generated by an exact
//! `use DBIx::QuickORM type => 'table'` import plus a static package-level
//! `table` declaration. Manual columns and database-driven `autorow` members
//! are not projected. Plain packages without a supported framework import are
//! skipped.
//!
...
struct WalkCtx {
    ...
    quickorm_table_package_active: bool,
}
    ...
    match &node.kind {
        ...
        NodeKind::Package { name, block, .. } => {
            if let Some(block) = block {
                ...
                ctx.quickorm_table_package_active = false;
        ...
        NodeKind::Use { module, args, .. } if is_quickorm_module(module) => {
            ctx.quickorm_table_package_active = use_args_select_quickorm_table_mode(args);
        }
        ...
        NodeKind::ExpressionStatement { expression } => {
            ...
            if ctx.quickorm_table_package_active {
                extract_quickorm_table_call(expression, file_id, ctx, out);
            }
        }
...
fn extract_quickorm_table_call(
    expression: &Node,
    file_id: FileId,
    ctx: &WalkCtx,
    out: &mut Vec<GeneratedMemberFact>,
    ...
    let Some(table_name) = args.first().and_then(quickorm_static_name_candidate) else {
        return;
    };

    let package = ctx.current_package.as_deref().unwrap_or("main");
    push_member(package, "qorm_table", &table_name, file_id, out);
}

fn quickorm_static_name_candidate(node: &Node) -> Option<NameCandidate> {
    let value = match &node.kind {
        NodeKind::String { value, interpolated } if !*interpolated => value,
        NodeKind::Identifier { name } => name,
        NodeKind::Binary { op, left, .. } if op == "=>" => {
            return quickorm_static_name_candidate(left);
        }
        _ => return None,
    };
...
"#,
    ...
    #[test]
    fn quickorm_table_package_emits_qorm_table_generated_member_fact()
    -> Result<(), Box<dyn std::error::Error>> {
        let facts = extract_from_source(
            r#"
...
"#,
        ...
        let names = canonical_names(&facts);
        assert!(names.contains(&"My::ORM::Table::User::qorm_table"));
        assert!(!names.contains(&"My::ORM::Table::User::user"));
        assert!(!names.contains(&"My::ORM::Table::User::id"));
        assert!(!names.contains(&"My::ORM::Table::User::name"));
        assert!(!names.contains(&"My::ORM::Table::User::email"));

        let fact = generated_fact(&facts, "My::ORM::Table::User::qorm_table")?;
        assert_eq!(fact.entity.kind, EntityKind::GeneratedMember);
        assert_eq!(fact.entity.provenance, Provenance::FrameworkSynthesis);
        assert_eq!(fact.entity.confidence, Confidence::Medium);
        assert_eq!(fact.anchor.provenance, Provenance::FrameworkSynthesis);
    ...
    #[test]
    fn quickorm_table_package_requires_exact_order_visible_import_profile() {
        let facts = extract_from_source(
            r#"
package BeforeImport;
table before_import => sub {};
use DBIx::QuickORM type => 'table';
...
table explicit_orm => sub {};

package OnlyTableDsl;
use DBIx::QuickORM only => ['table'];
table only_table => sub {};
...
"#,
        ...
        assert!(
            facts.is_empty(),
            "non-table-package, pre-activation, and runtime-nested calls must not synthesize qorm_table"
        );
    }
