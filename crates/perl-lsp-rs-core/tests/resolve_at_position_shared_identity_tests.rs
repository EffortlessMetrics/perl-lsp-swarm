...
fn nth_offset(needle: &str, n: usize) -> Result<u32, String> {
    let mut search_from = 0usize;
    let mut found = None;
    for _ in 0..=n {
        let index = SHADOWED_LEXICALS[search_from..]
            .find(needle)
            .ok_or_else(|| format!("fixture lacks occurrence {n} of {needle}"))?
            + search_from;
        found = Some(index);
        search_from = index + 1;
    }
    u32::try_from(found.unwrap_or_default())
        .map_err(|error| format!("fixture offset does not fit in u32: {error}"))
}

/// Offsets of the three declaration sites and the outer read, in source order.
fn outer_declaration() -> Result<u32, String> {
    nth_offset("$value", 0)
}
fn outer_read() -> Result<u32, String> {
    nth_offset("$value", 1)
}
fn inner_declaration() -> Result<u32, String> {
    nth_offset("$value", 2)
}
fn sibling_declaration() -> Result<u32, String> {
    nth_offset("$value", 4)
}
...
/// different answers.
fn indexed() -> Result<WorkspaceIndex, String> {
    let index = WorkspaceIndex::new();
    let uri = Url::parse(URI).map_err(|error| format!("fixture uri parses: {error}"))?;
    ...
    index
        .index_initial_file(uri, SHADOWED_LEXICALS.to_owned())
        .map_err(|error| format!("fixture indexes: {error:?}"))?;
...
fn resolve(index: &WorkspaceIndex, byte_offset: u32) -> Result<ResolveAtOutcome, String> {
...
#[test]
fn the_fixture_actually_resolves_identities() -> Result<(), String> {
    let index = indexed()?;
...
#[test]
fn definition_and_references_capture_the_same_subject_and_basis() -> Result<(), String> {
    let index = indexed()?;
...
#[test]
fn shadowed_lexicals_resolve_to_three_distinct_identities() -> Result<(), String> {
    let index = indexed()?;
    ...
    let entities: Vec<_> = [&outer, &inner, &sibling]
        ...
        .map(|outcome| {
            outcome
                ...
                .ok_or_else(|| format!("expected exact, got {outcome:?}"))
        })
...
#[test]
fn a_read_binds_to_its_own_declaration_not_the_nearest_spelling() -> Result<(), String> {
    let index = indexed()?;
...
#[test]
fn one_entity_can_carry_several_occurrence_roles() -> Result<(), String> {
    let index = indexed()?;
...
#[test]
fn a_cursor_with_no_occurrence_is_never_exact() -> Result<(), String> {
    let index = indexed()?;
...
#[test]
fn an_offset_past_end_of_source_is_never_exact() -> Result<(), String> {
    let index = indexed()?;
...
#[test]
fn an_unindexed_uri_yields_an_explicit_unknown_document_generation() -> Result<(), String> {
    let index = indexed()?;
...
#[test]
fn a_stale_view_produces_no_identity() -> Result<(), String> {
    let index = indexed()?;
    let basis = accepted_generation_basis(&index, URI);
    let byte_offset = outer_read()?;

    let outcome = index
        .with_semantic_queries_for_uri(URI, |file_id, queries| {
            let source = SemanticQueriesResolveSource::new(&queries);
            resolve_at_position(&source, file_id, byte_offset, &basis, true)
        })
        .ok_or_else(|| "semantic queries open for an indexed uri".to_owned())?;
