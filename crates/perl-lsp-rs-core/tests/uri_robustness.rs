...
/// Regression test for #815: `gen_lsp_types::Uri` used to silently accept an
/// empty string as a valid parse, returning a URI whose `.as_ref()` is `""`.
...
fn test_parse_uri_empty_string_returns_valid_fallback() {
    let uri = parse_uri("");
    assert!(!uri.as_ref().is_empty(), "empty input must not produce an empty URI");
    assert!(is_valid_uri(uri.as_ref()), "fallback for empty input must itself be a valid URI");
}
...
/// `""`, which `gen_lsp_types::Uri` used to accept without error, so the
...
fn test_parse_uri_whitespace_only_returns_valid_fallback() {
    ...
    for ws in ["   ", "\t\t", "\n\r\n", "  \t  \n  "] {
        ...
        assert!(!uri.as_ref().is_empty(), "whitespace-only input must not produce empty URI");
        assert!(
            is_valid_uri(uri.as_ref()),
...
fn test_parse_uri_scheme_only_returns_valid_fallback() {
    ...
    assert!(!uri.as_ref().is_empty(), "scheme-only input must not produce empty URI");
    assert!(is_valid_uri(uri.as_ref()), "result for 'file:' must be a valid URI");
}
...
fn test_parse_uri_http_scheme_preserved() {
    ...
    assert_eq!(uri.as_ref(), input, "valid http URI must be passed through unchanged");
}
...
fn test_parse_uri_https_scheme_preserved() {
    ...
    let uri = parse_uri(input);
    assert_eq!(uri.as_ref(), input, "valid https URI must be passed through unchanged");
}
...
fn test_parse_uri_urn_scheme_preserved() {
    ...
    let uri = parse_uri(input);
    assert!(is_valid_uri(uri.as_ref()), "urn: URI should produce a valid URI");
    // The urn must come through — either as-is or via fallback.
    assert!(!uri.as_ref().is_empty());
}
...
fn test_parse_uri_invalid_scheme_returns_valid_fallback() {
    ...
    for bad in ["123://not-valid", "no spaces://foo", "://missing-scheme"] {
        ...
        assert!(!uri.as_ref().is_empty(), "bad scheme '{bad}' must not produce empty URI");
        assert!(is_valid_uri(uri.as_ref()), "result for bad scheme '{bad}' must be a valid URI");
    }
}
...
fn test_parse_uri_idempotent_on_valid_file_uri() {
    ...
    for input in inputs {
        ...
        let second = parse_uri(first.as_ref());
        assert_eq!(first.as_ref(), second.as_ref(), "parse_uri must be idempotent for '{input}'");
    }
}
...
fn test_parse_uri_null_bytes_do_not_panic() {
    ...
    for bad in inputs {
        let uri = parse_uri(bad);
        assert!(!uri.as_ref().is_empty(), "null-byte input must not produce empty URI");
        assert!(is_valid_uri(uri.as_ref()), "result for null-byte input must be a valid URI");
    }
}
...
fn test_parse_uri_very_long_input_does_not_panic() {
    ...
    assert!(!uri.as_ref().is_empty(), "very long URI must not produce empty result");
    assert!(is_valid_uri(uri.as_ref()), "result for very long URI must be a valid URI");
}
...
fn test_parse_uri_multiple_bom_prefixes_do_not_panic() {
    ...
    assert!(!uri.as_ref().is_empty(), "double BOM must not produce empty URI");
    assert!(is_valid_uri(uri.as_ref()), "result for double BOM must be a valid URI");
}
...
fn test_parse_uri_fragment_identifier_preserved() {
    ...
    assert!(is_valid_uri(uri.as_ref()), "result must be a valid URI");
}
...
fn test_parse_uri_query_string_preserved() {
    ...
    let uri = parse_uri(input);
    assert!(!uri.as_ref().is_empty(), "query-string URI must not produce empty result");
    assert!(is_valid_uri(uri.as_ref()), "result must be a valid URI");
}
