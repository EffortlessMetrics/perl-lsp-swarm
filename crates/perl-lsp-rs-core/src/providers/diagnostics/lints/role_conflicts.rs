...
//!
//! The production diagnostics path builds a request-scoped `PackageGraphIndex`
//! (via `role_graph_scope::build_role_scoped_package_graph`) containing
//! `ComposesRole` edges for the roles consumed by the current file, then
//! passes it through `WorkspaceIndex::with_semantic_queries_for_uri_and_graph`.
//! This is necessary because the persistent workspace graph only contains
//! `Inherits` edges (from HIR lowering) — `ComposesRole` edges are extracted
//! by `perl-semantic-analyzer` which cannot be a dependency of `perl-workspace`
//! without a cycle.
//!
