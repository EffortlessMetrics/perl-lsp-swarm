...
//! The production diagnostics path now builds a request-scoped
//! `PackageGraphIndex` containing `ComposesRole` edges for consumed roles before
//! calling `with_semantic_queries_for_uri_and_graph`, so cross-file and
//! transitive conflict detection is active in the live server.
//!
