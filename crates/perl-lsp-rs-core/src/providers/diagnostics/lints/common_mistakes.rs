...
mod tests {
    ...
    fn pl404_emitter_declares_the_shape_observed_at_the_branch() {
        ...
        assert!(
            literal
                .iter()
                .any(|d| d.code.as_deref() == Some("PL404") && d.critic_observation.is_some()),
            "literal undef PL404 must carry an observation: {literal:?}"
        );
        let literal_observation = must_some(
            literal
                .iter()
        ...
        assert!(
            dataflow
                .iter()
                .any(|d| d.code.as_deref() == Some("PL404") && d.critic_observation.is_some()),
            "data-flow PL404 must carry an observation: {dataflow:?}"
        );
        let dataflow_observation = must_some(
            dataflow
                .iter()
                .find(|d| d.code.as_deref() == Some("PL404"))
                .and_then(|d| d.critic_observation.as_ref()),
    ...
    fn pl404_observations_declare_the_critic_scale_severity_the_producer_owns() {
        ...
        assert!(
            diags
                .iter()
                .any(|d| d.code.as_deref() == Some("PL404") && d.critic_observation.is_some()),
            "PL404 must carry an observation: {diags:?}"
        );
        let observation = must_some(
            diags
                .iter()
                .find(|d| d.code.as_deref() == Some("PL404"))
                .and_then(|d| d.critic_observation.as_ref()),
