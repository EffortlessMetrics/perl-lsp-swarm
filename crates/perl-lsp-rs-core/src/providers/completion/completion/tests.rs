...
#[test]
fn test_same_file_package_member_completion_without_workspace_index() {
...
my $sq = MathUtils::"#;
    ...
    assert!(
        ...
        "same-file source-backed package member should be offered without a workspace index; got {:?}",
        completions.iter().map(|completion| &completion.label).collect::<Vec<_>>()
    );
    assert!(
        completions.iter().any(|completion| completion.label == "cube"),
        "same-file source-backed package member should include all matching members; got {:?}",
        completions.iter().map(|completion| &completion.label).collect::<Vec<_>>()
    );
}

#[test]
fn test_same_file_package_member_completion_keeps_typed_prefix() {
    let code = r#"package MathUtils;
sub square {
    my ($n) = @_;
    return $n * $n;
...
#[test]
fn test_same_file_package_member_completion_skips_generated_framework_accessors() {
