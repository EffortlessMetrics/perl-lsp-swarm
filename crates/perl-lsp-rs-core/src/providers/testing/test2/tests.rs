...
fn test2_v1_default_exports_only_t2_and_no_pragmas() {
    ...
    let defaults = must_some(module_default_exports("Test2::V1"));
...
fn v1_pragma_default_predicate_exact_module_match_boundary() {
    ...
    // bundle".
    let v1 = must_some(resolve_import("Test2::V1", ""));
    assert_eq!(
        v1.pragmas,
    ...
    let suite = must_some(resolve_import("Test2::Suite", ""));
    assert_eq!(
        suite.pragmas,
...
fn test2_imports_rename_as_installs_alias_not_original() {
    let resolved = must_some(resolve_import("Test2::V0", "ok => {-as => 'my_ok'}"));
...
fn test2_imports_default_tag_keeps_full_set_with_rename() {
    let resolved = must_some(resolve_import("Test2::V0", "':DEFAULT', ok => {-as => 'my_ok'}"));
...
fn test2_imports_explicit_qw_list_replaces_default() {
    let resolved = must_some(resolve_import("Test2::V0", "qw/ok is/"));
...
fn test2_imports_prefix_rename() {
    let resolved = must_some(resolve_import("Test2::V0", "ok => {-prefix => 'my_'}"));
    assert!(resolved.symbols.contains("my_ok"));
    assert!(!resolved.symbols.contains("ok"));
}
...
fn test2_imports_compare_standalone_default_is_narrow() {
    ...
    let defaults = must_some(module_default_exports("Test2::Tools::Compare"));
    assert_eq!(defaults, &["is", "like"]);
    // But an explicit import of an EXPORT_OK symbol is trusted verbatim.
    let resolved = must_some(resolve_import("Test2::Tools::Compare", "qw/hash array/"));
...
fn test2_imports_subtest_tool_standalone_names() {
    let defaults = must_some(module_default_exports("Test2::Tools::Subtest"));
...
fn test2_imports_target_option_does_not_drop_exports() {
    let resolved = must_some(resolve_import("Test2::V0", "-target => 'Foo::Bar'"));
...
fn test2_empty_import_list_imports_nothing_and_no_pragmas() {
    ...
    let resolved = must_some(resolve_import("Test2::V0", "()"));
