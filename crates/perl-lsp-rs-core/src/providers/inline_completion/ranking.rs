...
pub(crate) struct RankedCompletionItem {
    pub(crate) score: InlineCandidateScore,
    pub(crate) order: usize,
    pub(crate) metadata: InlineCandidateMetadata,
    pub(crate) item: InlineCompletionItem,
}

#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub(crate) struct InlineCandidateMetadata {
    pub(crate) source: InlineCandidateSourceKind,
    pub(crate) reason: InlineCandidateReason,
    pub(crate) confidence: InlineCandidateConfidence,
}
...
impl InlineCandidateMetadata {
    ...
    pub(crate) fn stable_tiebreak(self) -> u8 {
...
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub(crate) enum InlineCandidateReason {
...
impl InlineCandidateReason {
    ...
    pub(crate) fn stable_rank(self) -> u8 {
        match self {
...
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub(crate) enum InlineCandidateConfidence {
    High,
    Medium,
    Low,
}
...
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub(crate) struct InlineCandidateScore(pub(crate) i16);
...
impl InlineCandidateScore {
    ...
    pub(crate) fn legacy_base(priority: u8) -> i16 {
        10_000 - i16::from(priority) * Self::LEGACY_PRIORITY_STEP
    }

    #[cfg(test)]
    pub(crate) fn from_legacy_priority(priority: u8) -> Self {
        Self(Self::legacy_base(priority))
    }
}

#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub(crate) enum InlineCandidateSourceKind {
    Receiver,
    Module,
    Syntax,
    Test,
...
pub(crate) fn receiver_targets_current_package(context: &SemanticInlineContext) -> bool {
