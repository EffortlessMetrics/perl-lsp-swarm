//! Request-local semantic context and style facts for inline completions.

use super::{PreparedInlineCompletionContext, indentation_style_from_line};
...
impl VariableFact {
    pub(crate) fn from_perl_variable(variable: &str) -> Option<Self> {
    ...
    pub(crate) fn as_perl_variable(&self) -> String {
        format!("{}{}", self.sigil.as_char(), self.name)
    }

    pub(crate) fn is_scalar_self(&self) -> bool {
...
impl InlineStyleContext {
    pub(crate) fn unknown(context: &PreparedInlineCompletionContext) -> Self {
        Self {
...
impl LanguagePreludeStyle {
    pub(crate) fn from_imports(imports: &[String]) -> Self {
