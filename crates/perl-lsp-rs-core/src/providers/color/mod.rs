...
fn detect_hex_colors(text: &str) -> Vec<ColorInformation> {
    ...
    detect_colors_with_captures(text, re, parse_hex_color)
}
...
fn detect_ansi_colors(text: &str) -> Vec<ColorInformation> {
    ...
    detect_colors_with_captures(text, re, parse_ansi_color)
}

/// Generic detector helper for regexes with:
/// - capture group 0 as the full literal range to highlight
/// - capture group 1 as the payload to parse into a Color
fn detect_colors_with_captures(
    text: &str,
    re: &Regex,
    parse_color: impl Fn(&str) -> Option<Color>,
) -> Vec<ColorInformation> {
    let mut colors = Vec::new();

    for (line_num, line) in text.lines().enumerate() {
        for captures in re.captures_iter(line) {
            let (Some(full_match), Some(payload_match)) = (captures.get(0), captures.get(1)) else {
                continue;
            };
            let Some(color) = parse_color(payload_match.as_str()) else {
                continue;
            };
