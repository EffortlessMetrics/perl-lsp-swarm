...
mod tests {
    use super::{
        ContentLengthFramer, FramingError, MAX_FRAME_SIZE, MAX_HEADER_BYTES, log_response,
        write_message, write_notification,
    };
    use crate::protocol::{JsonRpcError, JsonRpcId, JsonRpcResponse};
    use crate::transport::{ContentLengthMessageReader, IncomingMessageError, read_message};
    ...
    #[test]
    fn read_message_rejects_invalid_utf8_and_preserves_following_frame() -> io::Result<()> {
        ...
        assert!(
            ...
            "invalid UTF-8 must not become a request via U+FFFD replacement"
        );
    ...
    #[test]
    fn stateful_reader_rejects_invalid_utf8_then_reads_following_frame() -> io::Result<()> {
        ...
        assert!(
            matches!(outcome, Some(Err(IncomingMessageError::InvalidUtf8 { .. }))),
            "invalid UTF-8 must be a typed encoding failure, not U+FFFD replacement: {outcome:?}"
        );
        let recovered = reader.read_next(&mut cursor)?.ok_or_else(|| {
            io::Error::new(io::ErrorKind::UnexpectedEof, "expected recovered request")
        })?;
        assert_eq!(recovered.method, "recovered");
        Ok(())
