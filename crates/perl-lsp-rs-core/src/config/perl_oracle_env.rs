...
impl PerlOracleEnv {
    ...
    /// - `timeout`: `SYSTEM_INC_PROBE_TIMEOUT` (1 second). Unit tests may
    ///   widen this on their own thread through the `cfg(test)`-only
    ///   `effective_system_inc_probe_timeout` seam; released builds always
    ///   observe the constant.
    ...
    /// - `timeout`: `SYSTEM_INC_PROBE_TIMEOUT`, resolved through the same
    ///   seam as the startup probe.
