...
impl PerlOracleEnv {
    ...
    /// - `timeout`: `effective_system_inc_probe_timeout()`, which is the
    ///   1 second `SYSTEM_INC_PROBE_TIMEOUT` in every production build.
    ...
    /// - `timeout`: `effective_system_inc_probe_timeout()` (1 second in
    ///   production), inherited from the startup probe helper.
