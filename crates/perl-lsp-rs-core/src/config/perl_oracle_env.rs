...
impl PerlOracleEnv {
    ...
    /// - `timeout`: `SYSTEM_INC_PROBE_TIMEOUT` (1 second) in every shipped
    ///   build; `cfg(test)` builds may widen it per-thread so memoization
    ///   proofs do not race host spawn latency.
