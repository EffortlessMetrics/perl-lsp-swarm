...
impl PerlOracleEnv {
    ...
    /// - `timeout`: `effective_system_inc_probe_timeout()` — 1 second
    ///   (`SYSTEM_INC_PROBE_TIMEOUT`) in every non-test build.
    ...
    /// - `timeout`: `effective_system_inc_probe_timeout()` (`SYSTEM_INC_PROBE_TIMEOUT`).
