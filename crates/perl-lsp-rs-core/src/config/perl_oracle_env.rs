...
impl PerlOracleEnv {
    ...
    /// - `timeout`: `SYSTEM_INC_PROBE_TIMEOUT` (1 second), resolved through
    ///   `effective_system_inc_probe_timeout` so unit tests can widen the
    ///   budget on their own thread without changing release behaviour.
    ...
    /// - `timeout`: resolved by the startup `@INC` probe helper.
