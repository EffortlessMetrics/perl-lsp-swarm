...
mod tests {
    ...
    fn accepted_state_serializes_to_one_canonical_representation() {
        ...
        let serialized = must_with(serde_json::to_string(&state), "accepted state must serialize");
        let round_tripped: EffectiveCriticState = must_with(
            serde_json::from_str(&serialized),
            "canonical representation must deserialize",
        );
        ...
        let disabled = must_with(
            serde_json::to_string(&EffectiveCriticState::Disabled),
            "disabled must serialize",
        );
    ...
    fn disabled_state_carries_no_live_native_policy() {
        ...
        assert!(
            matches!(state, EffectiveCriticState::Disabled),
            "disabled state must not expose a native policy object, got {state:?}"
        );
        assert!(
