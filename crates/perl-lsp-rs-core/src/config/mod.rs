...
"#,
    ...
    fn system_inc_timeout_retries_once_and_recovers() {
        ...
        assert_eq!(calls.get(), 2, "a settled successful outcome must not launch a third probe");
        assert_eq!(config.system_inc_cache, Some(SystemIncProbeOutcome::Paths(recovered)));
