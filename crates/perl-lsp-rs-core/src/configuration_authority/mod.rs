...
mod tests {
    ...
    fn every_effective_public_config_leaf_has_exactly_one_authority() {
        ...
        assert_eq!(
            actual.len(),
            CONFIGURATION_AUTHORITY.len(),
            "duplicate owner/field authority"
        );
    }

    #[test]
    fn ids_are_unique_sorted_and_source_precedence_is_monotonic() {
        let ids = CONFIGURATION_AUTHORITY
            .iter()
            .map(|field| field.id)
            .collect::<Vec<_>>();
    ...
    fn source_markers_and_rust_fields_remain_present_in_config_implementation() {
        ...
        assert!(missing.is_empty(), "authority references missing implementation markers: {missing:?}");
    }
