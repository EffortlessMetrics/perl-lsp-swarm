...
pub fn load_sectioned_corpus_document(
    ...
    let mut document = typed::load_sectioned_corpus_document(asset_id.clone(), path.as_ref())?;
    ...
    for (index, case) in document.cases.iter_mut().enumerate() {
        ...
        if !structured_ids.insert(section_id.clone()) {
            return Err(CorpusLoadError::DuplicateSectionId { asset_id, section_id });
        }
...
mod tests {
    ...
    -> Result<(), Box<dyn std::error::Error>> {
        ...
        assert_eq!(first.cases[0].id.section_id, "generated-1-generated-case");
        Ok(())
    }

    #[test]
    fn explicit_section_id_remains_authoritative() -> Result<(), Box<dyn std::error::Error>> {
        let root = tempfile::tempdir()?;
