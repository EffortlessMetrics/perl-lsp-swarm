...
impl fmt::Display for CorpusLoadError {
    fn fmt(&self, formatter: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            ...
            Self::MalformedSection { asset_id, path, line, reason } => write!(
                formatter,
                "sectioned corpus asset {asset_id:?} is malformed at {}:{line}: {reason}",
                path.display()
            ),
            Self::SectionPopulationMismatch { asset_id, declared, parsed } => write!(
                formatter,
                "sectioned corpus asset {asset_id:?} declared {declared} sections but parsed {parsed}"
            ),
            Self::DuplicateSectionId { asset_id, section_id } => write!(
                formatter,
...
pub fn load_sectioned_corpus_document(
    ...
    let path = path.as_ref();
    let loaded = read_utf8_regular_file(path)?;
    let without_bom = loaded.source.strip_prefix('\u{feff}').unwrap_or(&loaded.source);
    ...
    for section in sections {
        if !seen.insert(section.id.clone()) {
            return Err(CorpusLoadError::DuplicateSectionId { asset_id, section_id: section.id });
        }
        cases.push(SectionedCase {
            id: SectionCaseId { asset_id: asset_id.clone(), section_id: section.id.clone() },
            section,
        });
    }
...
fn is_section_delimiter(line: &str) -> bool {
    ...
    !without_trailing_whitespace.is_empty()
        && without_trailing_whitespace.as_bytes().iter().all(|byte| *byte == b'=')
}
    ...
    F: FnOnce(&Path) -> io::Result<File>,
{
    ...
    if metadata_is_link_like(&metadata) {
        return Err(CorpusLoadError::SymlinkUnsupported { path: path.to_path_buf() });
    }
    if !metadata.is_file() {
        return Err(CorpusLoadError::NotRegularFile { path: path.to_path_buf() });
    }

    let mut bytes = Vec::new();
    file.read_to_end(&mut bytes).map_err(|error| CorpusLoadError::Io {
        path: path.to_path_buf(),
        message: error.to_string(),
    })?;
...
fn classify_open_error(path: &Path, error: io::Error) -> CorpusLoadError {
    if error.kind() == io::ErrorKind::NotFound {
        return CorpusLoadError::Missing { path: path.to_path_buf() };
    }
    if error.kind() == io::ErrorKind::Unsupported {
        return CorpusLoadError::NoFollowUnsupported { path: path.to_path_buf() };
    }

    match fs::symlink_metadata(path) {
        Ok(metadata) if metadata_is_link_like(&metadata) => {
            CorpusLoadError::SymlinkUnsupported { path: path.to_path_buf() }
        }
        Ok(metadata) if !metadata.is_file() => {
            CorpusLoadError::NotRegularFile { path: path.to_path_buf() }
        }
        _ => CorpusLoadError::Io { path: path.to_path_buf(), message: error.to_string() },
    }
}
...
fn open_readonly_no_follow(path: &Path) -> io::Result<File> {
    ...
    OpenOptions::new().read(true).custom_flags(NOFOLLOW_OPEN_FLAGS).open(path)
}
...
mod tests {
    ...
    #[test]
    fn sectioned_loader_binds_cases_to_parent_asset() -> Result<(), Box<dyn std::error::Error>> {
        let dir = tempfile::tempdir()?;
        let path = dir.path().join("sections.txt");
        fs::write(&path, sectioned_source("\n"))?;

        let loaded = load_sectioned_corpus_document("tree_sitter/corpus/sections.txt", &path)?;
    ...
    -> Result<(), Box<dyn std::error::Error>> {
        ...
        let loaded = load_sectioned_corpus_document("tree_sitter/corpus/legacy.txt", &path)?;
        assert_eq!(loaded.source, source);
        assert!(loaded.utf8_bom);
    ...
    #[test]
    fn sectioned_loader_rejects_duplicate_explicit_ids() -> Result<(), Box<dyn std::error::Error>> {
        let dir = tempfile::tempdir()?;
    ...
    -> Result<(), Box<dyn std::error::Error>> {
        ...
        assert!(matches!(
            ...
            Err(CorpusLoadError::MalformedSection { line: 5, reason: "missing_title", .. })
        ));
        Ok(())
    }
...
mod unsupported_target_tests {
    ...
    #[test]
    fn regular_file_fails_with_no_follow_unsupported() -> Result<(), Box<dyn std::error::Error>> {
