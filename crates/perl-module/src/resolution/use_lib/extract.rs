...
/// Extract path arguments from the text that follows a `use lib` or `no lib` keyword.
///
/// Returns the number of bytes consumed from `args` by the parsed argument text.
/// A return value of `0` means no arguments could be parsed; callers fall back to
/// the enclosing statement extent in that case.
...
/// Returns the number of bytes consumed from `rest` (including open/close delimiters).
fn extract_qw_paths(rest: &str, out: &mut Vec<UseLibPath>) -> usize {
    ...
    let Some(end) = inner.find(close) else {
        // Unterminated qw: extract what we can; fall back to inner length.
...
/// Extract a comma-separated list of quoted path strings.
///
/// Returns the number of bytes consumed from `s` (stopping after the last successfully
/// parsed path argument, not at any following comma or whitespace).
