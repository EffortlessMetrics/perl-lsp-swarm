...
fn parse(name: &str) -> Result<PeerMessage> {
    serde_json::from_str(&fixture(name)?).with_context(|| format!("parse {name}"))
}

#[test]
fn hello_request_fixture_matches_wire_types() -> Result<()> {
    ...
    let args: HelloArgs =
        serde_json::from_value(req.arguments.context("args")?).context("hello args")?;
...
#[test]
fn hello_response_fixture_matches_wire_types() -> Result<()> {
    ...
    let body: HelloResponseBody =
        serde_json::from_value(resp.body.context("body")?).context("hello response body")?;
...
#[test]
fn stopped_event_fixture_matches_wire_types() -> Result<()> {
    let PeerMessage::Event(ev) = parse("stopped_event.json")? else {
        bail!("expected event");
    };
    assert_eq!(ev.event, event::STOPPED);
    let body: StoppedEventBody =
        serde_json::from_value(ev.body.context("body")?).context("stopped body")?;
...
#[test]
fn set_breakpoints_request_fixture_matches_wire_types() -> Result<()> {
    ...
    let args: SetBreakpointsArgs =
        serde_json::from_value(req.arguments.context("args")?).context("set bp args")?;
...
#[test]
fn set_breakpoints_response_fixture_preserves_order() -> Result<()> {
    let PeerMessage::Response(resp) = parse("set_breakpoints_response.json")? else {
        bail!("expected response");
    };
    let body: SetBreakpointsResponseBody =
        serde_json::from_value(resp.body.context("body")?).context("set bp resp")?;
