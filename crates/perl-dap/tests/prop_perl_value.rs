...
proptest! {
    ...
    fn prop_perl_value_constructors_preserve_inputs(
        ...
        tied_value in proptest::option::of(arb_leaf_perl_value()),
    ) {
        ...
        prop_assert_eq!(
            PerlValue::Tied { class: "Tie::Example".to_string(), value: tied_value.clone().map(Box::new) },
            PerlValue::Tied { class: "Tie::Example".to_string(), value: tied_value.map(Box::new) },
        );
    }
}
