        ...
        W: Write + Send + 'static,
    {
        ...
        thread::spawn(move || {
            ...
            while let Ok(first_msg) = rx.recv() {
                ...
                let persistent_failure = write_event_payloads(
                    &mut *writer,
                    &payloads,
                    &mut consecutive_write_failures,
                    &transport_broken,
                );
                // Release the request loop's drain barrier for this batch:
                // every counted message has now been written (or its write
                // attempt failed on a broken transport, which fails open).
                event_drain.complete(batch_len);
        ...
        loop {
            ...
            loop {
                ...
                // `handle_request`: the transport loop bypasses that seam
                // (like the #9581 floor below), so the profiled family
                ...
                let response = if command == crate::reload_family::LOADED_MODULE_RELOAD_REQUEST
                    && self.loaded_module_reload_route_enabled()
                {
                    self.handle_loaded_module_reload(self.next_seq(), seq, arguments)
                } else {
                    match self.secondary_capability_floor_response(
                        seq,
                        &command,
                        arguments.as_ref(),
                    ) {
