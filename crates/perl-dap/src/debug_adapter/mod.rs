...
impl DebugAdapter {
    ...
    fn validate_source_path(&self, path: &str) -> Result<PathBuf, String> {
        ...
        Self::validate_source_path_at(path, roots, self.workspace_authority.is_bounded())
    }

    /// Apply the source boundary to a client spelling or an observed debugger alias.
    ///
    /// Callers pass a root snapshot, so no authority lock is nested under the
    /// session/store locks. `bounded` travels with the snapshot: an adapter that
    /// is workspace-bound must fail closed rather than fall through to the
    /// unbounded shape-only path, and the snapshot alone cannot say which it is
    /// — a bounded authority whose roots are momentarily empty and an unbounded
    /// one both present as an empty slice.
    fn validate_source_path_at(
        ...
        // Resolve relative spellings once at admission so later launch replay
        // uses the same file identity. Preserve absolute spellings because
        // Windows canonicalize may add a device prefix that does not match the
        // launch identity.
    ...
    fn register_observed_breakpoint_hit(
        ...
        Self::validate_source_path_at(resolved, roots, bounded)
            .ok()
            .as_deref()
...
mod tests {
    ...
    fn observed_engine_hit_gates_source_digest_reads() -> Result<(), Box<dyn Error>> {
        let dir = tempfile::tempdir()?;
        // One trusted root, bounded — the authority-model spelling of the
        // `Some(dir.path())` this test used before this branch.
        let bounded_root = dir.path().to_path_buf();
        ...
        let wrong_line = DebugAdapter::register_observed_engine_breakpoint_hit_with_digest_reader(
            ...
            source_text,
            2,
            std::slice::from_ref(&bounded_root),
            true,
            dir.path(),
            7,
            |_| {
