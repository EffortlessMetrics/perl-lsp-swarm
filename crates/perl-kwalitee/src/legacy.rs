...
use crate::indicator::{CATALOG, EvalSource, IndicatorScope, IndicatorSpec};
...
#[cfg(test)]
const GENERATED_MARKDOWN: &str =
    include_str!("../../../docs/reference/PERL_KWALITEE_MIGRATION.md");
...
pub enum LegacyCompatibilityError {
    ...
    #[error("legacy migration ledger/catalog mismatch: expected [{expected}], observed [{observed}]")]
    CatalogMismatch {
...
pub fn validate_legacy_migration_ledger(
    ...
    let expected = CATALOG.iter().map(|spec| spec.id).collect::<Vec<_>>();
    let observed = ledger
        .indicator
        .iter()
    ...
    for row in &ledger.indicator {
        match (
            row.destination_rail,
            row.action,
            row.destination_id.as_deref(),
        ) {
...
pub fn legacy_indicator_records() -> Result<Vec<LegacyIndicatorRecord>, LegacyCompatibilityError> {
    ...
    ledger
        ...
        .map(|row| {
            let spec = CATALOG
                .iter()
                .find(|spec| spec.id == row.legacy_id.as_str())
                .ok_or_else(|| {
...
pub fn render_legacy_migration_markdown() -> Result<String, LegacyCompatibilityError> {
    ...
    for record in records {
        let destination = match &record.destination_id {
            Some(destination_id) => format!(
                "`{}` / `{}`",
                record.destination_rail.as_str(),
                destination_id
            ),
...
mod tests {
    ...
    fn ledger_accounts_for_the_frozen_catalog_exactly_once_and_in_order() {
        let ledger = legacy_migration_ledger().expect("checked-in ledger must validate");
    ...
    #[test]
    fn generated_migration_reference_is_current() {
        let generated =
            render_legacy_migration_markdown().expect("migration reference must render");
    ...
    fn legacy_reader_accepts_only_the_frozen_kind_and_schema() {
        ...
        let encoded = serde_json::to_vec(&receipt).expect("sample receipt must encode");
        assert_eq!(
            read_legacy_receipt(&encoded).expect("legacy receipt must decode"),
            receipt
        );

        let mut wrong_schema = serde_json::to_value(&receipt).expect("sample value");
        wrong_schema["schema_version"] = serde_json::json!(2);
        let encoded = serde_json::to_vec(&wrong_schema).expect("wrong schema fixture");
        assert!(matches!(
            read_legacy_receipt(&encoded),
            Err(LegacyCompatibilityError::UnsupportedReceiptSchema { .. })
        ));

        let mut wrong_kind = serde_json::to_value(&receipt).expect("sample value");
        wrong_kind["kind"] = serde_json::json!("distribution_kwalitee");
        let encoded = serde_json::to_vec(&wrong_kind).expect("wrong kind fixture");
        assert!(matches!(
            read_legacy_receipt(&encoded),
