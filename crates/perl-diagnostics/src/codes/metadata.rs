    ...
    pub fn context_hint(&self) -> Option<&'static str> {
        match self {
            ...
            Self::UnusedPrivateSubroutine => Some(
                "This private subroutine (prefixed with `_`) is defined but never called in this file. \
                Remove it or call it somewhere, or rename it without the `_` prefix if it is part of the public API.",
            ),
