...
//!     symbol_table: None,         // Optional pre-pass sub symbol table
//! };
//!
    ...
    fn try_identifier_or_keyword(&mut self) -> Option<Token> {
        ...
        if is_perl_identifier_start(ch) {
            ...
            let token_type = if is_keyword_fast(text) {
                ...
                // Also check the optional per-file symbol table so that user-declared
                // subs (`sub my_fn;`) are treated the same as builtins for mode purposes.
