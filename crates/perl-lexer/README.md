...
deprecated as of 0.17.0 and are removed at the next semver boundary (#8749):
...
| Cargo feature `simd` | **Deprecated** (since 0.17.0, removed at the next semver boundary by #8749). Compatibility no-op: it selects no distinct implementation, no code depends on it, and `--features simd` builds are equivalent to the default build. No SIMD performance claim is made. |
...
invalidates a captured checkpoint.
