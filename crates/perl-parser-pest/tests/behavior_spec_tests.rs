...
#[test]
fn when_given_if_statement_then_parser_emits_if_statement_shape() -> Result<(), String> {
...
#[test]
fn when_assignment_uses_space_tilde_form_then_normalization_allows_parse() -> Result<(), String> {
...
#[test]
fn when_hash_uses_fat_comma_pairs_then_parser_keeps_hash_assignment_structure() -> Result<(), String>
{
...
#[test]
fn when_hash_uses_quoted_fat_comma_keys_then_assignment_remains_parseable() -> Result<(), String> {
...
#[test]
fn when_foreach_iterates_over_hash_keys_then_foreach_shape_is_retained() -> Result<(), String> {
...
#[test]
fn when_given_when_has_default_clause_then_parser_emits_given_shape() -> Result<(), String> {
...
#[test]
fn when_if_has_elsif_and_else_then_parser_recovers_primary_if_shape() -> Result<(), String> {
    let sexp = parse_to_sexp(
        r#"
        if ($x == 1) { print "one"; }
        elsif ($x == 2) { print "two"; }
...
#[test]
fn when_ternary_expression_is_used_then_parser_emits_ternary_shape() -> Result<(), String> {
...
#[test]
fn when_input_is_only_invalid_then_parser_returns_error() -> Result<(), String> {
    let mut parser = PureRustPerlParser::new();
    let result = parser.parse("my = ; ???");

    if result.is_ok() {
        return Err("expected invalid input to return an error".to_string());
    }
    Ok(())
}
