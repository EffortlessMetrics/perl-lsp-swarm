//! Asserts the lower-crate dependency contract for `perl-evidence-envelope`.
//!
//! `perl-evidence-envelope` owns `evidence_envelope.v1` and must sit below
//! the entire product stack, exactly like its precedent
//! `perl-source-identity` (#7652): no editor/LSP runtime, no DAP, no async
//! runtime, no parser or workspace crate, directly or transitively. See
//! issue #15057 for the accepted scope.
//!
...
/// Crates that must never appear in `perl-evidence-envelope`'s dependency
/// tree.
///
...
/// collision-resistant digest for [`crate::EnvelopeFingerprint`] and
/// [`crate::ReceiptId`]; `perl-source-identity` is reused for
/// `ContentDigest`/`ProjectId` rather than re-derived. The remainder are
/// their required support crates.
const PERMITTED: &[&str] = &[
    "perl-evidence-envelope",
    // perl-source-identity (reused, not reinvented — #7652)
...
fn dependency_tree() -> String {
    let output = Command::new(env!("CARGO"))
        .args(["tree", "-p", "perl-evidence-envelope", "--edges", "normal", "--prefix", "none"])
        .output()
...
fn no_forbidden_dependencies() {
    ...
    for forbidden in FORBIDDEN {
        assert!(
            ...
            "perl-evidence-envelope must not depend on `{forbidden}`, \
...
fn dependency_closure_is_exactly_permitted() {
    ...
    assert!(
        unexpected.is_empty(),
        "perl-evidence-envelope gained unreviewed dependencies: {unexpected:?}\n\
         Every crate in the closure must be justified and added to `PERMITTED`.\n\
         Full tree:\n{tree}"
    );

    assert!(
        names.contains(&"perl-evidence-envelope"),
...
fn package_names_parses_cargo_tree_output() {
    let sample = "perl-evidence-envelope v0.1.0 (/repo/crates/perl-evidence-envelope)\n\
                  perl-source-identity v0.17.0\n\
                  serde v1.0.0\n\
                  sha2 v0.11.0\n\
                  digest v0.11.0 (*)\n\
                  \n";
    assert_eq!(
        ...
        vec!["digest", "perl-evidence-envelope", "perl-source-identity", "serde", "sha2"]
    );

    // A path containing a crate-like substring must not be read as a package.
    let tricky = "perl-evidence-envelope v0.1.0 (/home/dev/perl-workspace-core/checkout)\n";
    assert_eq!(package_names(tricky), vec!["perl-evidence-envelope"]);
}
