//! Parser benchmark and comparison commands for the CI hygiene CLI.
//!
//! This module keeps benchmark orchestration separate from unrelated hygiene
//! checks so each command group has a single responsibility.
...
pub(crate) const QUICK_BENCH_SAMPLES: usize = 3;
...
pub(crate) fn cmd_compare_benchmarks(repo_root: &Path, args: &[String]) -> Result<i32> {
