...
-> Result<(), Box<dyn std::error::Error>> {
    ...
    let backreference = analysis
        ...
        .ok_or("numeric backreference fact")?;
    // Runtime text does not imply unknown structure here. `\Q...\E` quotes the value
    // before regex interpretation, so an interpolated `(foo)` arrives as literal text and
    // cannot open a group. `(?<x>b)` is therefore still capture 1 and `\1` still has an
    // exact target. Reporting `DynamicUnknown` would contradict the capture analyzer,
    // which keeps numbering exact across quoted interpolation for the same reason.
    assert!(matches!(
...
#[test]
fn bare_interpolation_still_makes_later_references_unknown()
-> Result<(), Box<dyn std::error::Error>> {
    let analysis = analyze(r"$runtime(?<x>b)\1");

    // The discriminating counterpart: without `\Q`, the interpolated value is interpreted
    // as pattern source and can open its own groups, so the capture `\1` names is no
    // longer knowable from this source and the reference must fail closed.
    assert!(analysis.status.dynamic_pattern);
    assert!(!analysis.status.is_complete());
...
-> Result<(), Box<dyn std::error::Error>> {
    ...
    let call = analysis
        ...
        .ok_or("relative subpattern call fact")?;
