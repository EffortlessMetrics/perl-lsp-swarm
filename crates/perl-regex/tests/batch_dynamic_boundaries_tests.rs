...
use perl_regex::validator::{
    RegexAnalysisCompleteness, RegexDiagnosticCode, RegexDynamicRegionKind,
...
-> Result<(), Box<dyn std::error::Error>> {
    ...
    assert_eq!(
        pattern.get(fact.range.start..fact.range.end),
        Some("(?{ { nested => 1 } })")
    );

    let compatibility = validator
        .find_code_execution(pattern, 100)
        .ok_or("missing compatibility finding")?;
