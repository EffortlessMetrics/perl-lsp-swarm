...
pub(crate) fn analyze_pattern_controls(
    ...
    for event in &stream.events {
        ...
        match event.kind {
            ...
            RegexEventKind::Interpolation { quoted } => {
                // Both spellings supply runtime pattern text, so both are recorded as
                // interpolation boundaries. They differ in what they can do to structure:
                // `\Q...\E` quotes the value before regex interpretation, so it cannot
                // introduce groups and capture numbering stays exact — the capture analyzer
                // draws the same line. Only bare interpolation can inject structure, so
                // only it poisons later reference resolution.
                if !quoted {
                    dynamic_positions.push(event.range.start);
                }
