...
use super::line_ending::generated_line_ending;
...
impl NativeFormatter {
    ...
    fn format_safe_subset(source: &str, config: &FormatConfig) -> String {
        ...
        let inferred_line_ending = generated_line_ending(source);

        for line in source.split_inclusive('\n') {
            let (body, line_ending) = split_line_ending(line);
            let emitted_line_ending =
            ...
            formatted
                .push_str(&preserve_generated_line_endings(formatted_body, emitted_line_ending));
    ...
    fn format_safe_subset_range(
        ...
        for (line_index, line) in source.split_inclusive('\n').enumerate() {
            ...
            if let Some(formatted_line) = formatted_body {
                let formatted_line =
                    preserve_generated_line_endings(formatted_line, emitted_line_ending);
    ...
    ///
    /// `formatted` is the output of [`Self::format_safe_subset`], not the
    /// caller's original document, so the terminator inferred here comes from
    /// the text *after* formatting. That is correct for the layouts this
    /// formatter produces, which preserve each line's existing terminator, but
    /// it is the reason the parameter is not called `source`.
    fn apply_final_newline(formatted: &str, config: &FormatConfig) -> String {
        match config.final_newline {
            ...
            FinalNewline::Insert => {
                ...
                format!("{trimmed}{}", generated_line_ending(formatted))
            }
