...
impl NativeFormatter {
    ...
    /// Format a complete document with pipeline counters, preserving the
    /// document-level lexical guards of [`Self::format_document_typed`].
    #[must_use]
    ...
    /// Format one range with pipeline counters, preserving the range-level
    /// lexical guards of [`Self::format_range_typed`].
    #[must_use]
    pub fn format_range_typed_with_counters(
        ...
        if matches!(config.mode, FormatterMode::Off) {
            return engine
                .format_range_typed_with_counters(source, range, config, context, counters);
        }
