...
/// Maximum structured fuzz input accepted by the replay decoder. The decoder
/// folds tail bytes into the seed, so accepting an unbounded slice would make
/// total work unbounded even though each generated subject is bounded.
pub const MAX_FUZZ_INPUT_BYTES: usize = 256;
...
pub fn case_from_fuzz_input(data: &[u8]) -> Option<GeneratedCase> {
    if data.len() < 9 || data.len() > MAX_FUZZ_INPUT_BYTES {
        return None;
    }
