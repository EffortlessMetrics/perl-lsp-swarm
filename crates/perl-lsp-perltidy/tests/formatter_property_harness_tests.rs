...
//! per case (`MAX_SUBJECT_BYTES`, `MAX_PLAN_EDITS`, `MAX_SUBJECT_LINES`) and
//! across the runner/decoder inputs (`MAX_PROPTEST_CASES`,
...
const MAX_PROPTEST_CASES: u32 = 256;
...
fn harness_proptest_config() -> ProptestConfig {
    ProptestConfig {
        cases: bounded_proptest_cases(std::env::var("PROPTEST_CASES").ok().as_deref()),
...
#[test]
fn proptest_case_budget_is_validated_and_capped() {
    assert_eq!(bounded_proptest_cases(Some("0")), DEFAULT_PROPTEST_CASES);
    assert_eq!(bounded_proptest_cases(Some("not-a-number")), DEFAULT_PROPTEST_CASES);
    assert_eq!(bounded_proptest_cases(Some("999999")), MAX_PROPTEST_CASES);
    assert_eq!(bounded_proptest_cases(Some("17")), 17);
}

fn bounded_proptest_cases(value: Option<&str>) -> u32 {
    value
        .and_then(|raw| raw.parse::<u32>().ok())
        .filter(|&cases| cases > 0)
        .unwrap_or(DEFAULT_PROPTEST_CASES)
        .min(MAX_PROPTEST_CASES)
}
...
fn fuzz_decoder_consumes_bytes_past_the_selector() -> TestResult {
    ...
    assert!(case_from_fuzz_input(&vec![0_u8; MAX_FUZZ_INPUT_BYTES + 1]).is_none());
    Ok(())
}
