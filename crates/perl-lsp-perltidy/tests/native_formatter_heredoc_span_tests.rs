...
fn range_formatting_bare_cr_uses_shared_range_validity() {
    ...
    for range in [
        ...
        // Under CR-aware shared range geometry these positions are valid, so
        // the heredoc-preservation guard (not the range gate) owns the
        // refusal: nothing is edited, and the reason names preservation.
        assert!(!result.changed);
        assert!(result.edits.is_empty());
        assert_eq!(typed.outcome.reason, FormatReasonCode::LiteralPreservationUnsupported);
    }
