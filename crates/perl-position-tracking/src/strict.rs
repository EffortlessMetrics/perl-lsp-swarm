...
fn line_bounds(source: &str, target_line: u32) -> Option<LineBounds> {
    ...
    loop {
        ...
        if line == target_line {
            return Some(LineBounds { line, start, content_end });
        }
...
fn lookup_byte_line(source: &str, target_byte: usize) -> ByteLineLookup {
    ...
    loop {
        ...
        let bounds = LineBounds { line, start, content_end };

        if (start..=content_end).contains(&target_byte) {
            return ByteLineLookup::Content(bounds);
        }
        if target_byte > content_end && target_byte < separator_end {
        ...
        let Some(next_line) = line.checked_add(1) else {
            return ByteLineLookup::Overflow;
        };
        line = next_line;
        start = separator_end;
        index = separator_end;
