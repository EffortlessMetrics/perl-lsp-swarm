//! # perl-parser
//!
//! The public parser facade for Perl source. It re-exports the parser, AST,
//! error, position, refactoring, semantic, and workspace surfaces that are
//! intentionally supported by downstream consumers.
//!
//! The parser reports syntax and recovery information. A successful parse is
//! not a claim that semantic analysis, module resolution, or runtime execution
//! succeeded; use the relevant published crate for those layers.
//!
...
//! ~~~
//!
//! The example uses the public facade path a downstream crate receives. More
//! involved parser and editor workflows belong in the repository guides and
//! focused crate documentation.
//!
//! ## Public layers
//!
//! - perl-parser provides the parser facade and AST-facing compatibility surface.
//! - perl-parser-core provides parser-kernel types and shared infrastructure.
//! - perl-semantic-analyzer provides semantic analysis.
//! - perl-lsp-rs and perl-lsp-rs-core provide the LSP runtime and providers.
//! - perllsp provides the standalone server binary.
//!
//! These are separate claim surfaces. Do not describe parser success as full
//! LSP, semantic, or runtime correctness.
//!
//! ## Documentation and support boundaries
//!
//! Current examples should use current crate paths and constructors. Historical
//! microcrate names are retained only in migration or archive material. See
//! docs/reference/MISSING_DOCUMENTATION_GUIDE.md for the example contract and
//! docs/reference/STABILITY.md for the published API policy.
//!
//! The repository's current public API is the authority for examples; generated
//! docs and release-channel receipts do not expand the support claim.
//!
#![deny(unsafe_code)]
