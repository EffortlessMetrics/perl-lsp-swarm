...
pub use snapshot::{
    ContentFingerprint, ParseGeneration, ParseSnapshot, ParseSnapshotStrategy,
    ParseSnapshotValidationError, ParseTerminalDisposition,
};
...
pub fn apply_edits(state: &mut IncrementalState, edits: &[Edit]) -> Result<ReparseResult> {
    ...
    if sorted_edits.len() == 1 {
        ...
        candidate.refresh_parse_output(ParseSnapshotStrategy::IncrementalTokenRestartThenFullParse);
