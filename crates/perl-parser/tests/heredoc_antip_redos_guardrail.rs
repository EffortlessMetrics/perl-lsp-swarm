...
fn test_antip_dynamic_delimiter_valid() {
    ...
    assert!(
        matches.is_some(),
        "Valid dynamic delimiter <<${{FOO_BAR}} should be detected"
    );
}
...
fn test_antip_regex_heredoc_valid() {
    ...
    assert!(
        matches.is_some(),
        "Valid regex heredoc (?{{...<<...}}) should be detected"
    );
}
...
fn test_antip_eval_heredoc_valid() {
    ...
    assert!(
        matches.is_some(),
        "Valid eval heredoc should be detected"
    );
}
...
fn test_antip_export_qw_valid() {
    ...
    assert!(
        matches.is_some(),
        "Valid @EXPORT qw list should be detected"
    );
}
