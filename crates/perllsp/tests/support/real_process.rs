...
//!
//! This client runs only Cargo's exact public `perllsp` integration-test binary.
//! It never falls through to PATH, a target directory, or `cargo run`. Stdout
//! is a bounded, strict Content-Length stream; stderr is retained separately as
//! a bounded byte tail.
...
fn exact_candidate() -> Result<ExactCandidate> {
    match PERLLSP_CANDIDATE {
        Some(path) => Ok(ExactCandidate {
            path,
            binary_name: "perllsp",
            environment_name: "CARGO_BIN_EXE_perllsp",
        }),
        None => bail!("Cargo did not provide CARGO_BIN_EXE_perllsp"),
    }
}
