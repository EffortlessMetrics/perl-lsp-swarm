...
fn line_ending_equality() -> Result<(), Box<dyn std::error::Error>> {
    assert_eq!(LineEnding::Lf, LineEnding::Lf.clone());
    assert_eq!(LineEnding::CrLf, LineEnding::CrLf.clone());
    assert_eq!(LineEnding::Cr, LineEnding::Cr.clone());
    assert_eq!(LineEnding::Mixed, LineEnding::Mixed.clone());
