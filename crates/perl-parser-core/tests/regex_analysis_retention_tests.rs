...
use perl_parser_core::syntax::{
    ...
    quote_geometry::RegexFamilyOperator,
...
#[test]
fn retains_source_bound_pattern_analysis_with_exact_geometry()
...
#[test]
fn derives_lexical_source_utf8_and_lossless_modifier_profile()
-> Result<(), Box<dyn Error>> {
...
#[test]
fn transliteration_retains_geometry_without_regex_body_analysis()
-> Result<(), Box<dyn Error>> {
    ...
    let record = output
        ...
        .records
        .first()
        .ok_or("missing transliteration record")?;

    assert_eq!(record.operator, Some(RegexFamilyOperator::Transliteration));
    assert_eq!(
        record.availability,
        RegexAnalysisAvailability::TransliterationNotRegex
    );
...
#[test]
fn substitution_e_and_inline_code_project_one_ast_compatibility_fact()
-> Result<(), Box<dyn Error>> {
    ...
    let record = output
        ...
        .records
        .first()
        .ok_or("missing substitution record")?;
...
-> Result<(), Box<dyn Error>> {
    ...
    assert!(pattern.controls.diagnostics.iter().any(|diagnostic| {
        diagnostic.code
            == perl_regex::analyzer::PatternControlDiagnosticCode::UnresolvedReference
    }));
    assert_eq!(output.regex_analysis.analysis_invocations(), 1);
    ...
    assert!(output.diagnostics.iter().any(|diagnostic| {
        diagnostic.severity() == ParseDiagnosticSeverity::Blocking
    }));
    Ok(())
}

#[test]
fn nested_quantifier_advisory_selects_the_outer_quantifier()
-> Result<(), Box<dyn Error>> {
    let source = "my $ok = /^(a+)+$/;";
    let output = parse_source_with_regex_analysis(source);
...
#[test]
fn fresh_and_prelexed_entry_points_retain_identical_tables()
-> Result<(), Box<dyn Error>> {
...
#[test]
fn source_edit_changes_digest_and_record_geometry()
-> Result<(), Box<dyn Error>> {
...
#[test]
fn consecutive_parses_do_not_share_pending_geometry()
-> Result<(), Box<dyn Error>> {
    ...
    let geometry = second.regex_analysis.records[0]
        .geometry
        .as_ref()
...
#[test]
fn malformed_pattern_is_retained_with_explicit_incompleteness()
-> Result<(), Box<dyn Error>> {
    ...
    assert!(record
        .pattern
        .as_ref()
        .is_some_and(|pattern| pattern.controls.status.malformed));
    assert!(output.regex_analysis.source_matches(source));
    Ok(())
}
