...
fn method_call_string_simple() {
    ...
    assert!(
        parse_has_no_errors(source),
        "Expected no errors for simple method call"
    );
}
...
fn method_call_with_args() {
    ...
    assert!(
        parse_has_no_errors(source),
        "Expected no errors for method call with args"
    );
}
...
fn nested_method_calls_in_string() {
    ...
    assert!(
        parse_has_no_errors(source),
        "Expected no errors for chained method calls"
    );
}
...
"#;
    assert!(
        parse_has_no_errors(source),
        "Expected no errors for DBI.pm case"
    );
}
...
fn hash_dereference_in_string() {
    ...
    assert!(
        parse_has_no_errors(source),
        "Expected no errors for hash dereference"
    );
}
...
"#;
    assert!(
        parse_has_no_errors(source),
        "Expected no errors for array/hash derefs"
    );
}
...
fn method_call_with_empty_parens() {
    ...
    assert!(
        parse_has_no_errors(source),
        "Expected no errors for method call with empty parens"
    );
}
...
fn method_call_with_numeric_argument() {
    ...
    assert!(
        parse_has_no_errors(source),
        "Expected no errors for method call with numeric arg"
    );
}
...
fn method_call_with_string_argument() {
    ...
    assert!(
        parse_has_no_errors(source),
        "Expected no errors for method call with string arg"
    );
}
...
fn method_call_underscore_prefix() {
    ...
    assert!(
        parse_has_no_errors(source),
        "Expected no errors for method with underscore prefix"
    );
}
...
"#;
    assert!(
        parse_has_no_errors(source),
        "Expected no errors for reserved method names"
    );
}
...
fn mixed_valid_and_invalid_deref_in_string() {
    ...
    assert!(
        parse_has_no_errors(source),
        "Expected no errors for mixed valid/invalid derefs"
    );
}
...
fn escaped_paren_in_array_deref() {
    ...
    assert!(
        parse_has_no_errors(source),
        "Expected no errors for escaped paren in array deref"
    );
}
...
fn method_call_followed_by_text() {
    ...
    assert!(
        parse_has_no_errors(source),
        "Expected no errors for method call followed by text"
    );
}
...
fn multiple_method_calls_separate_scalars() {
    ...
    assert!(
        parse_has_no_errors(source),
        "Expected no errors for multiple method calls"
    );
}
