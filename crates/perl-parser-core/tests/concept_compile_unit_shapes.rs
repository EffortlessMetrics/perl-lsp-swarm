...
fn count_in_subtree(node: &Node, predicate: &impl Fn(&NodeKind) -> bool) -> usize {
    usize::from(predicate(&node.kind))
        + node.children().into_iter().map(|child| count_in_subtree(child, predicate)).sum::<usize>()
}

#[test]
fn package_statement_and_block_forms_keep_exact_name_and_body_geometry() -> Result<(), String> {
    let source = concat!("package Alpha::One 1.23;\n", "package Beta::Two { sub inside { 1 } }\n",);
    ...
    walk(&ast, &mut |node| {
        if let NodeKind::Package { name, name_span, block } = &node.kind {
            ...
            let block_is_block =
                block.as_deref().is_some_and(|owned| matches!(&owned.kind, NodeKind::Block { .. }));
    ...
    assert_eq!(
        ...
        vec![
            (
                ...
                Some("package Alpha::One".to_string()),
                Some("".to_string()),
                None,
                false,
                0,
...
fn use_and_no_keep_exact_module_and_flattened_argument_boundaries() -> Result<(), String> {
    ...
    walk(&ast, &mut |node| match &node.kind {
        NodeKind::Use { module, args, .. } => {
            uses.push((module.clone(), args.clone(), source_text(source, node)))
        }
        NodeKind::No { module, args, .. } => {
            no_directives.push((module.clone(), args.clone(), source_text(source, node)))
        }
        _ => {}
    });
    ...
    assert_eq!(
        ...
        vec![
            ...
            ("v5.38".to_string(), Vec::<String>::new(), Some("use v5.38".to_string()),),
        ],
    ...
    walk(&ast, &mut |node| {
            ...
            && name == "require"
        {
            let Some(target) = args.first() else {
                targets.push(("missing".to_string(), None, args.len()));
                return;
            };
            let target_kind = match &target.kind {
                NodeKind::Variable { sigil, name } if sigil == "$" && name == "module" => "dynamic",
                NodeKind::Identifier { name } if name == "Static::Module" => "bareword",
                NodeKind::String { .. } => "string",
                _ => "unexpected",
            };
            targets.push((target_kind.to_string(), source_text(source, target), args.len()));
        }
    });
    assert_eq!(
        targets,
        vec![
            ("dynamic".to_string(), Some("$module".to_string()), 1,),
            ("bareword".to_string(), Some("Static::Module".to_string()), 1,),
            ("string".to_string(), Some("'relative/file.pl'".to_string()), 1,),
        ],
