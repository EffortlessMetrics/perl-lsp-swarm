...
"#;
    ...
    assert!(
        ...
        "PR1 must not change provider behavior"
    );
    ...
    let prog_x_facts: Vec<_> = prog_body
        .facts
        .iter()
    ...
    assert_eq!(prog_writes, 1, "outer $x should have 1 write (declaration)");

    // All program root facts must be anchored
    for fact in &prog_x_facts {
        assert!(
            fact.source_anchor.is_anchored(),
            "all program root facts must be source-anchored"
        );
    }
    ...
    let foo_x_facts: Vec<_> = foo_body
        .facts
        .iter()
        .filter(|f| f.name.name == "x" && f.name.sigil == "$")
        .collect();
    ...
    for fact in &foo_x_facts {
        assert!(
            fact.source_anchor.is_anchored(),
            "all foo facts must be source-anchored"
        );
    }
...
"#;
    ...
    let counter_n_facts: Vec<_> = counter_body
        .facts
        .iter()
...
fn fixture_3_modify_nodes_skipped() -> Result<(), Box<dyn std::error::Error>> {
    ...
    assert_eq!(
        reads, 0,
        "$x++ must NOT count as a Read (Modify is skipped)"
    );
    assert_eq!(writes, 1, "should have 1 write from declaration");

    // Skipped count must track the Modify operation
    assert!(
        receipt.skipped_node_count >= 1,
        "Modify should be counted in skipped_node_count"
    );
...
fn fixture_4_empty_sub_body() -> Result<(), Box<dyn std::error::Error>> {
    ...
    assert_eq!(
        receipt.bodies.len(),
        2,
        "must have 2 bodies (ProgramRoot + empty)"
    );

    // empty sub (body_idx=1)
    let empty_body = &receipt.bodies[1];
    assert_eq!(
        empty_body.body_idx, 1,
        "empty sub must have body_idx=1"
    );
    assert!(
        empty_body.facts.is_empty(),
        "empty body should have no facts"
    );
    assert_eq!(
        empty_body.total_node_count, 0,
        "empty body should have 0 total nodes"
    );
