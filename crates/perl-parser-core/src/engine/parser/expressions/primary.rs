impl<'a> Parser<'a> {
    ...
    fn parse_qualified_identifier(&mut self) -> ParseResult<Node> {
        ...
        Ok(self.charge_node(NodeKind::Identifier { name }, SourceLocation { start, end })?)
    }
    ...
    fn parse_primary_inner(&mut self) -> ParseResult<Node> {
        ...
        match token_kind {
            ...
            TokenKind::Transliteration => {
                ...
                Ok(self.charge_node(
                    NodeKind::Transliteration {
                        expr: Box::new(charged_operand_2),
                        search,
                        replace,
                        modifiers,
                        negated: false,
            ...
            TokenKind::HeredocStart => {
                let start_token = self.advance_token()?;
            ...
            TokenKind::LeftShift => {
                ...
                if self.peek_kind() == Some(TokenKind::RightShift) {
                    self.consume_token()?; // consume >>
                    let end = self.previous_position();
                    Ok(self.charge_node(NodeKind::Diamond, SourceLocation { start, end })?)
                } else {
                ...
                if self.peek_kind() == Some(TokenKind::Greater) {
                    ...
                    if self.peek_kind() == Some(TokenKind::Greater) {
                        ...
                        if pattern.is_empty() {
                            ...
                            Ok(self.charge_node(NodeKind::Glob { pattern }, SourceLocation { start, end })?)
                    ...
                    || token.text.starts_with('&')
                {
                    ...
                    match token.text.as_ref() {
                        ...
                        "new" => {
                            ...
                            let new_token = self.advance_token()?;
            ...
            TokenKind::LeftParen => {
                ...
                if self.peek_kind() == Some(TokenKind::RightParen) {
                    let end_token = self.advance_token()?;
                    return Ok(self.charge_node(
                            ...
                            if let Some(name) = bare_call_name {
                                ...
                                let args = match args_node.into_parts() {
                                    ...
                                    (NodeKind::HashLiteral { pairs }, _) => pairs
                                        ...
                                        .flat_map(|(k, v)| [k, v])
                                        .collect(),
                                    (kind, location) => vec![self.charge_node(kind, location)?],
                                };
                                let call_end = args
                                    .last()
                    ...
                    || self.peek_kind() == Some(TokenKind::FatArrow)
                {
                    ...
                    if self.peek_kind() == Some(TokenKind::FatArrow) {
                        ...
                        if let NodeKind::Identifier { ref name } = elements[last_idx].kind {
                            ...
                            elements[last_idx] = self.charge_node(
                        ...
                        if self.peek_kind() == Some(TokenKind::FatArrow) {
                            self.advance_token()?; // consume redundant chained =>
                        }
                        if self.peek_kind() != Some(TokenKind::RightParen) {
                    ...
                    self.build_list_or_hash(elements, saw_fat_comma, start, end)
                } else {
    ...
    fn parse_array_literal_contents(&mut self) -> ParseResult<Node> {
        ...
        let end = self.previous_position();
        Ok(self.charge_node(NodeKind::ArrayLiteral { elements }, SourceLocation { start, end })?)
    }
}
