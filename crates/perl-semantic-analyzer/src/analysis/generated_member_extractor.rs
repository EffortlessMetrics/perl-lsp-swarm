...
impl GeneratedMemberExtractor {
    ...
    /// accessor, getter, setter, predicate, clearer, builder, or DBIx::Class
    /// relationship method generated by supported framework declarations.
    ///
    ...
    pub fn extract_from_models(models: &[ClassModel], package: &str) -> Vec<GeneratedMember> {
        ...
        for model in models {
            ...
            if model.framework == Framework::DbixClass {
                collect_dbix_members(model, pkg, &mut members);
            }
        }
...
fn collect_dbix_members(model: &ClassModel, package: &str, members: &mut Vec<GeneratedMember>) {
    for method in &model.methods {
        let Some(kind) = method.generated_kind else {
            continue;
        };
        members.push(make_member(
            &method.name,
...
mod tests {
    ...
    #[test]
    fn dbix_class_members_preserve_kinds_and_provenance() -> Result<(), String> {
        let code = r#"
...
"#;
        ...
        let id_members = members_named(&members, "id");
        let id = id_members.first().ok_or("expected generated member for id")?;
        assert_eq!(id.kind, GeneratedMemberKind::Accessor);
        assert_eq!(id.provenance, Provenance::FrameworkSynthesis);
        assert_eq!(id.confidence, Confidence::Medium);

        let posts_members = members_named(&members, "posts");
        let posts = posts_members.first().ok_or("expected generated member for posts")?;
        assert_eq!(posts.kind, GeneratedMemberKind::Method);
        assert_eq!(posts.package, "MyApp::Schema::Result::Author");
        assert_eq!(posts.provenance, Provenance::FrameworkSynthesis);
        assert_eq!(posts.confidence, Confidence::Medium);
        Ok(())
    }

    #[test]
    fn dbix_relationship_syntax_without_dbix_import_is_not_synthesized() {
        let code = r#"
package Plain::Package;
__PACKAGE__->has_many('posts', 'Post', 'author_id');
"#;
        let members = parse_and_extract(code);
        assert!(members_named(&members, "posts").is_empty());
    }
