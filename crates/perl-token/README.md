...
- **`TokenRef<'src>`** -- borrowed token view with the same geometry seal; construct with [`TokenRef::new_checked`]. `text` remains public for compatibility, and `to_owned_token()` revalidates externally replaced text, canonicalizing a width mismatch to safe recovery geometry. An empty `UnknownRest` with non-empty geometry is the explicit payload-free representation.
