...
- **`TokenRef<'src>`** -- borrowed token view with the same geometry seal; construct with [`TokenRef::new_checked`]. An empty `UnknownRest` with non-empty geometry is the explicit payload-free representation.
