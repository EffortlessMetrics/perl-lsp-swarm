            ...
            if attempt == 0 {
                ...
                return Err(format!(
                    "TestServerBuilder: Initialize failed after retry: {init_response:#}
                     ...
                     initialize deadline, which a cold build cannot meet. It does NOT mean the 
                     feature under test is broken.
                     Fix: build the server first with `cargo build -p perllsp --bin perllsp`, 
                     or point PERL_LSP_BIN at an existing binary. Note `perllsp` lives in a 
                     different package than these tests, so `cargo test -p perl-lsp-rs` does 
                     not build it on its own."
                ));
            }
        }
