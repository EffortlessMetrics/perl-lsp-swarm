...
#[test]
fn nested_projects_have_behavior_bearing_outer_and_inner_module_facts()
-> Result<(), Box<dyn Error>> {
    ...
    for kind in [
        ...
        let intended = fixture
            .intended_root
            .as_ref()
            .ok_or("nested fixture missing intended root")?;
        let outer = fixture
            .outer_root
            .as_ref()
            .ok_or("nested fixture missing outer root")?;
        ...
        assert!(outer_module.contains("OUTER") || outer_module.contains("OUTER_CONFIG"));
...
fn sibling_projects_cannot_pass_from_the_other_distribution() -> Result<(), Box<dyn Error>> {
    ...
    let intended = fixture
        .intended_root
        .as_ref()
        .ok_or("sibling fixture missing intended root")?;
    let sibling = fixture
        .sibling_root
        .as_ref()
        .ok_or("sibling fixture missing sibling root")?;
...
fn worktree_and_single_file_boundaries_are_explicit() -> Result<(), Box<dyn Error>> {
    let matrix = EmacsProjectFixtureMatrix::new()?;
    let worktree = matrix
        .fixture(EmacsProjectCaseKind::GitWorktreeShape)
        .ok_or("worktree fixture missing")?;
    let worktree_root = worktree
        .intended_root
        .as_ref()
        .ok_or("worktree fixture missing intended root")?;
    let dot_git = worktree_root.join(".git");
    assert!(dot_git.is_file());
    assert!(fs::read_to_string(dot_git)?.starts_with("gitdir: "));

    let standalone = matrix
        .fixture(EmacsProjectCaseKind::StandaloneFile)
        .ok_or("standalone fixture missing")?;
...
#[test]
fn stock_project_probe_is_bound_to_the_exact_open_file_and_receipt()
-> Result<(), Box<dyn Error>> {
    let matrix = EmacsProjectFixtureMatrix::new()?;
    let fixture = matrix
        .fixture(EmacsProjectCaseKind::MakefilePlNoVcs)
        .ok_or("probe fixture missing")?;
