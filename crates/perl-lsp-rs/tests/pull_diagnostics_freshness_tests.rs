...
fn pull_document_diagnostic_stable_doc_returns_result() -> TestResult {
    ...
    assert!(
        ...
        "result must carry an 'items' array"
    );
...
fn pull_document_diagnostic_pre_advanced_generation_does_not_false_positive() -> TestResult {
    ...
    assert!(
        gen_after_change.is_some(),
        "document must be open after didChange"
    );
...
fn pull_document_diagnostic_concurrent_didchange_does_not_panic() -> TestResult {
    ...
    assert!(
        resp.is_some(),
        "pull handler must always return Some result, never None"
    );
...
fn pull_workspace_diagnostic_returns_entry_per_doc() -> TestResult {
    ...
    assert!(
        resp.is_some(),
        "workspace/diagnostic must return Some result"
    );
    ...
    let uris: Vec<&str> = outer
        .iter()
        .filter_map(|e| e.get("uri")?.as_str())
        .collect();

    assert!(
        uris.contains(&uri1),
        "workspace diagnostic must include {uri1}; got: {uris:?}"
    );
    assert!(
        uris.contains(&uri2),
        "workspace diagnostic must include {uri2}; got: {uris:?}"
    );
...
fn pull_workspace_diagnostic_concurrent_didchange_does_not_panic() -> TestResult {
    ...
    assert!(
        resp.is_some(),
        "workspace/diagnostic must always return Some result"
    );
