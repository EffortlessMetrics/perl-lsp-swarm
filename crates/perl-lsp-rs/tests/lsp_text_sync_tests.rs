...
fn test_multi_line_replacement() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let changes = vec![TextDocumentContentChangeEvent::TextDocumentContentChangePartial(
        TextDocumentContentChangePartial {
            range: Range {
...
fn test_full_document_replace() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let changes = vec![TextDocumentContentChangeEvent::TextDocumentContentChangeWholeDocument(
        TextDocumentContentChangeWholeDocument { text: "brand new content\n".to_string() },
    )];
