//! Initialize-time ordering for the client configuration pull (#7708).
//!
//! Scope note: this in-process harness owns *ordering* and *request shape*.
//! Exactly-once is owned by the exact-`perllsp`-process transcript in
//! `crates/perllsp/tests/lsp_post_initialize_configuration_contract.rs`,
//! because `LspHarness` cannot express it: `TestWriter` stashes each
//! server-initiated request into `server_requests` *and* appends the raw
//! frame to `output_buffer`, which `drain_server_requests` re-parses and
//! stashes again. Every server request is therefore observed twice here,
//! regardless of how many the server actually sent. Asserting a count
//! through that harness would be asserting on a harness defect.
...
fn workspace_configuration_is_requested_only_after_initialized() -> Result<(), String> {
    ...
    assert!(
        !configuration_requests.is_empty(),
        "post-initialize convergence must issue the workspace/configuration request: {post_initialized_requests:?}"
    );
