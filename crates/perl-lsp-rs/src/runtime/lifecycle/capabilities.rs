...
mod tests {
    ...
    fn initialize_parses_code_action_disabled_support() {
        ...
        assert!(
            ...
            "disabledSupport=true must be stored in ClientCapabilities"
        );
    }

    #[test]
    fn initialize_leaves_code_action_disabled_support_false_when_absent() {
        let server = LspServer::new();
        ...
        assert!(
            !server.client_capabilities.lock().code_action_disabled_support,
            "disabledSupport must default to false when not declared"
        );
    }
