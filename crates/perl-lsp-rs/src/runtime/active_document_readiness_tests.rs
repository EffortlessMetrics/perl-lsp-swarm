...
mod tests {
    ...
    fn ready_notification_is_not_emitted_before_required_effects() {
        ...
        let (state, generation, _seq) = server
            .test_active_document_readiness(&key)
            .expect("open document must have a readiness entry");
    ...
    fn stale_effect_attachment_cannot_satisfy_newer_generation() {
        ...
        let stale_instance = {
            ...
            std::sync::Arc::clone(&docs.get(&key).expect("open").generation.clone())
        };
        ...
        let (_, generation, _) =
            server.test_active_document_readiness(&key).expect("entry survives");
        ...
        let (state, generation, _) =
            server.test_active_document_readiness(&key).expect("entry present");
    ...
    fn pull_client_profile_marks_push_publication_not_applicable() {
        ...
        let key = server.normalize_uri_key(uri);
        let (state, _, _) = server.test_active_document_readiness(&key).expect("entry present");
        assert_eq!(
