    ...
    fn select_terminal(
        ...
        match terminal_kind {
            ...
            IncomingTerminalKind::Shutdown => {
                state.counters.shutdown_selected =
                    state.counters.shutdown_selected.saturating_add(1);
            }
...
mod tests {
    ...
    #[test]
    fn supported_identities_round_trip_without_the_unsupported_path() -> TestResult {
    ...
    fn phase_progression_is_monotonic() -> TestResult {
        ...
        assert!(
            registry.anomalies().iter().any(|anomaly| {
    ...
    fn response_write_failure_is_distinct_and_cleans_once() -> TestResult {
        let registry = IncomingRequestRegistry::new(1)?;
        let handle = registry
            .admit(JsonRpcId::String("write-failure".to_string()), "textDocument/rename")?;
