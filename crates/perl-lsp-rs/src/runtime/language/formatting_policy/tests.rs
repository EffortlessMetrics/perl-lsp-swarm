...
#[test]
fn multi_range_refusal_discards_prior_edits() -> Result<(), Box<dyn std::error::Error>> {
    let server = LspServer::new();
...
#[test]
fn overlapping_multi_ranges_are_refused_before_formatting() -> Result<(), Box<dyn std::error::Error>>
{
    let server = LspServer::new();
    advertise(&server, Surface::Ranges);
