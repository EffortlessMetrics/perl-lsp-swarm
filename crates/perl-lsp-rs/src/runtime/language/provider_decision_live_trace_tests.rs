...
1;
"#;
const OUR_VARIABLE_SEMANTIC_TOKEN_URI: &str = "file:///workspace/lib/Trace/OurVariableTokens.pm";
const OUR_VARIABLE_SEMANTIC_TOKEN_DOC: &str = r#"use strict;
use warnings;
...
1;
"#;
const STATE_VARIABLE_SEMANTIC_TOKEN_URI: &str =
    "file:///workspace/lib/Trace/StateVariableTokens.pm";
// File-scoped `state` so the state-variable detector is the first reviewed
// compiler-token candidate to match (no `sub`/`my`/`our`/etc. ahead of it),
// which lets the live request expose the state-variable trace as the acted
// decision.
const STATE_VARIABLE_SEMANTIC_TOKEN_DOC: &str = r#"use strict;
use warnings;
use feature 'state';
...
fn open_state_variable_semantic_token_document(
    server: &LspServer,
) -> Result<(), Box<dyn std::error::Error>> {
    server.test_handle_did_open(Some(json!({
        "textDocument": {
            "uri": STATE_VARIABLE_SEMANTIC_TOKEN_URI,
            "text": STATE_VARIABLE_SEMANTIC_TOKEN_DOC,
            "languageId": "perl",
            "version": 1
        }
    })))?;
...
fn open_our_variable_semantic_token_document(
    server: &LspServer,
) -> Result<(), Box<dyn std::error::Error>> {
    server.test_handle_did_open(Some(json!({
        "textDocument": {
            "uri": OUR_VARIABLE_SEMANTIC_TOKEN_URI,
            "text": OUR_VARIABLE_SEMANTIC_TOKEN_DOC,
            "languageId": "perl",
            "version": 1
        }
    })))?;
...
#[test]
fn live_semantic_tokens_request_exposes_reviewed_our_variable_declaration_trace()
-> Result<(), Box<dyn std::error::Error>> {
    let server = create_server();
    initialize(&server)?;
    open_our_variable_semantic_token_document(&server)?;
    ...
    response_result(
        server.handle_request(request(
            ...
            "textDocument/semanticTokens/full",
            Some(json!({
                "textDocument": {"uri": OUR_VARIABLE_SEMANTIC_TOKEN_URI, "version": 1}
            })),
        )),
        "semantic tokens our variable declaration",
    )?;
    ...
    assert_eq!(
        receipt.get("source_backed_state").and_then(Value::as_str),
        Some("source_backed_our_variable_declaration_live_token_match")
    );
    assert_eq!(
        receipt.get("compiler_token_class").and_then(Value::as_str),
        Some("our_variable_declaration")
    );
    assert_eq!(receipt.get("live_token_type").and_then(Value::as_str), Some("variable"));
    assert_eq!(receipt.get("live_token_match_count").and_then(Value::as_u64), Some(1));
    assert_eq!(receipt.get("fallback").and_then(Value::as_str), Some("none"));
    ...
    assert!(
        boundary.contains("broader variable classes")
            && boundary.contains("generated/no-source")
            && boundary.contains("dynamic-boundary"),
        "our-variable trace must preserve scoped blockers: {boundary}"
    );
    assert!(
        explanation.get("user_message").and_then(Value::as_str).is_some_and(
            |message| message.contains("compiler our-variable declaration live trace")
        ),
        "explanation must surface the reviewed our-variable declaration trace: {explanation}"
    );
    Ok(())
}

#[test]
fn live_semantic_tokens_request_exposes_reviewed_state_variable_declaration_trace()
-> Result<(), Box<dyn std::error::Error>> {
    let server = create_server();
    initialize(&server)?;
    open_state_variable_semantic_token_document(&server)?;
    ...
    response_result(
        server.handle_request(request(
            ...
            "textDocument/semanticTokens/full",
            Some(json!({
                "textDocument": {"uri": STATE_VARIABLE_SEMANTIC_TOKEN_URI, "version": 1}
            })),
        )),
        "semantic tokens state variable declaration",
    )?;
    ...
    assert_eq!(
        receipt.get("source_backed_state").and_then(Value::as_str),
        Some("source_backed_state_variable_declaration_live_token_match")
    );
    assert_eq!(
        receipt.get("compiler_token_class").and_then(Value::as_str),
    ...
    assert!(
        boundary.contains("broader variable classes")
            && boundary.contains("generated/no-source")
            && boundary.contains("dynamic-boundary"),
        "state-variable trace must preserve scoped blockers: {boundary}"
    );
    assert!(
        explanation.get("user_message").and_then(Value::as_str).is_some_and(
            |message| message.contains("compiler state-variable declaration live trace")
        ),
        "explanation must surface the reviewed state-variable declaration trace: {explanation}"
    );
    Ok(())
}
