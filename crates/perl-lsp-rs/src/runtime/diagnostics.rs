...
/// Shared owning-root projection for capture, service gates, and publication.
/// A retained single-file project version supplies the adjacent-directory
/// fallback only when no workspace folder owns the document.
pub(super) fn critic_root_for_document(
    uri: &str,
    workspace_folders: &parking_lot::Mutex<Vec<super::workspace_folder::WorkspaceFolderState>>,
    root_path: &parking_lot::Mutex<Option<std::path::PathBuf>>,
    single_file_config: &parking_lot::Mutex<Option<perl_lsp_rs_core::config::ProjectConfig>>,
) -> Option<std::path::PathBuf> {
    let folders = workspace_folders.lock();
    best_workspace_folder_for_doc(&folders, uri)
        .and_then(|folder| folder.path.clone().or_else(|| source_path_from_uri(&folder.uri)))
        .or_else(|| {
            single_file_config.lock().as_ref()?.perl.version.as_ref()?;
            // Match document-key drive normalization for every capture and recheck.
            source_path_from_uri(&perl_uri::uri_key(uri))?.parent().map(ToOwned::to_owned)
        })
...
impl PullDiagnosticsOrchestrator {
    ...
    pub fn build_context(&self, server: &LspServer, uri: &str) -> PullDiagnosticsContext {
        ...
        let workspace_root = critic_root_for_document(
            uri,
            &server.workspace_folders,
            &server.root_path,
            &server.single_file_project_config,
        );
        ...
        let accepted_state_currentness = {
            ...
            PullAcceptedStateCurrentness::new(std::sync::Arc::new(move || {
                let live_root = critic_root_for_document(
                    &currentness_uri,
                    &workspace_folders,
                    &root_path,
                    &single_file_config,
...
impl LspServer {
    ...
    pub(super) fn handle_document_diagnostic(
        ...
        if let Some((doc, generation, gen_at_snapshot, workspace_gen_at_snapshot)) = doc_snapshot {
            ...
            // may analyze many native rules over large trees. Initialized
    ...
    pub(super) fn handle_workspace_diagnostic(
        ...
        if !self.workspace_topology_stable.load(Ordering::SeqCst)
            || self.workspace_identity_generation.load(Ordering::SeqCst)
                != workspace_gen_at_snapshot
        {
            tracing::debug!(
    ...
    pub(crate) fn capture_accepted_critic(&self, subject: &str) -> AcceptedCriticSnapshot {
        let root_key = critic_root_for_document(
            subject,
    ...
    fn evaluate_native_critic(
        ...
        let snapshot_is_current = move || {
            ...
            .map(|path| path.to_string_lossy().into_owned());
            topology_stable.load(std::sync::atomic::Ordering::SeqCst)
...
mod tests {
    ...
    -> Result<(), Box<dyn std::error::Error>> {
        ...
        {
            return Err(
                "single-file push and pull must share current project root authority".into()
            );
        }
...
"
    ...
    fn merged_pull_internal_finding_keeps_message_union() {
        // Native findings now enter the typed report before the final serializer.
        // Exercise that production serializer's negotiated message union.
        let doc = crate::state::DocumentState::new("print 'hello';\n", 1);
        let server = LspServer::new();
        let finding = lsp_types::Diagnostic {
        ...
        let string_diag = server.lsp_diagnostic_to_json(&finding, &doc, "file:///test.pl", false);
        ...
        let markup_diag = server.lsp_diagnostic_to_json(&finding, &doc, "file:///test.pl", true);
