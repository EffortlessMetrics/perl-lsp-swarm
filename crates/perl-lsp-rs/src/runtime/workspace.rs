...
fn read_watched_file_content(uri: &str, purpose: &str) -> Option<String> {
    uri_to_fs_path(uri).and_then(|path| match crate::util::read_text_file_with_encoding(&path) {
                ...
                for file in files {
                    ...
                    if let Some(coordinator) = self.coordinator() {
                        ...
                        if let Some(path) = uri_to_fs_path(new_uri) {
                            if let Ok(content) = crate::util::read_text_file_with_encoding(&path) {
                    ...
                    if let Some(coordinator) = self.coordinator() {
                        if is_perl_source_uri(uri) {
                            if let Some(path) = uri_to_fs_path(uri) {
                                match crate::util::read_text_file_with_encoding(&path) {
                                    Ok(content) => {
                                        coordinator.notify_change(uri);
                                        if let Ok(url) = url::Url::parse(uri) {
    ...
    pub(super) fn start_workspace_indexing(&self) {
        ...
        std::thread::spawn(move || {
            ...
            for path in files {
                ...
                let content = match crate::util::read_text_file_with_encoding(&path) {
                    Ok(c) => c,
                    Err(e) => {
...
impl LspServer {
    ...
    fn read_workspace_text(&self, uri: &str) -> Option<String> {
        ...
        uri_to_fs_path(uri).and_then(|path| crate::util::read_text_file_with_encoding(&path).ok())
    }
}
...
mod tests {
    ...
    /// bytes must not panic or silently truncate. We fall back to Latin-1
    /// of the original bytes so the caller still gets something
    /// reasonable to index.
    #[cfg(feature = "workspace")]
    #[test]
