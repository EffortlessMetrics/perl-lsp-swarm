...
use perl_lsp_rs_core::platform::resolve_perl_path_with_toolchain;
...
fn probe_perl(config: &WorkspaceConfig, workspace: &Path) -> PerlReport {
    probe_perl_with_resolver(config, workspace, resolve_perl_path_with_toolchain)
}

fn probe_perl_with_resolver(
    config: &WorkspaceConfig,
    workspace: &Path,
    resolve_perl_path: impl FnOnce() -> anyhow::Result<PathBuf>,
) -> PerlReport {
    ...
    match run_command_with_timeout(command, timeout_secs) {
        ...
        Ok(output) => PerlReport {
            ...
            source,
            version: None,
            error: Some(format!("version probe exited with status {}", output.status)),
        },
        Err(error) => {
...
fn effective_root_candidates(
    ...
    let perl5lib_paths = perl5lib_paths
        .iter()
        .map(|path| EffectiveRootCandidate { raw: path.clone(), source: "PERL5LIB" });
...
fn path_report_with_raw(
    ...
    let resolved = resolve_path(workspace, resolved_path);
    let status = path_status(&resolved);
...
mod tests {
    ...
    -> TestResult {
        ...
        let metadata_error = missing.metadata().err().ok_or("missing path should not stat")?;
        assert_eq!(
            metadata_error.kind(),
            std::io::ErrorKind::NotFound,
            "input that hits the boundary: error.kind() == std::io::ErrorKind::NotFound"
        );
        let error = workspace_dir(missing_dir).err().ok_or("missing workspace should fail")?;
    ...
    #[cfg(unix)]
    #[test]
    fn workspace_dir_boundary_discriminator() -> TestResult {
    ...
    fn effective_root_details_boundary_discriminator_input_that_hits_the_boundary_normalized_path_text_candidate_raw_equals_normalized()
     {
        ...
        assert_eq!(
            normalized_path_text(&candidates[1].raw),
            normalized,
    ...
    fn system_inc_report_enabled_resolves_probe_paths() -> TestResult {
        ...
        assert_eq!(report.paths, vec![temp.path().join("relative-system-inc"), absolute_inc]);
        Ok(())
    }
