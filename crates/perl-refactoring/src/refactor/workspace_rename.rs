...
mod tests {
    ...
    fn test_is_perl_ident_char() {
        ...
        // Continuation bytes (0x80–0xBF) — represented as replacement char — are not ident chars.
        // The original byte-level bug: 0xB0 (continuation byte of 数 in UTF-8) would pass
        // `!is_ascii_alphanumeric()`, falsely appearing as a word boundary.
        // The char-based fix correctly handles this because the bytes are decoded as whole chars.
    }
    ...
    -> Result<(), Box<dyn std::error::Error>> {
        // Covers is_word_start path: char directly before match is an identifier char (underscore).
    ...
    -> Result<(), Box<dyn std::error::Error>> {
        // Covers is_word_end path: char directly after match is an identifier char (underscore).
    ...
    -> Result<(), Box<dyn std::error::Error>> {
        ...
        // Old byte code: byte at match_start-1 = 0xB1 (continuation), not ASCII alphanum
        //   => falsely signalled word boundary => incorrect match.
        // New char code: chars().next_back() = '\u{03B1}', is_alphanumeric() = true
        //   => is_word_start = false => correctly skips the embedded match.
        let dir = tempfile::tempdir()?;
        let path = dir.path().join("main.pl");
        // Build content with Unicode prefix + "foo" (as a variable component, not standalone)
