...
pub struct ObservedRunnerSubjectInput {
    ...
    pub plan: RunnerPlan,
    /// Scheduling the caller declared for this run (#7737). Upstream scheduling
    /// state is not observed, so the join compares `plan.scheduling` against
    /// this independent declaration instead of letting the candidate plan
    /// supply its own reconstruction input.
