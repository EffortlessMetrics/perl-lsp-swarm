...
fn validate_cluster_history_shape(history: &FailureClusterHistory) -> Vec<String> {
    ...
    for entry in &history.entries {
        ...
        for (label, value) in [
            ...
            ("current_stage", entry.current_stage.as_str()),
        ...
        for transition in &entry.transitions {
            ...
            for (label, value) in [
                ...
                ("to_cluster_id", transition.to_cluster_id.as_str()),
            ...
            if transition.to_cluster_id != entry.cluster_id {
                violations.push(format!(
                    "transition {} does not terminate at cluster {}",
                    transition.transition_id, entry.cluster_id
                ));
            }
            if transition.from_cluster_id == transition.to_cluster_id {
                violations.push(format!(
        ...
        if entry.status == FailureClusterHistoryStatus::Resolved
            && !entry.transitions.iter().any(|transition| {
                transition.to_cluster_id == entry.cluster_id
...
fn merge_cluster_history(
    ...
    for cluster in &report.clusters {
            ...
            history.entries.iter_mut().find(|entry| entry.cluster_id == cluster.cluster_id)
        {
            ...
            entry.current_stage = cluster.signature.stage.clone();
...
fn history_entry_from_cluster(
    ...
    FailureClusterHistoryEntry {
        ...
        current_stage: cluster.signature.stage.clone(),
...
fn validate_history_against_report(
    ...
    for cluster in &report.clusters {
        ...
        if entry.current_stage != cluster.signature.stage {
            violations.push(format!(
...
mod tests {
    ...
    fn cluster_history_preserves_missing_clusters_and_tracks_membership_growth() -> TestResult {
        ...
        assert_eq!(history_after_absence.entries, history.entries);
    ...
    fn cluster_history_accepts_only_an_explicit_bound_transition() -> TestResult {
        ...
        entry.transitions.push(FailureClusterHistoryTransition {
            ...
            to_cluster_id: entry.cluster_id.clone(),
