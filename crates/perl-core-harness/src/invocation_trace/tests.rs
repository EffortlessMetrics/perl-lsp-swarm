...
/// Minimal structural validator covering exactly the JSON Schema keywords
/// the registered schema uses (mirrors the #12281 discipline). Unknown
/// keywords fail closed instead of passing.
mod schema_check {
        ...
        if let Some(branches) = schema.get("oneOf").and_then(Value::as_array) {
            ...
            if passing != 1 {
                let details = branches
                    ...
                    .join(" | ");
                return Err(format!(
                    "instance satisfies {passing} oneOf branches, expected 1; instance={instance};                      branch errors: {details}"
                ));
            }
        }
        Ok(())
    ...
    /// this schema uses. Any other grammar fails closed instead of passing.
    ...
    fn split_bracket_and_quantifier(body: &str) -> Result<(&str, Quantifier), String> {
        ...
        let bounds = if let Some((low, high)) = numbers.split_once(',') {
            let max = if high.is_empty() { None } else { Some(parse_number(high)?) };
