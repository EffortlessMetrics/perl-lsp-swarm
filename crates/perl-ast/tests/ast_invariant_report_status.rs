...
fn number(start: usize, end: usize) -> Node {
    Node::new(NodeKind::Number { value: "1".to_string() }, SourceLocation::new(start, end))
}
...
fn report_status_keeps_findings_and_completeness_independent() {
    ...
    let invalid = validate_ast("1", &number(0, 4), AstInvariantOptions::default());
    assert!(invalid.has_findings());
    assert!(invalid.is_complete());
    assert!(!invalid.is_valid());

    let incomplete =
        validate_ast("1", &number(0, 4), AstInvariantOptions::default().with_max_findings(0));
