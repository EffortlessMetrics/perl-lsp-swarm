...
- Adding new `NodeKind` variants require updating `to_sexp()`, `to_sexp_inner()`, `kind_name()`, and the exhaustive child traversal methods (`try_for_each_child_with_field_observed()` and `NodeKind::for_each_child_mut_inner()` — the single mutable authority that public `for_each_child_mut()` and destructor detachment both consume) — `ALL_KIND_NAMES` is auto-derived and does not need manual updating
- `Node` implements `Drop` (iterative, depth-independent destruction, #8836): direct by-value field moves out of a `Node` are E0509; use `Node::into_parts()` for by-value consumption. Do not add a second mutable child table or a Drop-specific `NodeKind` match
