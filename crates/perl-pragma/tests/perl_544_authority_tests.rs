...
fn explicit_feature_preserves_older_version_authority() {
    let ast = program(
        vec![use_node("v5.42", &[], 0, 10), use_node("feature", &["'enhanced_xx'"], 20, 48)],
        48,
    );
    let environment = CompileTimePragmaEnvironment::build(&ast);
    let snapshot = environment.snapshot_at(48);

    assert_eq!(snapshot.perl_version(), Some(PerlVersion::new(5, 42)),);
...
fn feature_all_and_experimental_admit_enhanced_xx() {
    ...
    let experimental = program(vec![use_node("experimental", &["'enhanced_xx'"], 0, 34)], 34);
    ...
    let ast = program(
        vec![Node::new(
            NodeKind::FunctionCall {
                ...
                args: vec![Node::new(NodeKind::VString { value: "v5.44".to_string() }, loc(8, 13))],
            },
            loc(0, 14),
        )],
        14,
...
fn conditional_version_target_retains_version_authority() {
    let ast = program(vec![use_node("if", &["$]", ">=", "5.044", "v5.44"], 0, 30)], 30);
