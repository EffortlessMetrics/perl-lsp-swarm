...
4. **Commit coherence** — do the commits tell a story?
   ...
   ```
   > **MCP alternative (web/no-gh sessions):** `mcp__github__pull_request_read(method:"get_commits", owner, repo, pullNumber:<number>)` — returns the commit list without requiring a local checkout; or get `baseRefName` via `method:"get"` and use git locally.
