...
1. **Read the issue:**
   ...
   ```
   > **MCP alternative (web/no-gh sessions):** `mcp__github__issue_read(method:"get", issue_number:<number>)` → `.title`, `.body`, `.labels`; then `mcp__github__issue_read(method:"get_comments", issue_number:<number>)` → comment bodies.
