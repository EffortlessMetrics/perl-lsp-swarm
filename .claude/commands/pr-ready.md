...
```
> **MCP alternative (web/no-gh sessions):** `mcp__github__pull_request_read(method:"get", owner, repo, pullNumber:$NUMBER)` — `isDraft`, `title`, and `state` fields all present in the response.
...
gh pr ready $NUMBER
```
> **MCP alternative (web/no-gh sessions):** `mcp__github__update_pull_request(owner, repo, pullNumber:$NUMBER, draft:false)` — full parity. See [docs/reference/GH_MCP_FALLBACK.md].
