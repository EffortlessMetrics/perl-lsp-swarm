...
```
> **MCP alternative (web/no-gh sessions):** `mcp__github__list_pull_requests(state:"open")` → filter in agent code for `isDraft == true`.
...
```
> **MCP alternative (web/no-gh sessions):** `mcp__github__pull_request_read(method:"get", pullNumber:<number>)` → read `.isDraft`, `.title`, `.state`.
...
```
> **MCP alternative (web/no-gh sessions):** `mcp__github__pull_request_read(method:"get", pullNumber:<number>)` for labels; `mcp__github__pull_request_read(method:"get_files", pullNumber:<number>)` for changed files.
...
gh pr ready $NUMBER
```
> **MCP alternative (web/no-gh sessions):** `mcp__github__update_pull_request(pullNumber:<number>, draft:false)` — full parity.
...
```
> **MCP alternative (web/no-gh sessions):** `mcp__github__pull_request_read(method:"get", pullNumber:<number>)` → read `.url`.
