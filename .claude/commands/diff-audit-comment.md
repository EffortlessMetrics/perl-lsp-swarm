...
2. **Main-green guard** (per the 2026-04-26 main-green directive): verify the PR's CI includes workspace-wide checks SUCCESS (`Compile All Targets`, `PR Smoke` with workspace fmt, `Windows Guardrails`), not just per-crate. Per-crate green is necessary but not sufficient — workspace-wide cascades break main after merge. If `PR Smoke` shows fmt drift in any file (PR's own OR a recently-merged unrelated file the branch hasn't picked up), route to needs-ci-fix with cascade-update instruction.
...
1. Post comment:
   ...
   *Diff auditor — final coherence check before merge.*
   ...
   ```
   > **MCP alternative (web/no-gh sessions):** `mcp__github__add_issue_comment(owner, repo, issue_number:<number>, body:"<content>")` — full parity for posting PR/issue comments.
