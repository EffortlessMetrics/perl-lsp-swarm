...
2. Read the PR description and linked issue:
   ...
   ```
   > **MCP alternative (web/no-gh sessions):** `mcp__github__pull_request_read(method:"get", pullNumber:<number>)` → read `.title`, `.body`, `.labels`.

3. If the PR links an issue, read the issue for the original spec:
   ```bash
   gh issue view <number> --json body --jq '.body'
   ```
   > **MCP alternative (web/no-gh sessions):** `mcp__github__issue_read(method:"get", issue_number:<number>)` → read `.body`.
