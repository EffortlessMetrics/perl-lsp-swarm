...
2. Read the PR description and linked issue:
   ...
   ```
   > **MCP alternative (web/no-gh sessions):** `mcp__github__pull_request_read(method:"get", owner, repo, pullNumber:<number>)` — returns title, body, labels, and all PR metadata in one call.
