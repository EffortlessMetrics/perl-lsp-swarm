...
```
> **MCP alternative (web/no-gh sessions):** `gh pr checkout` is a git operation; in a worktree context use `git fetch origin pull/<number>/head:<branch> && git checkout <branch>`. For the comment: `mcp__github__add_issue_comment(issue_number:<number>, body:"Standards review complete...")`.
...
git push
```
> **MCP alternative (web/no-gh sessions):** `gh pr checkout` is a git operation; in a worktree context use `git fetch origin pull/<number>/head:<branch> && git checkout <branch>` (per CLAUDE.md worktree workflow). MCP has no equivalent for local branch operations.
...
```
> **MCP alternative (web/no-gh sessions):** `mcp__github__add_issue_comment(issue_number:<number>, body:"Standards review complete...")`.
...
```
> **MCP alternative (web/no-gh sessions):** `mcp__github__add_issue_comment(issue_number:<number>, body:"Standards review: NEEDS BUILDER...")` — full parity for PR comments.
