...
2. If GREEN, set sign-off label (verified apply — see `/label-apply-verified`):
   ```
   /label-apply-verified pr <number> "ci-green"
   ```
...
3. If RED or STALE, do NOT set label. Flag for pr-responder (verified apply — see `/label-apply-verified`):
   ```
   /label-apply-verified pr <number> "needs-ci-fix"
   ```
