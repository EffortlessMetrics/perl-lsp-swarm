...
- `.claude/agents/` — five programme-executing agent definitions (authority, tools, model, lifetime);
...
The removed `.claude/commands/` catalogue and the retired 38-file persona roster under `.claude/agents/` remain recoverable through Git history and archived research. They are donor history, not active runtime authority. The current five-definition roster in `.claude/agents/` is active.
