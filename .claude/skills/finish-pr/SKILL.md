---
name: finish-pr
description: Carry one selected pull request through feedback repair, candidate-bound local proof, Claude-native substantive review, live integration, squash merge, and reconciliation.
...
review, or landed-effect verification. Use Agent Teams only when lateral communication
changes the result; use Ultracode only inside this coherent claim. Children return
evidence, not merge verdicts.
...
Keep subagent/Team topology, role changes, temporary experiments, raw bot/CI logs,
repeated snapshots, retries, and routine skill transitions runtime-local.
