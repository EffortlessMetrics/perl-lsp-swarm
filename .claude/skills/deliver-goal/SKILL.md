...
This is the Claude campaign-root flow. Preserve the verbatim goal source, current
...
Do not turn the main Claude thread into a general implementation worker. Its job is to
...
Do not post frontier rows, task state, or subagent liveness to GitHub.

A wake event is the next fact that can change a lane decision. Examples: a subagent or
...
→ `deliver-pr`(#123)
→ `orchestrate-work`
→ claim-local writer/review agents or Ultracode workflow as useful
→ `finish-pr`
→ lane result
```
...
Use separate whole-flow agents for coherent independent claims, focused subagents for
bounded evidence questions, Ultracode for dynamically discovered claim-local work, and
Agent Teams only where lateral communication changes the result.
