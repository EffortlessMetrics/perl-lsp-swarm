...
Require observed execution evidence: the proof ran as an instrument, failed against current/wrong behavior for the intended reason, and relevant controls ran without vacuous failure. Then try to falsify the oracle using a test adversary, external authority, production-path review, or mutation-style analysis where useful.
