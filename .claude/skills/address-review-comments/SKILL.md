...
4. Use `scripts/reviews/disposition` with the PR, thread ID, human reply, class, and required class-specific evidence (`--commit`, `--argument`, `--superseded-by`, or `--issue`).
5. Let the helper emit the canonical `Disposition:` / `Evidence:` reply and `<!-- disposition:v1 ... -->` marker before resolving.
