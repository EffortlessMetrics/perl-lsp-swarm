...
fuzz_target!(|data: &[u8]| {
    ...
    while let Some(id) = split_u64(data, offset) {
        // The registry API is keyed by `JsonRpcId` (JSON-RPC 2.0 ids are i64 or
        // string); fold the raw u64 into that domain via wrapping conversion so
        // adversarial inputs still exercise both negative and positive ids.
