...
- **Automatic `.perltidyrc` discovery at the workspace root.** When no
  ...
  project-local `.perltidyrc` once during `initialize` (workspace root, then
  `$HOME/.perltidyrc`, then `$PERLTIDYRC`) and uses it when building the
  formatter config. Explicit configuration always takes precedence. (#1777)
