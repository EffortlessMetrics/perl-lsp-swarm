...
description: Compile Codex's runtime-local campaign, persistent claim-lane, role-specialized context, and focused evidence-worker graph for the selected route; join evidence and preserve GitHub as durable state.
---
...
attention bias improves evidence, and focused workers for bounded questions.
...
A skill transition does not require a new agent. A role is not a permanent stage lock.
A dedicated reviewer can consume several review skills in one shared context and may be
promoted in place to repair an accepted finding when the parent grants authority and no
same-candidate writer exists.

Create another context only when it provides real independence, a different environment,
a split claim/prerequisite, or useful high-output compression. Do not create one agent
per skill or review lens merely to repeat PR ingestion.
...
`$deliver-pr` and remains the same context while the route moves through review,
...
review. It may consume several related skills without being replaced. The standard
Codex review profile is `.codex/agents/pr-reviewer.toml`.
...
Answers one bounded question or consumes one named `$skill`. It may change source,
oracle, method, threat model, environment, or attention surface. It returns evidence,
...
| One PR or coherent claim | Persistent `$deliver-pr` lane |
| Broad PR campaign | Several disjoint PR lanes and/or dedicated review contexts |
| Independent cumulative review | Persistent `pr-reviewer` context consuming several review skills |
...
| Focused proof | Current authorized context when admitted, or bounded worker when another environment materially helps |
| CI log/artifact classification | Current PR context or focused high-output worker returning evidence |
...
Do not create a review agent, then a repair agent, then a proof agent solely because the
skill changed. That destroys loaded context and turns route execution into orchestration
work. Dedicated role contexts remain useful; reuse them across the skills their role
consumes.
...
duplicate, completed, closed, cancelled, or `Not found` handles to preserve a count. A
...
Reconstruct it from GitHub and repository artifacts after replacement. Never commit it,
post agent liveness, or turn it into a portfolio database.
