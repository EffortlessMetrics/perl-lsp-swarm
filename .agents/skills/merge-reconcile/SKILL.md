...
This is Codex's irreversible integration backstop. Normal entry is:
...
- `CLOSED_UNMERGED` with a useful durable close/supersede disposition → reconcile it.
- `CLOSED_UNMERGED` without a durable disposition → `NOT_PROVEN`.
- `OPEN` → establish predecessor judgments below.
- unknown/partial state → `NOT_PROVEN`.
