...
The accountable root owns this cumulative judgment for the claim frame. A bounded
review worker does not acquire merge authority.
...
The **accountable root** records any such waiver, owns the compare-and-swap transition,
and either merges or arms auto-merge. A bounded writer/reviewer does not inherit this
authority merely because it handled the claim's candidate.
...
its typed `ABANDONED`/`EXTERNAL_BLOCKER`/`NOT_PROVEN` result to the accountable root;
there is no subordinate orchestrator that must remain alive to represent it.
