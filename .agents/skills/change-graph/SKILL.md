...
Focused workers and review lenses do not ingest this graph by default. They read root and
nearest package guidance, then consume the exact skill named in their brief. A child
...
Use each invoked skill's `Routes`, `Valid exits`, or equivalent transition table. Skills
own procedure and next routing; the orchestrator owns the joined result.
...
Keep agent handles, role changes, queue order, retries, temporary todo lists, proof-token
allocation, polling, worktree liveness, and private reasoning runtime-local. GitHub and
repository artifacts reconstruct the route after compaction or provider replacement;
they do not mirror the runtime.
...
A skill transition does not require a new agent. A PR reviewer may consume several
...
- focused worker: execute only the named bounded skill/question.
