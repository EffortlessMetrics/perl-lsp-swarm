...
This is the root goal flow. Preserve the verbatim goal source when available; the
...
The main/root orchestrator owns goal meaning, claim selection, cross-claim dependencies,
...
repository artifacts after replacement. Never commit them, serialize agent liveness, or
mirror them through labels/comments.
...
Then focus the accountable root on that claim by invoking `$deliver-pr` in the same
root context:
...
The root retains claim meaning, route selection, evidence joins, review disposition,
and continuation. Researchers, builders, reviewers, explorers, forks, or teams are
bounded execution contexts.

A provider may physically nest an agent or workflow when that specific execution has an
evidence-backed advantage. That does not change logical authority and must not become a
required whole-flow hierarchy.
