...
campaign should run it now. These are different questions and a campaign can fail on
either.

Name the goal's current phase and what it admits. The failure this prevents is not poor
claim selection — it is that every valuable adjacent finding becomes runnable while the
phase predicate remains unsatisfied, so the campaign accumulates good PRs without
converging on its own acceptance criterion:
...
Deferring is not discarding. Record a deferred finding as a durable issue and continue;
...
phase to accommodate work already begun is how the predicate quietly stops governing.
