---
name: "source-command-ops-merge-batch"
description: "Ops step 2 — merge a batch of up to 3 PRs"
---

# source-command-ops-merge-batch

Use this skill when the user asks to run the migrated source command `ops-merge-batch`.
...
7. If a merge fails or pre-check fails:
   ...
   - mergeStateStatus = UNSTABLE → verify which check is red: if BOTH required checks (`Perl LSP Rust Small Result`, `ripr+ New Gap Gate`) are green on the current HEAD SHA, UNSTABLE from a non-required check is mergeable (per AGENTS.md "Merge with UNSTABLE is OK"). Skip only if a required check is failing or still in flight.
