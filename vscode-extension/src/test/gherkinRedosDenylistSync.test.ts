...
describe('gherkin ReDoS guard: nullable-path boundary states (#9806)', () => {
  test('retains a lower-score path when it is the only dangerous boundary', () => {
    // The first seven nullable groups create more than eight higher-score `b`
    // boundary paths. The lower-score path that selects `(a|)` is the only one
    // whose trailing `a` domain can meet the final two `a+` groups; pruning by
    // score alone used to discard it and return false.
    const source = `^${'(b|bb|)'.repeat(7)}(a|)${'(a|aa|)'.repeat(6)}(a+)(a+)c$`;

    expect(source.match(/\([^)]*\|[^)]*\)/g)?.length).toBeGreaterThan(8);
    expect(isPotentiallyExpensiveRegex(source)).toBe(true);
  });
});
