...
// `[0-9]+`) is linear-time and safe — flagging it produced false negatives that
// suppressed step-definition links for ordinary `"([^"]+)"` patterns (see #859).
// Inner quantifiers covered: unbounded (`+`, `*`) and bounded (`{m}`, `{m,}`,
// `{m,n}`). Keep this in sync with the identical constant in
