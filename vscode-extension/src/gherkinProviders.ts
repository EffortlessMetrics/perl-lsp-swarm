...
// contains a quantifier* (e.g. `(a+)+`, `([a-z]{2,5})+`), a backreference,
...
// patterns (see #859). Bounded inner quantifiers `{m}`, `{m,}`, `{m,n}` in a
// quantified group (`([a-z]{2,5})+`) also backtrack super-linearly (see #953).
// Keep this in sync with the identical constant in gherkinStepDefinitions.ts.
const POTENTIALLY_EXPENSIVE_REGEX_RE = /(?:\([^)]*(?:[+*]|\{\d+(?:,\d*)?\})[^)]*\))[+*{]|\\[1-9]|\(\?<[=!]|(\(\?[!=])/;
