...
// contains a quantifier* (e.g. `(a+)+`, `([a-z]{2,5})+`), a backreference, or
// a lookaround. A single character class followed by one quantifier (`[^"]+`,
// `[0-9]+`) is linear-time and safe — flagging it produced false "ambiguous"
// classifications for ordinary step definitions, including the `"([^"]+)"`
// patterns this module generates itself (see buildGeneratedStepPattern).
// Inner quantifiers covered: unbounded (`+`, `*`) and bounded (`{m}`, `{m,}`,
// `{m,n}`). Keep this in sync with the identical constant in gherkinProviders.ts.
export const POTENTIALLY_EXPENSIVE_REGEX_RE = /(?:\([^)]*(?:[+*]|\{\d+(?:,\d*)?\})[^)]*\))[+*{]|\\[1-9]|\(\?<[=!]|(\(\?[!=])/;
