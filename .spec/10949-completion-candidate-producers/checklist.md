...
- [x] Every standard completion candidate producer and append site is inventoried from current source (53 rows, `syn`-derived over 38 tracked files).
...
- [x] A new hidden producer fails the check — proven by source mutation, not only by fixture.
- [x] A post-finalizer append fails the check — proven by source mutation.
...
- [x] No user-visible completion behavior changes. The only product-source edits in this PR are the two mutations, both reverted; the diff touches `xtask`, `policy/`, `docs/architecture/` and `.spec/` only.
