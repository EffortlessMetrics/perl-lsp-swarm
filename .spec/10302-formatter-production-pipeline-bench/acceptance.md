...
| NPC-008 | Future #10302 contract, not delivered by PR #13190: a checked-in authoritative registry covers every matrix member, and a distinct runtime sidecar has exactly one dedicated serialized receipt-pass row per registry key with strict Criterion/registry/sidecar joining. The current candidate has neither the checked-in registry nor the runtime sidecar/validator path; those remain `NOT_PROVEN`. | Future registry completeness, three-way join fixtures, receipt retention, and strict extraction/validation commands | NOT_PROVEN |
