...
| FMT-FINAL-008 | Stale/invalid/conflicting/unrepresentable plans are non-applied, while distinct same-position insertions preserve the independent oracle's input order | `reversed_ranges_are_rejected_not_clamped`; `unreachable_positions_are_rejected_not_clamped`; `mid_code_point_positions_are_rejected_in_both_encodings`; `adjacent_edits_are_allowed_but_overlap_is_rejected`; `distinct_zero_width_insertions_preserve_oracle_input_order` | offline |
