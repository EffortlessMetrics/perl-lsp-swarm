...
| A6 | Successors can consume the state without redesign | `enter` / `mark` / `restore` / `current_form` are form-agnostic |
...
**Behavioral acceptance** — `tests/class_grammar_context.rs`, 14 tests.
...
|---|---|---|
| A | `restore()` is a no-op — frames leak | 6 of 14 fail |
| B | `admits_class_members()` always true | 7 of 14 fail |
| C | `restore()` clears all frames instead of restoring the observed depth | 1 of 14 fails — `leaving_an_inner_class_body_restores_the_enclosing_class_context` |
