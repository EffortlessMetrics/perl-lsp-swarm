...
| Currentness table | every sibling row cites live state verified against pinned main SHA `9b7d333170e5` (2026-09-17) | context.md Status section |
...
| SOS-C02 | Bounded identity only: every present entry carries a digest (`sha256_content` or `directory_tree_digest`); filename, version, executable bit, or familiar path is never ownership; relative paths are exact — no absolute paths, drive letters, backslashes, colons (drive or NTFS alternate-data-stream syntax), control characters, trailing dot/space elidable spellings, DOS reserved device basenames, parent segments, empty segments, or glob/shell metacharacters — and absolute root identities carry the same Windows alias exclusions outside the drive prefix | entry rules M06–M09 in validator |
| SOS-C03 | Ownership classes are exactly the issue's eight values; class→retention disposition is one total fixed function; duplicate rows for one path are invalid, and a row inside another enumerated directory row is a double count (the directory row's digest already covers its subtree) | classification table + nested-row rule in validator |
...
| SOS-C08 | `standalone_removal_plan.v1` is pure and total: exactly one disposition per manifest row (remove_exact/remove_marker/preserve/revalidate); no actions outside manifest rows; remove_marker ⇔ marker roles; `verified_identity_sha256` is permitted on destructive dispositions only — preserve/revalidate rows carrying it are rejected; `order_index` is the enforced execution order — exactly the canonical 0-based sequence matching array position, so duplicates, gaps, permutations, and reversal are invalid and array/index order are one authority | plan totality rules P01–P03 + order rule P04 + evidence-binding rule |
...
| SOS-C17 | The packet's proof is hosted: the checked validator battery and the schema-only parity harness run in CI (`standalone-contract-tests.yml`) on any PR touching the packet, so contract drift cannot land without a hosted run noticing | `.github/workflows/standalone-contract-tests.yml` |
...
|---|---|---|---|
| This packet's successors | schemas + fixtures + validator | none | removal-executor successors under #11527 implement executors against these documents |
| #11179 (landed) / #11425–#11430 | candidate/selection identities | none | bind real digests when lanes land |
| #11467–#11469 (consolidated) | marker rows | none | compose ownership when persistence lands |
