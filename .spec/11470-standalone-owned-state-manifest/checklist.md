...
3. `fixtures/experience/install_owned_state/*.json` — six positive scenario
   manifests, three negative-control manifests, four plans (two valid by
...
4. `xtask/examples/standalone_owned_state.rs` — checked validator: closed
   typed structs, manifest coherence rules (identity bounds, running-state
   both-direction rule, total class→retention function), plan totality and
   destructive-legality matrix with exact-currentness digests, result
   vocabulary coherence, canonical serialization.
...
cargo test -p xtask --example standalone_owned_state --locked                            # 16 focused tests
