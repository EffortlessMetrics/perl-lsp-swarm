...
def publish_dependency_graph(packages: list[dict[str, Any]]) -> dict[str, set[str]]:
    """Use the shared publication graph policy rather than a second SCC implementation."""
    helper_path = Path(__file__).with_name("publish-topo.py")
...
def derive_crates(metadata: dict[str, Any]) -> list[dict[str, Any]]:
    ...
    dependencies = publish_dependency_graph(list(by_name.values()))
    dependencies = {
