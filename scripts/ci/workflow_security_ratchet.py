...
GH_AW_GENERATED_MARKER = "automatically generated by gh-aw"


def _is_gh_aw_generated(lines: Sequence[str]) -> bool:
    # gh-aw lock files are compiler output verified by the compile-check
    # workflow; their structure is not authored YAML the ratchet can require
    # to change. Scope the exemption to files carrying the generator marker.
    return any(GH_AW_GENERATED_MARKER in line for line in lines[:40])
...
def scan(
    ...
    *,
    ...
    for path in paths:
        ...
        gh_aw_generated = is_workflow and _is_gh_aw_generated(lines)
