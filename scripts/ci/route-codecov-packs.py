...
def package_test_targets(crate_name: str, repo_root: Path = REPO_ROOT) -> PackageTestTargets | None:
    ...
    # Cargo's edition-2015 auto-discovery rule is per target kind: only a
    # manually declared [[bin]] disables BINARY discovery.  An explicit [lib]
    # does not -- verified against `cargo metadata --no-deps`, which still
    # exposes the implicit src/main.rs bin for a 2015 package declaring [lib].
