...
from dap_scorecard_transport import frame_message, read_framed_message  # noqa: E402
...
def build_scorecard(
    ...
    for name in LAUNCH_FIXTURES:
        try:
            elapsed = probe_launch(binary, fixtures[name].resolve(), timeout_seconds)
    ...
    for _attempt in range(ATTACH_ATTEMPTS):
        try:
            elapsed = probe_attach(binary, timeout_seconds)
    ...
    try:
        ...
        detail = f"exact-binary stdio session failed: {exc}"
    ...
    return {
        ...
        "subject": {
            ...
            "process_invocations": REQUIRED_PROCESS_INVOCATIONS,
        },
        "perl_available": True,
        "perl": {"path": str(perl), "version": run_text([str(perl), "-e", "print $^V"])},
