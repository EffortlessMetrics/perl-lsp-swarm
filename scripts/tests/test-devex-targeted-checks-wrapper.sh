...
WRAPPER="${REPO_ROOT}/scripts/devex-targeted-checks.sh"
...
echo "=== devex-targeted-checks wrapper test suite ==="
echo ""

if [[ ! -f "$WRAPPER" ]]; then
  echo "ERROR: devex-targeted-checks.sh not found at ${WRAPPER}"
  exit 1
fi
...
(
  cd "$TMPDIR_BASE"
  PATH="${FAKE_BIN}:$PATH" bash "$WRAPPER" --base origin/main --mode tests
) > "${PASS_DIR}/out.txt" 2> "${PASS_DIR}/err.txt" || code=$?
assert_exit_zero "delegates to cargo xtask targeted-checks" "$code"
assert_args_equal "forwards targeted-checks arguments unchanged" "$EXPECTED_PASS_ARGS" "$FAKE_LOG"
...
(
  cd "$TMPDIR_BASE"
  PATH="${FAKE_BIN}:$PATH" FAKE_CARGO_EXIT=37 bash "$WRAPPER" --mode all
