...
WRAPPER="${REPO_ROOT}/scripts/test-lsp-cancellation.sh"
...
echo "=== test-lsp-cancellation wrapper test suite ==="
echo ""

if [[ ! -f "$WRAPPER" ]]; then
  echo "ERROR: test-lsp-cancellation.sh not found at ${WRAPPER}"
  exit 1
fi
...
(
  cd "$TMPDIR_BASE"
  PATH="${FAKE_BIN}:$PATH" bash "$WRAPPER" --nocapture
) > "${PASS_DIR}/out.txt" 2> "${PASS_DIR}/err.txt" || code=$?
assert_exit_zero "delegates to cargo xtask ci-hygiene test-lsp-cancellation" "$code"
assert_args_equal "forwards cancellation test arguments unchanged" "$EXPECTED_PASS_ARGS" "$FAKE_LOG"
...
(
  cd "$TMPDIR_BASE"
  PATH="${FAKE_BIN}:$PATH" FAKE_CARGO_EXIT=37 bash "$WRAPPER" --nocapture
