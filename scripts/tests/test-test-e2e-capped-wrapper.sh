...
WRAPPER="${REPO_ROOT}/scripts/test-e2e-capped.sh"
...
echo "=== test-e2e-capped wrapper test suite ==="
echo ""

if [[ ! -f "$WRAPPER" ]]; then
  echo "ERROR: test-e2e-capped.sh not found at ${WRAPPER}"
  exit 1
fi
...
(
  cd "$TMPDIR_BASE"
  PATH="${FAKE_BIN}:$PATH" bash "$WRAPPER" --package perl-lsp-rs-core
) > "${PASS_DIR}/out.txt" 2> "${PASS_DIR}/err.txt" || code=$?
assert_exit_zero "delegates to cargo xtask ci-hygiene test-e2e-capped" "$code"
assert_args_equal "forwards test-e2e-capped arguments unchanged" "$EXPECTED_PASS_ARGS" "$FAKE_LOG"
...
(
  cd "$TMPDIR_BASE"
  PATH="${FAKE_BIN}:$PATH" FAKE_CARGO_EXIT=37 bash "$WRAPPER" --package perl-lsp-rs-core
