...
"""Tests for the ci-audit-workflows compatibility shim."""
...
CI_AUDIT_WORKFLOWS = REPO_ROOT / "scripts" / "ci-audit-workflows.py"


class CiAuditWorkflowsShimTest(unittest.TestCase):
    ...
    def test_propagates_cargo_exit_code(self) -> None:
        ...
        self.assertEqual(
            calls[0]["args"],
            ["cargo", "xtask", "ci-audit-workflows", "--check"],
        )
    ...
    def _run_wrapper(self, argv: list[str], return_code: int) -> tuple[int, list[dict[str, object]]]:
        ...
        with mock.patch.object(sys, "argv", [str(CI_AUDIT_WORKFLOWS), *argv]):
            with mock.patch("subprocess.call", side_effect=fake_call):
                with self.assertRaises(SystemExit) as raised:
                    runpy.run_path(str(CI_AUDIT_WORKFLOWS), run_name="__main__")
