...
E2E_GATE_SCRIPT="${REPO_ROOT}/scripts/e2e-gate.sh"
...
echo "=== e2e-gate wrapper test suite ==="
echo ""

if [[ ! -f "$E2E_GATE_SCRIPT" ]]; then
  echo "ERROR: e2e-gate.sh not found at ${E2E_GATE_SCRIPT}"
  exit 1
fi
...
(
  cd "$REPO_ROOT"
  PATH="${FAKE_BIN}:$PATH" bash "$E2E_GATE_SCRIPT" --check
) > "${PASS_DIR}/out.txt" 2> "${PASS_DIR}/err.txt" || code=$?
assert_exit_zero "delegates to cargo xtask ci-hygiene e2e-gate" "$code"
assert_args_equal "forwards e2e-gate arguments unchanged" "$EXPECTED_PASS_ARGS" "$FAKE_LOG"
...
(
  cd "$REPO_ROOT"
  PATH="${FAKE_BIN}:$PATH" FAKE_CARGO_EXIT=37 bash "$E2E_GATE_SCRIPT" --check
