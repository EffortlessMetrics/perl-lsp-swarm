...
echo "=== preflight wrapper test suite ==="
echo ""

if [[ ! -f "$PREFLIGHT_SCRIPT" ]]; then
  echo "ERROR: preflight.sh not found at ${PREFLIGHT_SCRIPT}"
  exit 1
fi
...
(
  cd "$REPO_ROOT"
  PATH="${FAKE_BIN}:$PATH" bash "$PREFLIGHT_SCRIPT" --check
) > "${PASS_DIR}/out.txt" 2> "${PASS_DIR}/err.txt" || code=$?
assert_exit_zero "delegates to cargo xtask ci-hygiene preflight" "$code"
assert_args_equal "forwards preflight arguments unchanged" "$EXPECTED_PASS_ARGS" "$FAKE_LOG"
...
(
  cd "$REPO_ROOT"
  PATH="${FAKE_BIN}:$PATH" FAKE_CARGO_EXIT=37 bash "$PREFLIGHT_SCRIPT" --check
