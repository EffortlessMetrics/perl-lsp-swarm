...
WRAPPER="${REPO_ROOT}/scripts/ci-cost-monitor.sh"
...
echo "=== ci-cost-monitor wrapper test suite ==="
echo ""

if [[ ! -f "$WRAPPER" ]]; then
  echo "ERROR: ci-cost-monitor.sh not found at ${WRAPPER}"
  exit 1
fi
...
(
  cd "${TMPDIR_BASE}"
  PATH="${FAKE_BIN}:$PATH" bash "$WRAPPER" --days 14 --json
) > "${PASS_DIR}/out.txt" 2> "${PASS_DIR}/err.txt" || code=$?
assert_exit_zero "delegates to cargo xtask ci-cost-monitor" "$code"
assert_file_equals "runs from the wrapper repo root" "$EXPECTED_PWD" "$FAKE_PWD_LOG"
assert_file_equals "forwards ci-cost-monitor arguments unchanged" "$EXPECTED_ARGS" "$FAKE_ARGS_LOG"
...
(
  cd "$REPO_ROOT"
  PATH="${FAKE_BIN}:$PATH" FAKE_CARGO_EXIT=37 bash "$WRAPPER" --days 7
