...
def load_topology_json(raw: bytes | str, *, supported_versions=(1, 2)) -> dict[str, Any]:
    ...
    """
    allowed = " or ".join(str(version) for version in supported_versions)
    admission_error = f"release topology schema must appear once and be exactly {allowed}"
    exact = json.loads(
        raw,
