...
public-api-check:
    ...
    # Keep the executable policy in one tested adapter so check and update cannot
    # drift in their generator, empty-output, or baseline-handling semantics.
    bash scripts/ci/public-api-ratchet.sh check
...
public-api-update:
    ...
    bash scripts/ci/public-api-ratchet.sh update
