...
temporary plans, queue order, proof-token allocation, and runtime frontier remain
runtime-local.
...
- A focused subagent reads only the named skill, bounded question, accepted facts, and
  nearest guidance.
...
boundary, or a precise `NOT_PROVEN` boundary—not at research, a plan, one subagent
return, or green checks that do not complete the claim.
...
The normal claim owner is one persistent lane per PR or coherent claim. It keeps the
claim loaded across review, repair, proof, review refresh, live CI, and closeout.

A dedicated review context is useful when independent attention matters. One reviewer
may consume `review-pr`, `review-candidate`, `review-tests`, external-oracle,
...
subagent per skill or review lens merely to repeat ingestion.
...
Do not write subagent handles, liveness, queue order, retries, ordinary skill
transitions, private reasoning, or temporary frontier state into GitHub or tracked files.
...
Do not return an intermediate review packet merely so another subagent can rediscover
the PR. Split to a new claim lane only when the durable claim itself splits or a separate
...
subagent verdict. It proportionately challenges proof discrimination, production
...
- retain a claim worktree across near-term review, repair, and proof when the same context
  will continue;
