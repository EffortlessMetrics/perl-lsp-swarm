...
/// no more than 59 (which corresponds to 74 − 15 = 59). Later collapse waves
/// may reduce the count further.
#[test]
fn test_published_crate_count_stays_below_g1a_inventory() -> Result<(), Box<dyn std::error::Error>>
{
    let root = project_root();
    ...
    assert!(
        count <= 59,
        "G1a collapse: published crate count must stay at or below 59.\n\
         Before G1a: 74 crates (includes 15 providers)\n\
         G1a after: 59 crates (15 providers collapsed into perl-lsp-rs-core)\n\
