...
/// CommonMark parser. Heading detection narrows to rendered-prose lines, and
/// requirement evaluation restricts its input further through
/// `rendered_prose`, so code-block text and HTML comments cannot satisfy
/// markers.
...
/// Remove HTML comments, including comments spanning multiple lines, so
/// markers hidden from rendered Markdown cannot satisfy the contract. An
/// unterminated comment swallows the remainder of the text: stripping can
/// only make validation stricter, never more permissive.
...
/// Reduce extracted section text to rendered prose before requirement
/// evaluation: HTML comments are removed and fenced or indented code lines are
/// dropped, so required markers count only when they are visible in the
/// rendered document.
fn rendered_prose(text: &str) -> String {
    let without_comments = strip_html_comments(text);
    let flags = prose_flags(&without_comments);

    without_comments
        .lines()
        .zip(flags)
        .filter_map(|(line, prose)| prose.then_some(line))
        .collect::<Vec<_>>()
        .join("\n")
}
...
fn validate_subsection(
    ...
    match h3_heading_count(section, heading) {
        ...
        1 => h3_section(section, heading)
            .map(|subsection| {
                validate_requirements(scope, &rendered_prose(&subsection), requirements)
            })
            .unwrap_or_else(|| {
...
fn validate(text: &str) -> Vec<String> {
    // Hidden text cannot establish admission: strip comments before heading
    // detection so a commented-out admission section is simply absent.
    ...
    failures.extend(validate_requirements(
        ...
        &rendered_prose(&h3_preamble(&section)),
...
#[test]
fn complete_admission_hidden_in_html_comment_fails_closed() {
    let text = format!(
        r#"
## Procedure
Earlier content.
...
"#
    ...
    assert!(
        errors[0].starts_with(&format!("missing section '{SECTION_HEADING}'")),
        "comment-hidden admission must read as absent: {}",
        errors[0]
    );
}

#[test]
fn markers_only_inside_fenced_code_under_genuine_subsections_fail_closed() {
    let text = format!(
        r#"
## Mutation admission
Before the accountable root edits the candidate directly or delegates any candidate
...
"#
    ...
    assert!(
        errors.iter().any(|error| error.contains("semantic key is missing claim")),
        "fence-hidden semantic markers must not count: {:?}",
        errors
    );
    assert!(
        errors.iter().any(|error| error.contains("mechanical key is missing repository identity")),
        "fence-hidden mechanical markers must not count: {:?}",
        errors
    );
    assert!(
        errors.iter().any(|error| error.contains("same-subject join is missing same-subject join")),
        "fence-hidden join markers must not count: {:?}",
        errors
    );
}

#[test]
fn markers_only_inside_indented_code_under_genuine_subsections_fail_closed() {
    let text = format!(
        r#"
## Mutation admission
Before the accountable root edits the candidate directly or delegates any candidate
...
"#
    ...
    assert!(
        errors.iter().any(|error| error.contains("semantic key is missing claim")),
        "indent-hidden semantic markers must not count: {:?}",
        errors
    );
    assert!(
        errors.iter().any(|error| error.contains("mechanical key is missing repository identity")),
        "indent-hidden mechanical markers must not count: {:?}",
        errors
    );
    assert!(
        errors.iter().any(|error| error.contains("same-subject join is missing same-subject join")),
        "indent-hidden join markers must not count: {:?}",
        errors
    );
}
