...
#[test]
fn coverage_pack_manifest_declares_ci_audit_workflows_shim_pack_owns_shim() -> Result<()> {
    let manifest_path = PathBuf::from(env!("CARGO_MANIFEST_DIR"))
        .parent()
        .map(PathBuf::from)
        .ok_or_else(|| anyhow!("xtask manifest path has no parent"))?
    ...
    let pack = packs
        ...
        .find(|pack| {
            pack.get("id").and_then(toml::Value::as_str)
                == Some("patch-coverage-ci-audit-workflows-shim")
        })
        .ok_or_else(|| anyhow!("missing ci-audit-workflows shim coverage pack"))?;
    let files = pack
        .get("files")
        .and_then(toml::Value::as_array)
        .ok_or_else(|| anyhow!("coverage pack files must be an array"))?;
    ...
    assert!(
        files
            .iter()
            .filter_map(toml::Value::as_str)
            .any(|value| value == "scripts/ci-audit-workflows.py")
    );
    assert!(
        files
            .iter()
            .filter_map(toml::Value::as_str)
            .any(|value| value == "scripts/tests/test-ci-audit-workflows-shim.py")
    );
    assert!(
        commands
            .iter()
            .filter_map(toml::Value::as_str)
            .any(|value| value == "python scripts/tests/test-ci-audit-workflows-shim.py")
    );
    Ok(())
}
...
#[test]
fn coverage_pack_manifest_declares_devex_doctor_pack_owns_doctor_wrapper() -> Result<()> {
    let manifest_path = PathBuf::from(env!("CARGO_MANIFEST_DIR"))
        .parent()
        .map(PathBuf::from)
        .ok_or_else(|| anyhow!("xtask manifest path has no parent"))?
    ...
    let pack = packs
        ...
        .find(|pack| {
            pack.get("id").and_then(toml::Value::as_str)
                == Some("patch-coverage-devex-doctor-wrapper")
        })
        .ok_or_else(|| anyhow!("missing devex-doctor wrapper coverage pack"))?;
    let files = pack
        .get("files")
        .and_then(toml::Value::as_array)
        .ok_or_else(|| anyhow!("coverage pack files must be an array"))?;
    ...
    assert!(
        files
            .iter()
            .filter_map(toml::Value::as_str)
            .any(|value| value == "scripts/devex-doctor.sh")
    );
    assert!(
        files
            .iter()
            .filter_map(toml::Value::as_str)
            .any(|value| value == "scripts/tests/test-devex-doctor-wrapper.sh")
    );
    assert!(
        commands
            .iter()
            .filter_map(toml::Value::as_str)
            .any(|value| value == "bash scripts/tests/test-devex-doctor-wrapper.sh")
    );
    Ok(())
}

#[test]
fn coverage_pack_manifest_declares_devex_targeted_pack_owns_targeted_wrapper() -> Result<()> {
    let manifest_path = PathBuf::from(env!("CARGO_MANIFEST_DIR"))
        .parent()
        .map(PathBuf::from)
        .ok_or_else(|| anyhow!("xtask manifest path has no parent"))?
    ...
    let pack = packs
        ...
        .find(|pack| {
            pack.get("id").and_then(toml::Value::as_str)
                == Some("patch-coverage-devex-targeted-checks-wrapper")
        })
        .ok_or_else(|| anyhow!("missing devex-targeted-checks wrapper coverage pack"))?;
    let files = pack
        .get("files")
        .and_then(toml::Value::as_array)
        .ok_or_else(|| anyhow!("coverage pack files must be an array"))?;
    ...
    assert!(
        files
            .iter()
            .filter_map(toml::Value::as_str)
            .any(|value| value == "scripts/devex-targeted-checks.sh")
    );
    assert!(
        files
            .iter()
            .filter_map(toml::Value::as_str)
            .any(|value| value == "scripts/tests/test-devex-targeted-checks-wrapper.sh")
    );
    assert!(
        commands
            .iter()
            .filter_map(toml::Value::as_str)
            .any(|value| value == "bash scripts/tests/test-devex-targeted-checks-wrapper.sh")
    );
    Ok(())
}

#[test]
fn coverage_pack_manifest_declares_lsp_cancellation_pack_owns_cancellation_wrapper() -> Result<()> {
    let manifest_path = PathBuf::from(env!("CARGO_MANIFEST_DIR"))
        .parent()
        .map(PathBuf::from)
        .ok_or_else(|| anyhow!("xtask manifest path has no parent"))?
    ...
    let pack = packs
        ...
        .find(|pack| {
            pack.get("id").and_then(toml::Value::as_str)
                == Some("patch-coverage-lsp-cancellation-wrapper")
        })
        .ok_or_else(|| anyhow!("missing lsp-cancellation wrapper coverage pack"))?;
    let files = pack
        .get("files")
        .and_then(toml::Value::as_array)
        .ok_or_else(|| anyhow!("coverage pack files must be an array"))?;
    ...
    assert!(
        files
            .iter()
            .filter_map(toml::Value::as_str)
            .any(|value| value == "scripts/test-lsp-cancellation.sh")
    );
    assert!(
        files
            .iter()
            .filter_map(toml::Value::as_str)
            .any(|value| value == "scripts/tests/test-lsp-cancellation-wrapper.sh")
    );
    assert!(
        commands
            .iter()
            .filter_map(toml::Value::as_str)
            .any(|value| value == "bash scripts/tests/test-lsp-cancellation-wrapper.sh")
    );
    Ok(())
}

#[test]
fn coverage_pack_manifest_declares_test_capped_pack_owns_test_capped_wrapper() -> Result<()> {
    let manifest_path = PathBuf::from(env!("CARGO_MANIFEST_DIR"))
        .parent()
        .map(PathBuf::from)
        .ok_or_else(|| anyhow!("xtask manifest path has no parent"))?
    ...
    let pack = packs
        ...
        .find(|pack| {
            pack.get("id").and_then(toml::Value::as_str)
                == Some("patch-coverage-test-capped-wrapper")
        })
        .ok_or_else(|| anyhow!("missing test-capped wrapper coverage pack"))?;
    let files = pack
        .get("files")
        .and_then(toml::Value::as_array)
        .ok_or_else(|| anyhow!("coverage pack files must be an array"))?;
    ...
    assert!(
        files.iter().filter_map(toml::Value::as_str).any(|value| value == "scripts/test-capped.sh")
    );
    assert!(
        files
            .iter()
            .filter_map(toml::Value::as_str)
            .any(|value| value == "scripts/tests/test-test-capped-wrapper.sh")
    );
    assert!(
        commands
            .iter()
            .filter_map(toml::Value::as_str)
            .any(|value| value == "bash scripts/tests/test-test-capped-wrapper.sh")
    );
    Ok(())
}

#[test]
fn coverage_pack_manifest_declares_test_e2e_capped_pack_owns_test_e2e_capped_wrapper() -> Result<()>
{
    let manifest_path = PathBuf::from(env!("CARGO_MANIFEST_DIR"))
        .parent()
        .map(PathBuf::from)
    ...
    let pack = packs
        ...
        .find(|pack| {
            pack.get("id").and_then(toml::Value::as_str)
                == Some("patch-coverage-test-e2e-capped-wrapper")
        })
        .ok_or_else(|| anyhow!("missing test-e2e-capped wrapper coverage pack"))?;
    let files = pack
        .get("files")
        .and_then(toml::Value::as_array)
        .ok_or_else(|| anyhow!("coverage pack files must be an array"))?;
    ...
    assert!(
        files
            .iter()
            .filter_map(toml::Value::as_str)
            .any(|value| value == "scripts/test-e2e-capped.sh")
    );
    assert!(
        files
            .iter()
            .filter_map(toml::Value::as_str)
            .any(|value| value == "scripts/tests/test-test-e2e-capped-wrapper.sh")
    );
    assert!(
        commands
            .iter()
            .filter_map(toml::Value::as_str)
            .any(|value| value == "bash scripts/tests/test-test-e2e-capped-wrapper.sh")
    );
    Ok(())
}

#[test]
fn coverage_pack_manifest_declares_ci_cost_monitor_pack_owns_ci_cost_monitor_wrapper() -> Result<()>
{
    let manifest_path = PathBuf::from(env!("CARGO_MANIFEST_DIR"))
        .parent()
        .map(PathBuf::from)
    ...
    let pack = packs
        ...
        .find(|pack| {
            pack.get("id").and_then(toml::Value::as_str)
                == Some("patch-coverage-ci-cost-monitor-wrapper")
        })
        .ok_or_else(|| anyhow!("missing ci-cost-monitor wrapper coverage pack"))?;
    let files = pack
        .get("files")
        .and_then(toml::Value::as_array)
        .ok_or_else(|| anyhow!("coverage pack files must be an array"))?;
    ...
    assert!(
        files
            .iter()
            .filter_map(toml::Value::as_str)
            .any(|value| value == "scripts/ci-cost-monitor.sh")
    );
    assert!(
        files
            .iter()
            .filter_map(toml::Value::as_str)
            .any(|value| value == "scripts/tests/test-ci-cost-monitor-wrapper.sh")
    );
    assert!(
        commands
            .iter()
            .filter_map(toml::Value::as_str)
            .any(|value| value == "bash scripts/tests/test-ci-cost-monitor-wrapper.sh")
    );
    Ok(())
}

#[test]
fn coverage_pack_manifest_declares_build_timing_pack_owns_build_timing_wrapper() -> Result<()> {
    let manifest_path = PathBuf::from(env!("CARGO_MANIFEST_DIR"))
        .parent()
        .map(PathBuf::from)
        .ok_or_else(|| anyhow!("xtask manifest path has no parent"))?
    ...
    let pack = packs
        ...
        .find(|pack| {
            pack.get("id").and_then(toml::Value::as_str)
                == Some("patch-coverage-build-timing-receipt-wrapper")
        })
        .ok_or_else(|| anyhow!("missing build-timing receipt wrapper coverage pack"))?;
    let files = pack
        .get("files")
        .and_then(toml::Value::as_array)
        .ok_or_else(|| anyhow!("coverage pack files must be an array"))?;
    ...
    assert!(
        files
            .iter()
            .filter_map(toml::Value::as_str)
            .any(|value| value == "scripts/build-timing-receipt.sh")
    );
    assert!(
        files
            .iter()
            .filter_map(toml::Value::as_str)
            .any(|value| value == "scripts/tests/test-build-timing-receipt-wrapper.sh")
    );
    assert!(
        commands
            .iter()
            .filter_map(toml::Value::as_str)
            .any(|value| value == "bash scripts/tests/test-build-timing-receipt-wrapper.sh")
    );
    Ok(())
}
...
#[test]
fn coverage_pack_manifest_declares_publish_receipts_pack_owns_receipt_wrapper() -> Result<()> {
    let manifest_path = PathBuf::from(env!("CARGO_MANIFEST_DIR"))
        .parent()
        .map(PathBuf::from)
        .ok_or_else(|| anyhow!("xtask manifest path has no parent"))?
    ...
    let pack = packs
        ...
        .find(|pack| {
            pack.get("id").and_then(toml::Value::as_str)
                == Some("patch-coverage-publish-receipts-wrapper")
        })
        .ok_or_else(|| anyhow!("missing publish-receipts wrapper coverage pack"))?;
    let files = pack
        .get("files")
        .and_then(toml::Value::as_array)
        .ok_or_else(|| anyhow!("coverage pack files must be an array"))?;
    ...
    assert!(
        files
            .iter()
            .filter_map(toml::Value::as_str)
            .any(|value| value == "scripts/publish-receipts.sh")
    );
    assert!(
        commands
            .iter()
            .filter_map(toml::Value::as_str)
            .any(|value| value == "bash scripts/tests/test-publish-receipts-wrapper.sh")
    );
    Ok(())
}
...
#[test]
fn ci_route_cli_maps_ci_audit_workflows_shim_to_focused_proof_pack() -> Result<()> {
    let temp = TempDir::new()?;
    let receipt = temp.path().join("ci-route.json");
    let summary = temp.path().join("ci-route.md");
    ...
    assert_eq!(
        route.pointer("/changed_surfaces/0").and_then(Value::as_str),
        Some("ci-audit-workflows-shim")
    );
    assert!(
        route.get("required_proof_packs").and_then(Value::as_array).is_some_and(|packs| packs
            .iter()
            .any(|pack| {
                pack.get("id").and_then(Value::as_str) == Some("ci-audit-workflows-shim-focused")
                    && pack.get("commands").and_then(Value::as_array).is_some_and(|commands| {
                        commands.iter().any(|command| {
                            command.as_str()
                                == Some("python scripts/tests/test-ci-audit-workflows-shim.py")
                        })
                    })
            })),
        "ci-audit-workflows shim changes must run focused proof"
    );
    assert!(
        route.get("coverage_pack_selector").and_then(Value::as_array).is_some_and(Vec::is_empty),
        "ci-audit-workflows shim proof pack is non-LCOV and must not be uploaded as Codecov coverage"
    );
    assert_eq!(
        route
            .pointer("/skipped_by_policy/patch-coverage-ci-audit-workflows-shim")
            .and_then(Value::as_str),
        Some("non-LCOV CI policy/routing surface; covered by focused CI gates")
    );
    let summary = fs::read_to_string(summary)?;
    assert!(summary.contains("python scripts/tests/test-ci-audit-workflows-shim.py"));
    assert!(
        summary.contains(
...
#[test]
fn ci_route_cli_maps_debt_report_shim_to_focused_proof_pack() -> Result<()> {
    let temp = TempDir::new()?;
    let receipt = temp.path().join("ci-route.json");
    let summary = temp.path().join("ci-route.md");
    ...
    assert_eq!(
        route.pointer("/changed_surfaces/0").and_then(Value::as_str),
        Some("debt-report-shim")
    );
    assert!(
        route.get("required_proof_packs").and_then(Value::as_array).is_some_and(|packs| packs
            .iter()
            .any(|pack| {
                pack.get("id").and_then(Value::as_str) == Some("debt-report-shim-focused")
                    && pack.get("commands").and_then(Value::as_array).is_some_and(|commands| {
                        commands.iter().any(|command| {
                            command.as_str()
                                == Some("python scripts/tests/test-debt-report-shim.py")
                        })
                    })
            })),
        "debt report shim changes must run focused proof"
    );
    assert!(
        route.get("coverage_pack_selector").and_then(Value::as_array).is_some_and(Vec::is_empty),
        "debt report shim proof pack is non-LCOV and must not be uploaded as Codecov coverage"
    );
    assert_eq!(
        route.pointer("/skipped_by_policy/patch-coverage-debt-report-shim").and_then(Value::as_str),
        Some("non-LCOV CI policy/routing surface; covered by focused CI gates")
    );
    let summary = fs::read_to_string(summary)?;
    assert!(summary.contains("python scripts/tests/test-debt-report-shim.py"));
    assert!(
        summary.contains("`patch-coverage-debt-report-shim`: non-LCOV CI policy/routing surface")
    );
    Ok(())
}

#[test]
fn ci_route_cli_maps_debt_pr_summary_shim_to_focused_proof_pack() -> Result<()> {
    let temp = TempDir::new()?;
    let receipt = temp.path().join("ci-route.json");
    let summary = temp.path().join("ci-route.md");
    ...
    assert_eq!(
        route.pointer("/changed_surfaces/0").and_then(Value::as_str),
        Some("debt-pr-summary-shim")
    );
    assert!(
        route.get("required_proof_packs").and_then(Value::as_array).is_some_and(|packs| packs
            .iter()
            .any(|pack| {
                pack.get("id").and_then(Value::as_str) == Some("debt-pr-summary-shim-focused")
                    && pack.get("commands").and_then(Value::as_array).is_some_and(|commands| {
                        commands.iter().any(|command| {
                            command.as_str()
                                == Some("python scripts/tests/test-debt-pr-summary-shim.py")
                        })
                    })
            })),
        "debt PR summary shim changes must run focused proof"
    );
    assert!(
        route.get("coverage_pack_selector").and_then(Value::as_array).is_some_and(Vec::is_empty),
        "debt PR summary shim proof pack is non-LCOV and must not be uploaded as Codecov coverage"
    );
    assert_eq!(
        route
            .pointer("/skipped_by_policy/patch-coverage-debt-pr-summary-shim")
            .and_then(Value::as_str),
        Some("non-LCOV CI policy/routing surface; covered by focused CI gates")
    );
    let summary = fs::read_to_string(summary)?;
    assert!(summary.contains("python scripts/tests/test-debt-pr-summary-shim.py"));
    assert!(
        summary
            .contains("`patch-coverage-debt-pr-summary-shim`: non-LCOV CI policy/routing surface")
    );
    Ok(())
}
...
#[test]
fn ci_route_cli_maps_update_parser_matrix_shim_to_focused_proof_pack() -> Result<()> {
    let temp = TempDir::new()?;
    let receipt = temp.path().join("ci-route.json");
    let summary = temp.path().join("ci-route.md");
    ...
    assert_eq!(
        route.pointer("/changed_surfaces/0").and_then(Value::as_str),
        Some("update-parser-matrix-shim")
    );
    assert!(
        route.get("required_proof_packs").and_then(Value::as_array).is_some_and(|packs| packs
            .iter()
            .any(|pack| {
                pack.get("id").and_then(Value::as_str) == Some("update-parser-matrix-shim-focused")
                    && pack.get("commands").and_then(Value::as_array).is_some_and(|commands| {
                        commands.iter().any(|command| {
                            command.as_str()
                                == Some("python scripts/tests/test-update-parser-matrix-shim.py")
                        })
                    })
            })),
        "update-parser-matrix shim changes must run focused proof"
    );
    assert!(
        route.get("coverage_pack_selector").and_then(Value::as_array).is_some_and(Vec::is_empty),
        "update-parser-matrix shim proof pack is non-LCOV and must not be uploaded as Codecov coverage"
    );
    assert_eq!(
        route
            .pointer("/skipped_by_policy/patch-coverage-update-parser-matrix-shim")
            .and_then(Value::as_str),
        Some("non-LCOV CI policy/routing surface; covered by focused CI gates")
    );
    let summary = fs::read_to_string(summary)?;
    assert!(summary.contains("python scripts/tests/test-update-parser-matrix-shim.py"));
    assert!(summary.contains(
        "`patch-coverage-update-parser-matrix-shim`: non-LCOV CI policy/routing surface"
    ));
    Ok(())
}
...
#[test]
fn ci_route_cli_maps_devex_doctor_wrapper_to_doctor_proof_pack() -> Result<()> {
    let temp = TempDir::new()?;
    let receipt = temp.path().join("ci-route.json");
    let summary = temp.path().join("ci-route.md");
    ...
    assert_eq!(
        route.pointer("/changed_surfaces/0").and_then(Value::as_str),
        Some("devex-doctor-wrapper")
    );
    assert!(
        route.get("required_proof_packs").and_then(Value::as_array).is_some_and(|packs| packs
            .iter()
            .any(|pack| {
                pack.get("id").and_then(Value::as_str) == Some("devex-doctor-wrapper-focused")
                    && pack.get("commands").and_then(Value::as_array).is_some_and(|commands| {
                        commands.iter().any(|command| {
                            command.as_str()
                                == Some("bash scripts/tests/test-devex-doctor-wrapper.sh")
                        })
                    })
            })),
        "devex-doctor wrapper changes must run focused proof"
    );
    assert!(
        route.get("coverage_pack_selector").and_then(Value::as_array).is_some_and(Vec::is_empty),
        "devex-doctor wrapper proof pack is non-LCOV and must not be uploaded as Codecov coverage"
    );
    assert_eq!(
        route
            .pointer("/skipped_by_policy/patch-coverage-devex-doctor-wrapper")
            .and_then(Value::as_str),
        Some("non-LCOV CI policy/routing surface; covered by focused CI gates")
    );
    let summary = fs::read_to_string(summary)?;
    assert!(summary.contains("bash scripts/tests/test-devex-doctor-wrapper.sh"));
    assert!(
        summary
            .contains("`patch-coverage-devex-doctor-wrapper`: non-LCOV CI policy/routing surface")
    );
    Ok(())
}

#[test]
fn ci_route_cli_maps_devex_targeted_wrapper_to_targeted_proof_pack() -> Result<()> {
    let temp = TempDir::new()?;
    let receipt = temp.path().join("ci-route.json");
    let summary = temp.path().join("ci-route.md");
    ...
    assert_eq!(
        route.pointer("/changed_surfaces/0").and_then(Value::as_str),
        Some("devex-targeted-checks-wrapper")
    );
    assert!(
        route.get("required_proof_packs").and_then(Value::as_array).is_some_and(|packs| packs
            .iter()
            .any(|pack| {
                pack.get("id").and_then(Value::as_str)
                    == Some("devex-targeted-checks-wrapper-focused")
                    && pack.get("commands").and_then(Value::as_array).is_some_and(|commands| {
                        commands.iter().any(|command| {
                            command.as_str()
                                == Some("bash scripts/tests/test-devex-targeted-checks-wrapper.sh")
                        })
                    })
            })),
        "devex-targeted-checks wrapper changes must run focused proof"
    );
    assert!(
        route.get("coverage_pack_selector").and_then(Value::as_array).is_some_and(Vec::is_empty),
        "devex-targeted-checks wrapper proof pack is non-LCOV and must not be uploaded as Codecov coverage"
    );
    assert_eq!(
        route
            .pointer("/skipped_by_policy/patch-coverage-devex-targeted-checks-wrapper")
            .and_then(Value::as_str),
        Some("non-LCOV CI policy/routing surface; covered by focused CI gates")
    );
    let summary = fs::read_to_string(summary)?;
    assert!(summary.contains("bash scripts/tests/test-devex-targeted-checks-wrapper.sh"));
    assert!(summary.contains(
        "`patch-coverage-devex-targeted-checks-wrapper`: non-LCOV CI policy/routing surface"
    ));
    Ok(())
}

#[test]
fn ci_route_cli_maps_lsp_cancellation_wrapper_to_cancellation_proof_pack() -> Result<()> {
    let temp = TempDir::new()?;
    let receipt = temp.path().join("ci-route.json");
    let summary = temp.path().join("ci-route.md");
    ...
    assert_eq!(
        route.pointer("/changed_surfaces/0").and_then(Value::as_str),
        Some("lsp-cancellation-wrapper")
    );
    assert!(
        route.get("required_proof_packs").and_then(Value::as_array).is_some_and(|packs| packs
            .iter()
            .any(|pack| {
                pack.get("id").and_then(Value::as_str) == Some("lsp-cancellation-wrapper-focused")
                    && pack.get("commands").and_then(Value::as_array).is_some_and(|commands| {
                        commands.iter().any(|command| {
                            command.as_str()
                                == Some("bash scripts/tests/test-lsp-cancellation-wrapper.sh")
                        })
                    })
            })),
        "test-lsp-cancellation wrapper changes must run focused proof"
    );
    assert!(
        route.get("coverage_pack_selector").and_then(Value::as_array).is_some_and(Vec::is_empty),
        "test-lsp-cancellation wrapper proof pack is non-LCOV and must not be uploaded as Codecov coverage"
    );
    assert_eq!(
        route
            .pointer("/skipped_by_policy/patch-coverage-lsp-cancellation-wrapper")
            .and_then(Value::as_str),
        Some("non-LCOV CI policy/routing surface; covered by focused CI gates")
    );
    let summary = fs::read_to_string(summary)?;
    assert!(summary.contains("bash scripts/tests/test-lsp-cancellation-wrapper.sh"));
    assert!(
        summary.contains(
            "`patch-coverage-lsp-cancellation-wrapper`: non-LCOV CI policy/routing surface"
        )
    );
    Ok(())
}
...
#[test]
fn ci_route_cli_maps_test_capped_wrapper_to_test_capped_proof_pack() -> Result<()> {
    let temp = TempDir::new()?;
    let receipt = temp.path().join("ci-route.json");
    let summary = temp.path().join("ci-route.md");
    ...
    assert_eq!(
        route.pointer("/changed_surfaces/0").and_then(Value::as_str),
        Some("test-capped-wrapper")
    );
    assert!(
        route.get("required_proof_packs").and_then(Value::as_array).is_some_and(|packs| packs
            .iter()
            .any(|pack| {
                pack.get("id").and_then(Value::as_str) == Some("test-capped-wrapper-focused")
                    && pack.get("commands").and_then(Value::as_array).is_some_and(|commands| {
                        commands.iter().any(|command| {
                            command.as_str()
                                == Some("bash scripts/tests/test-test-capped-wrapper.sh")
                        })
                    })
            })),
        "test-capped wrapper changes must run focused proof"
    );
    assert!(
        route.get("coverage_pack_selector").and_then(Value::as_array).is_some_and(Vec::is_empty),
        "test-capped wrapper proof pack is non-LCOV and must not be uploaded as Codecov coverage"
    );
    assert_eq!(
        route
            .pointer("/skipped_by_policy/patch-coverage-test-capped-wrapper")
            .and_then(Value::as_str),
        Some("non-LCOV CI policy/routing surface; covered by focused CI gates")
    );
    let summary = fs::read_to_string(summary)?;
    assert!(summary.contains("bash scripts/tests/test-test-capped-wrapper.sh"));
    assert!(
        summary
            .contains("`patch-coverage-test-capped-wrapper`: non-LCOV CI policy/routing surface")
    );
    Ok(())
}

#[test]
fn ci_route_cli_maps_test_e2e_capped_wrapper_to_test_e2e_capped_proof_pack() -> Result<()> {
    let temp = TempDir::new()?;
    let receipt = temp.path().join("ci-route.json");
    let summary = temp.path().join("ci-route.md");
    ...
    assert_eq!(
        route.pointer("/changed_surfaces/0").and_then(Value::as_str),
        Some("test-e2e-capped-wrapper")
    );
    assert!(
        route.get("required_proof_packs").and_then(Value::as_array).is_some_and(|packs| packs
            .iter()
            .any(|pack| {
                pack.get("id").and_then(Value::as_str) == Some("test-e2e-capped-wrapper-focused")
                    && pack.get("commands").and_then(Value::as_array).is_some_and(|commands| {
                        commands.iter().any(|command| {
                            command.as_str()
                                == Some("bash scripts/tests/test-test-e2e-capped-wrapper.sh")
                        })
                    })
            })),
        "test-e2e-capped wrapper changes must run focused proof"
    );
    assert!(
        route.get("coverage_pack_selector").and_then(Value::as_array).is_some_and(Vec::is_empty),
        "test-e2e-capped wrapper proof pack is non-LCOV and must not be uploaded as Codecov coverage"
    );
    assert_eq!(
        route
            .pointer("/skipped_by_policy/patch-coverage-test-e2e-capped-wrapper")
            .and_then(Value::as_str),
        Some("non-LCOV CI policy/routing surface; covered by focused CI gates")
    );
    let summary = fs::read_to_string(summary)?;
    assert!(summary.contains("bash scripts/tests/test-test-e2e-capped-wrapper.sh"));
    assert!(
        summary.contains(
            "`patch-coverage-test-e2e-capped-wrapper`: non-LCOV CI policy/routing surface"
        )
    );
    Ok(())
}
...
#[test]
fn ci_route_cli_maps_ci_cost_monitor_wrapper_to_ci_cost_monitor_proof_pack() -> Result<()> {
    let temp = TempDir::new()?;
    let receipt = temp.path().join("ci-route.json");
    let summary = temp.path().join("ci-route.md");
    ...
    assert_eq!(
        route.pointer("/changed_surfaces/0").and_then(Value::as_str),
        Some("ci-cost-monitor-wrapper")
    );
    assert!(
        route.get("required_proof_packs").and_then(Value::as_array).is_some_and(|packs| packs
            .iter()
            .any(|pack| {
                pack.get("id").and_then(Value::as_str) == Some("ci-cost-monitor-wrapper-focused")
                    && pack.get("commands").and_then(Value::as_array).is_some_and(|commands| {
                        commands.iter().any(|command| {
                            command.as_str()
                                == Some("bash scripts/tests/test-ci-cost-monitor-wrapper.sh")
                        })
                    })
            })),
        "ci-cost-monitor wrapper changes must run focused proof"
    );
    assert!(
        route.get("coverage_pack_selector").and_then(Value::as_array).is_some_and(Vec::is_empty),
        "ci-cost-monitor wrapper proof pack is non-LCOV and must not be uploaded as Codecov coverage"
    );
    assert_eq!(
        route
            .pointer("/skipped_by_policy/patch-coverage-ci-cost-monitor-wrapper")
            .and_then(Value::as_str),
        Some("non-LCOV CI policy/routing surface; covered by focused CI gates")
    );
    let summary = fs::read_to_string(summary)?;
    assert!(summary.contains("bash scripts/tests/test-ci-cost-monitor-wrapper.sh"));
    assert!(
        summary.contains(
            "`patch-coverage-ci-cost-monitor-wrapper`: non-LCOV CI policy/routing surface"
        )
    );
    Ok(())
}
...
#[test]
fn ci_route_cli_maps_build_timing_wrapper_to_build_timing_proof_pack() -> Result<()> {
    let temp = TempDir::new()?;
    let receipt = temp.path().join("ci-route.json");
    let summary = temp.path().join("ci-route.md");
    ...
    assert_eq!(
        route.pointer("/changed_surfaces/0").and_then(Value::as_str),
        Some("build-timing-receipt-wrapper")
    );
    assert!(
        route.get("required_proof_packs").and_then(Value::as_array).is_some_and(|packs| packs
            .iter()
            .any(|pack| {
                pack.get("id").and_then(Value::as_str)
                    == Some("build-timing-receipt-wrapper-focused")
                    && pack.get("commands").and_then(Value::as_array).is_some_and(|commands| {
                        commands.iter().any(|command| {
                            command.as_str()
                                == Some("bash scripts/tests/test-build-timing-receipt-wrapper.sh")
                        })
                    })
            })),
        "build-timing receipt wrapper changes must run focused proof"
    );
    assert!(
        route.get("coverage_pack_selector").and_then(Value::as_array).is_some_and(Vec::is_empty),
        "build-timing receipt wrapper proof pack is non-LCOV and must not be uploaded as Codecov coverage"
    );
    assert_eq!(
        route
            .pointer("/skipped_by_policy/patch-coverage-build-timing-receipt-wrapper")
            .and_then(Value::as_str),
        Some("non-LCOV CI policy/routing surface; covered by focused CI gates")
    );
    let summary = fs::read_to_string(summary)?;
...
#[test]
fn ci_route_cli_maps_publish_receipts_wrapper_to_receipt_proof_pack() -> Result<()> {
    let temp = TempDir::new()?;
    let receipt = temp.path().join("ci-route.json");
    let summary = temp.path().join("ci-route.md");
    ...
    assert_eq!(
        route.pointer("/changed_surfaces/0").and_then(Value::as_str),
        Some("publish-receipts-wrapper")
    );
    assert!(
        route.get("required_proof_packs").and_then(Value::as_array).is_some_and(|packs| packs
            .iter()
            .any(|pack| {
                pack.get("id").and_then(Value::as_str) == Some("publish-receipts-wrapper-focused")
                    && pack.get("commands").and_then(Value::as_array).is_some_and(|commands| {
                        commands.iter().any(|command| {
                            command.as_str()
                                == Some("bash scripts/tests/test-publish-receipts-wrapper.sh")
                        })
                    })
            })),
        "publish-receipts wrapper changes must run the focused receipt proof"
    );
    assert!(
        route.get("coverage_pack_selector").and_then(Value::as_array).is_some_and(Vec::is_empty),
        "publish-receipts wrapper proof pack is non-LCOV and must not be uploaded as Codecov coverage"
    );
    assert_eq!(
        route
            .pointer("/skipped_by_policy/patch-coverage-publish-receipts-wrapper")
            .and_then(Value::as_str),
        Some("non-LCOV CI policy/routing surface; covered by focused CI gates")
    );
    let summary = fs::read_to_string(summary)?;
    assert!(summary.contains("bash scripts/tests/test-publish-receipts-wrapper.sh"));
    assert!(
        summary.contains(
            "`patch-coverage-publish-receipts-wrapper`: non-LCOV CI policy/routing surface"
        )
    );
    Ok(())
}
...
#[test]
fn ci_route_cli_maps_swarm_summary_wrapper_to_summary_proof_pack() -> Result<()> {
    let temp = TempDir::new()?;
    let receipt = temp.path().join("ci-route.json");
    let summary = temp.path().join("ci-route.md");
    ...
    assert_eq!(
        route.pointer("/changed_surfaces/0").and_then(Value::as_str),
        Some("swarm-summary-wrapper")
    );
    assert!(
        route.get("required_proof_packs").and_then(Value::as_array).is_some_and(|packs| packs
            .iter()
            .any(|pack| {
                pack.get("id").and_then(Value::as_str) == Some("swarm-summary-wrapper-focused")
                    && pack.get("commands").and_then(Value::as_array).is_some_and(|commands| {
                        commands.iter().any(|command| {
                            command.as_str()
                                == Some("bash scripts/tests/test-swarm-summary-wrapper.sh")
                        })
                    })
            })),
        "swarm-summary wrapper changes must run focused proof"
    );
    assert!(
        route.get("coverage_pack_selector").and_then(Value::as_array).is_some_and(Vec::is_empty),
        "swarm-summary wrapper proof pack is non-LCOV and must not be uploaded as Codecov coverage"
    );
    assert_eq!(
        route
            .pointer("/skipped_by_policy/patch-coverage-swarm-summary-wrapper")
            .and_then(Value::as_str),
        Some("non-LCOV CI policy/routing surface; covered by focused CI gates")
    );
    let summary = fs::read_to_string(summary)?;
    assert!(summary.contains("bash scripts/tests/test-swarm-summary-wrapper.sh"));
    assert!(
        summary
            .contains("`patch-coverage-swarm-summary-wrapper`: non-LCOV CI policy/routing surface")
    );
    Ok(())
}
...
#[test]
fn ci_route_cli_maps_worktree_cleanup_wrapper_to_cleanup_proof_pack() -> Result<()> {
    let temp = TempDir::new()?;
    let receipt = temp.path().join("ci-route.json");
    let summary = temp.path().join("ci-route.md");
    ...
    assert_eq!(
        route.pointer("/changed_surfaces/0").and_then(Value::as_str),
        Some("worktree-cleanup-wrapper")
    );
    assert!(
        route.get("required_proof_packs").and_then(Value::as_array).is_some_and(|packs| packs
            .iter()
            .any(|pack| {
                pack.get("id").and_then(Value::as_str) == Some("worktree-cleanup-wrapper-focused")
                    && pack.get("commands").and_then(Value::as_array).is_some_and(|commands| {
                        commands.iter().any(|command| {
                            command.as_str()
                                == Some("bash scripts/tests/test-cleanup-worktrees-wrapper.sh")
                        })
                    })
            })),
        "worktree cleanup wrapper changes must run focused proof"
    );
    assert!(
        route.get("coverage_pack_selector").and_then(Value::as_array).is_some_and(Vec::is_empty),
        "worktree cleanup wrapper proof pack is non-LCOV and must not be uploaded as Codecov coverage"
    );
    assert_eq!(
        route
            .pointer("/skipped_by_policy/patch-coverage-worktree-cleanup-wrapper")
            .and_then(Value::as_str),
        Some("non-LCOV CI policy/routing surface; covered by focused CI gates")
    );
    let summary = fs::read_to_string(summary)?;
    assert!(summary.contains("bash scripts/tests/test-cleanup-worktrees-wrapper.sh"));
    assert!(
        summary.contains(
            "`patch-coverage-worktree-cleanup-wrapper`: non-LCOV CI policy/routing surface"
        )
    );
    Ok(())
}
