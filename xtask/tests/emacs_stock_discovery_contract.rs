...
use xtask::emacs_stock_discovery::{
    ClientFamily, RegistrationEntry, SourceState, checked_baseline, render_checked_json,
};
...
fn released_and_source_rows_stay_independent_for_both_clients() {
    ...
    let dimensions = baseline
        .observations
        .iter()
        .map(|row| (row.client_family, row.source_state, row.subject_id.as_str()))
        .collect::<Vec<_>>();

    assert_eq!(
        dimensions,
        vec![
            (ClientFamily::Eglot, SourceState::Released, "released_eglot_gnu_elpa_1_24"),
            (ClientFamily::Eglot, SourceState::UpstreamSource, "source_eglot_emacs_f4f249a2"),
            (ClientFamily::LspMode, SourceState::Released, "released_lsp_mode_10_0_0"),
            (ClientFamily::LspMode, SourceState::UpstreamSource, "source_lsp_mode_e15b8205"),
        ],
...
fn exact_rows_require_manual_registration_for_perllsp() {
    ...
    for row in baseline.observations {
        assert!(row.observation_complete, "{} must be a complete source observation", row.subject_id);
        assert!(!row.manual_registration_injected, "{} must not contain repository setup", row.subject_id);
        assert!(!row.perllsp_present, "{} unexpectedly contains a stock perllsp entry", row.subject_id);
        assert!(
...
fn lsp_mode_rows_retain_competing_clients_and_priority_order() {
    ...
    for row in baseline
        .observations
        .iter()
        .filter(|row| row.client_family == ClientFamily::LspMode)
    {
        let observed = row
            .entries
            .iter()
            .map(|entry| (entry.entry_id.as_str(), entry.server_id.as_deref(), entry.priority))
            .collect::<Vec<_>>();
        assert_eq!(
            observed,
            vec![
                ...
                ("pls", Some("pls"), Some(-1)),
                ("perl_language_server", Some("perl-language-server"), Some(-2)),
            ],
...
fn changed_lsp_mode_priority_is_visible() {
    ...
    let row = baseline
        .observations
        .iter_mut()
        .find(|row| row.client_family == ClientFamily::LspMode)
        .expect("lsp-mode row");
