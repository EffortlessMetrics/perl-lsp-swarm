...
fn semantic_inline_receipts_cli_writes_dashboard_inventory() -> Result<()> {
    ...
    assert_eq!(semantic_inline.len(), 17);
    assert_eq!(
        semantic_inline
    ...
    assert_eq!(
        semantic_inline
            .get("project_dbi_receiver")
            .and_then(|entry| entry.get("workflow_id"))
            .and_then(Value::as_str),
        Some("project_dbi_receiver_inline_completion_quality")
    );
    assert_eq!(
        semantic_inline
            .get("project_lexical_return")
            .and_then(|entry| entry.get("workflow_id"))
            .and_then(Value::as_str),
        Some("project_lexical_return_inline_completion_quality")
    );
