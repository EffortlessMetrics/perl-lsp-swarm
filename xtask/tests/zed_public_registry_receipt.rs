...
fn public_template_uses_the_official_registry_stage() -> Result<(), Box<dyn Error>> {
    ...
    let receipt = read_json(
        &root,
        ".ci/fixtures/zed-perl-upstream/receipts/public-registry-template.json",
    )?;
    assert_eq!(
...
fn public_subject_cannot_invent_publication_or_promotion() -> Result<(), Box<dyn Error>> {
    ...
    assert_eq!(
        subject.get("status").and_then(Value::as_str),
        Some("blocked_pending_publication")
    );
    assert_eq!(
        subject.pointer("/registry/extension_id").and_then(Value::as_str),
        Some("perl")
    );
    assert_eq!(
        subject
            .pointer("/registry/submodule_path")
            .and_then(Value::as_str),
        Some("extensions/perl")
    );
    for cell in [
        "registry_row",
        "managed_download_row",
        "path_row",
        "documentation_projection",
    ] {
        assert_eq!(
            subject
                .pointer(&format!("/promotion/{cell}"))
                .and_then(Value::as_str),
            Some("not_proven")
        );
    }
...
fn managed_and_path_routes_remain_distinct() -> Result<(), Box<dyn Error>> {
    ...
    assert_eq!(
        subject
            .pointer("/promotion/managed_download_row")
            .and_then(Value::as_str),
        Some("not_proven")
    );
    assert_eq!(
