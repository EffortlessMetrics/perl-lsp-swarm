...
fn known_windows_drift_blocks_after_writing_drift_receipt() -> Result<()> {
    ...
    assert_eq!(receipt["manifest_verification"]["status"].as_str(), Some("verified"));
    assert!(has_blocker(&receipt, "same_version_divergent_product"));
    assert!(has_blocker(&receipt, "product_drift"));
    Ok(())
}
...
fn repo_root() -> PathBuf {
    PathBuf::from(env!("CARGO_MANIFEST_DIR"))
        .parent()
        .expect("xtask manifest must live below the repository root")
        .to_path_buf()
}
