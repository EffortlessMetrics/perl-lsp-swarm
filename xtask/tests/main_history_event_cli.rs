...
///
/// A reported force push over a proven fast-forward exits 5: no history was
/// lost, so it is not a rewrite (2); the graph *is* proven, so it is not an
/// unverifiable 3; and `main` should not accept a force push at all, so it does
/// not pass silently either.
#[test]
fn force_reported_over_a_fast_forward_retains_both_axes_and_exits_five() -> Result<()> {
    let repository = initialized_repository()?;
    let before = git(repository.path(), &["rev-parse", "HEAD"])?;
    commit_file(repository.path(), "second.txt", "second\n", "second")?;
