...
fn focused_reproduce_command_is_invocable_cli() {
    // The command must run as spelled by the real `gates` CLI: `--tier`
    // takes the kebab-case tier (the plan's native tier is snake_case),
    // `--base` takes the *git* base ref used for scope-aware planning --
    // which is `selection.base`, not the requested profile -- and `--gate`
    // takes the gate id (review thread 3871822422).
    let plan = compiled_plan();
    let result = build_success(&plan);
    assert_eq!(
...
#[test]
fn focused_reproduce_command_adds_staged_only_for_the_commit_tier() {
    // The commit tier is the one tier the CLI only reproduces faithfully
    // with `--staged`; every other tier must not carry the flag. Without
    // this case the tier-conditional branch is unproven.
    let mut input = plan_input();
    input.expansion.included_native_tiers.push("commit".to_string());
    for disposition in &mut input.dispositions {
        disposition.native_tier = "commit".to_string();
    }
    let plan = CiRoutePlanV1::compile(input).expect("commit-tier fixture plan compiles");
    let result = build_success(&plan);
    assert_eq!(
        result.focused_reproduce_command,
        format!("cargo xtask gates --tier commit --base {SHA_B} --gate fmt_gate --staged"),
        "the commit tier must reproduce with --staged"
    );
}
