...
/// One synthetic host session's shape.
struct SessionSpec {
    role: &'static str,
    events: Vec<DriverEvent>,
    wire: WireEvidence,
...
fn scratch_session(index: usize, plan: &VimHostRunPlan, spec: SessionSpec) -> HostSessionRecord {
    HostSessionRecord {
        index,
        ...
        wire: spec.wire,
        lifecycle_wire: spec.lifecycle_wire,
        plan: plan.clone(),
        ledger: Some(ledger_of(spec.pid, 0, "available")),
        settled_probe_clean: spec.settled,
...
fn canonical_sessions(plan: &VimHostRunPlan, digest: &str) -> Vec<HostSessionRecord> {
    let fresh_wire = || LifecycleWire { initialize_count: 1, ..LifecycleWire::default() };
    vec![
        scratch_session(
            1,
            plan,
            SessionSpec {
                role: "full_lifecycle_session",
                events: complete_host1_events(digest),
                wire: wire_with_caps(Some(false)),
                lifecycle_wire: host1_wire(),
                status: Some(0),
        ...
        scratch_session(
            ...
            plan,
            SessionSpec {
                role: "replacement_host_session",
                events: complete_host2_events(digest),
                wire: wire_with_caps(Some(false)),
                lifecycle_wire: fresh_wire(),
                status: Some(0),
                cleanup: CleanupResult::Pass,
                timed_out: false,
                kill_requested: false,
        ...
        scratch_session(
            ...
            plan,
            SessionSpec {
                role: "assertion_failure_session",
                events: complete_host3_events(digest),
                wire: wire_with_caps(Some(false)),
                lifecycle_wire: fresh_wire(),
                status: Some(2),
        ...
        scratch_session(
            ...
            plan,
            SessionSpec {
                role: "timeout_interruption_session",
                events: complete_host4_events(digest),
                wire: wire_with_caps(Some(false)),
                lifecycle_wire: fresh_wire(),
                status: None,
                cleanup: CleanupResult::NotProven,
...
fn a_server_restart_relabel_fails_typed_host_replacement_absent() -> Result<()> {
    ...
    let sessions = vec![scratch_session(
        ...
        &plan,
        SessionSpec {
            role: "server_restart_relabel_session",
            events: complete_relabel_events(&digest),
            wire: wire_with_caps(Some(false)),
            lifecycle_wire: LifecycleWire { initialize_count: 2, ..LifecycleWire::default() },
            status: Some(0),
            cleanup: CleanupResult::Pass,
            timed_out: false,
            kill_requested: false,
...
fn journey_receipt_carries_prefixed_host_barriers_and_the_eight_catalog_cells() -> Result<()> {
    ...
    let workspace = cells
        .iter()
        .find(|cell| cell.id == CELL_WORKSPACE_REOPEN)
        .ok_or_else(|| anyhow::anyhow!("workspace cell missing"))?;
...
#[test]
fn the_journey_aggregate_proves_designed_failure_cleanup_without_hiding_it() -> Result<()> {
    let dir = tempfile::tempdir()?;
    let plan = scratch_lifecycle_plan(&dir.path().join("scratch"))?;
    let digest = plan.identity.candidate_artifact_sha256.clone();
    let sessions = canonical_sessions(&plan, &digest);
    let aggregate = xtask::vim_host_lifecycle_run::aggregate_journey_observation(&sessions);
    ensure!(
        aggregate.cleanup == CleanupResult::Pass,
        "the designed forced-failure hosts' observed settled cleanup completes the journey's \
         owned-resource proposition"
    );
    ensure!(
        aggregate.cleanup_detail.contains("host-3 (assertion_failure_session): pass")
            && aggregate.cleanup_detail.contains("host-4 (timeout_interruption_session): pass"),
        "the aggregate names each host's contribution"
    );

    // A designed-failure host without the settled observation degrades the
    // aggregate honestly (missing observation, never zero).
    let mut gapped = canonical_sessions(&plan, &digest);
    gapped[3].settled_probe_clean = None;
    let aggregate = xtask::vim_host_lifecycle_run::aggregate_journey_observation(&gapped);
    ensure!(
        aggregate.cleanup == CleanupResult::NotProven,
        "a missing settled observation keeps the journey cleanup not_proven"
    );

    // A leaking host fails the aggregate outright.
    let mut leaking = canonical_sessions(&plan, &digest);
    leaking[0].observation.cleanup = CleanupResult::Fail;
    let aggregate = xtask::vim_host_lifecycle_run::aggregate_journey_observation(&leaking);
    ensure!(
        aggregate.cleanup == CleanupResult::Fail,
        "an observed leak fails the journey aggregate"
    );
    Ok(())
}
