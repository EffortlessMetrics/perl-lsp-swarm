...
#[test]
fn observation_template_is_not_run_and_cell_complete() -> Result<(), Box<dyn Error>> {
    let root = repo_root()?;
...
#[test]
fn driver_uses_reviewed_zed_surfaces_and_exact_provider_identity() -> Result<(), Box<dyn Error>> {
    let root = repo_root()?;
...
#[test]
fn shared_rust_validator_is_the_pass_authority() -> Result<(), Box<dyn Error>> {
    let root = repo_root()?;
