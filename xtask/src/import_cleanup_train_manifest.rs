...
fn validate_manifest(m: &Manifest) -> Result<()> {
    ...
    for node in &add_missing_active {
        match node.product_context.as_str() {
            ...
            "internal_plan" => {
                // Bind the insert before testing it: a lone `if` arm body trips
                // the armed `collapsible_match` deny (#6113), and the collapse
                // clippy suggests would bury this set mutation in a match guard.
                let first_for_role =
...
fn wire_coherence(node: &TrainNode) -> Result<()> {
    match node.wire_kind.as_str() {
        // The guards carry the incompatibility test so each arm names one
        // rejection. A compatible payload simply falls through to `_` exactly
        // as it did when the test lived in the arm body.
        "workspace_edit"
            if !matches!(
