...
fn validate_manifest(m: &Manifest) -> Result<()> {
    ...
    for node in &add_missing_active {
        match node.product_context.as_str() {
            ...
            "internal_plan" => {
                // Bound before the test: `insert` mutates `spine_seen`, so a
                // match guard would perform the insertion during pattern
                // selection. See #12914.
                let first_seen =
...
fn wire_coherence(node: &TrainNode) -> Result<()> {
    match node.wire_kind.as_str() {
        // Pure predicates, so the guard form is sound: a failing guard falls
        // through to the catch-all arm, which is the same no-op the inner
        // `if`'s else-path performed.
        "workspace_edit"
            if !matches!(
