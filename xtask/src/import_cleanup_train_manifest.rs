...
fn validate_manifest(m: &Manifest) -> Result<()> {
    ...
    for node in &add_missing_active {
        match node.product_context.as_str() {
            ...
            // The guard inserts for every internal-plan row; a rejected
            // insert means that (context, role) spine already collapsed. A
            // successful insert falls through to the no-op catch-all, which
            // is what the previous nested `if` did.
            "internal_plan"
