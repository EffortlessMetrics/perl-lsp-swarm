...
fn validate_manifest(m: &Manifest) -> Result<()> {
    ...
    for node in &add_missing_active {
        match node.product_context.as_str() {
            ...
            // `insert` is a side effect, so this guarded arm is only
            // equivalent to the nested `if` because a match guard runs at
            // most once for the arm whose pattern matched, and a first
            // sighting (`insert` == true) falls through to the catch-all
            // arm — exactly the no-op the nested `if` performed (#12910).
            "internal_plan"
