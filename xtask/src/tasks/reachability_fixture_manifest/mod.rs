...
fn load_or_get_scan(root: &Path, relative: &str, scans: &mut BTreeMap<String, FixtureScan>) {
    ...
    match fs::read_to_string(root.join(relative)) {
        ...
        Err(_) => {
            // The io reason is not retained: unreadable fixtures already
            // yield digest/reference violations downstream, so storing the
            // string would be write-only debt.
            scans.insert(relative.to_string(), FixtureScan::Unavailable);
        }
    }
}
