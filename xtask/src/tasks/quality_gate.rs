...
fn normalize_policy(raw: &str) -> Result<NormalizedPolicy> {
    let mut policy: TomlValue =
        toml::from_str(raw).context("parsing quality exception policy for lifecycle projection")?;
    let table = policy
        .as_table_mut()
        .ok_or_else(|| color_eyre::eyre::eyre!("quality exception policy root must be a table"))?;

    if let Some(due_review) = table.get("due_review") {
        let due_review = due_review.as_str().ok_or_else(|| {
            color_eyre::eyre::eyre!("quality exception due_review must be a string")
        })?;
