    ...
    for feature in catalog.features() {
            ...
            && feature.counts_in_coverage
        {
            violations.push(format!(
                "UNTESTED_PROVEN: {:?} is advertised+proven but cites no tests. Either cite behavior evidence, downgrade to not_proven, or set counts_in_coverage=false (if it's protocol plumbing).",
                feature.id
            ));
        }
    }
    ...
    for (area, features) in by_area {
        ...
        for feature in features {
            let status = match (feature.maturity, feature.advertised) {
                (Maturity::Proven, true) => "✅ Proven (evidence cited)",
                (Maturity::Proven, false) => "✅ Proven (not advertised)",
                (Maturity::Preview, _) => "🔧 Preview",
                (Maturity::Unsupported, _) => "🚫 Unsupported (negative-gated)",
                (Maturity::NotProven, true) => "⚠️ Advertised, evidence not proven (#7029)",
                (Maturity::NotProven, false) => "⚠️ Not proven (#7029)",
                (Maturity::Planned, _) => "❌ Not implemented",
            };
...
fn generate_report() -> Result<()> {
    ...
    let proven = catalog
        .feature
        .iter()
        .filter(|f| matches!(f.maturity, Maturity::Proven) && f.advertised)
        .count();
    let preview = catalog
        .feature
        .iter()
    ...
    println!("\nBreakdown (evidence states, #7029):");
    println!("  Proven:      {} advertised features", proven);
    println!("  Preview:     {} advertised features", preview);
