...
const ROWS: &[MatrixRow] = &[
    ...
    MatrixRow {
        ...
        server_shape: "`semanticTokensProvider.full = { delta: true }`; `range = true`",
        ...
        priority: "P0",
        notes: "Full results mint a result-id; delta is implemented (see the semantic-token delta row).",
    },
    MatrixRow {
        feature: "Semantic-token delta",
        since_or_flag: "LSP 3.17 semantic tokens",
        client_gate: "`textDocument.semanticTokens.requests.full.delta`",
        server_shape: "`semanticTokensProvider.full.delta = true`",
        method_or_shape: "`textDocument/semanticTokens/full/delta`",
        status: "implemented+tested+documented",
        proof: "`lsp_3_17_semantic_tokens_tests`; `lsp_caps_contract_shapes`; `check-lsp-318-claims`",
        owner: "`crates/perl-lsp-rs/src/runtime/language/semantic_tokens.rs`; capability builder",
        priority: "P1",
        notes: "Result-id state is minted per full result and cached per URI; delta returns minimal prefix/suffix edits, with full fallback on an unknown previousResultId.",
    },
    MatrixRow {
    ...
    MatrixRow {
        ...
        since_or_flag: "LSP 3.18",
        client_gate: "no client gate",
        server_shape: "`Command.tooltip` on reachable command-producing surfaces",
        method_or_shape: "`textDocument/codeLens`; `codeLens/resolve`; `textDocument/codeAction`; `initialize` `codeActionProvider.documentation`",
        status: "implemented+tested+documented",
        proof: "`lsp_codelens_tests`; `lsp_command_tooltip_tests`; `lsp_318_negative_claims`; `check-lsp-318-claims`",
        owner: "`crates/perl-lsp-rs-core/src/protocol/command.rs`; CodeLens handlers; `crates/perl-lsp-rs/src/runtime/language/code_actions.rs`; initialize documentation",
        priority: "P3",
        notes: "Plain-text tooltips are required on every reachable LSP Command object. CodeLens receipts are unchanged. Completion, inline-completion, and document-link surfaces do not produce Command objects. CodeAction.documentation Command objects share one producer.",
    },
    MatrixRow {
        feature: "`RelativePattern` watcher registrations (3.17 watcher globs)",
        since_or_flag: "LSP 3.17",
        ...
        priority: "P3",
        notes: "Watcher-glob `RelativePattern` is the LSP 3.17 surface. Capable clients receive `baseUri`/`pattern` watcher globs rooted at workspace folders; unsupported clients or invalid roots keep string glob fallback. Document-filter and notebook-filter relative patterns are separate 3.18 rows below (#8897).",
    },
    MatrixRow {
        feature: "Document-filter `relative pattern` (text-document filters)",
        since_or_flag: "LSP 3.18",
        client_gate: "n/a; no implemented product path emits relative document filters",
        server_shape: "document-filter `pattern` objects with `baseUri` (not emitted)",
        method_or_shape: "any `DocumentSelector`/document-filter producer",
        status: "negative-gated+documented",
