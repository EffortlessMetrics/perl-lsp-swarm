...
const ROWS: &[MatrixRow] = &[
    ...
    MatrixRow {
        ...
        status: "implemented+tested+documented",
        proof: "`lsp_inline_completion_registration_tests`; `lsp_318_negative_claims`; `check-lsp-318-claims`",
        owner: "`crates/perl-lsp-rs/src/runtime/language/misc.rs`; inline-completion provider",
        priority: "P1",
        notes: "The deterministic feature pragma inline item emits `insertText` as `{ kind: \"snippet\", value }`; simple inline items retain plain string `insertText`.",
    },
    MatrixRow {
