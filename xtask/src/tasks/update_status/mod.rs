...
//!
//! `docs/project/status/lsp.md` is generated by
//! `cargo xtask catalog-authority sync-status` (#6731) and is intentionally
//! not written here; this task keeps only the ROADMAP compliance fence in
//! sync when the lsp subsystem runs.
...
pub fn run(write: bool, check: bool, only: Option<StatusSubsystem>) -> Result<()> {
    ...
    if need_lsp {
        run_subsystem("lsp", "cargo xtask update-status --write --only lsp", || {
            // `docs/project/status/lsp.md` is owned by
            // `cargo xtask catalog-authority sync-status` (#6731); this task
            // keeps only the ROADMAP fence in sync, from the same renderer.
