...
fn render(inventory: &Inventory) -> Result<String> {
    ...
    writeln!(output, "> `{INVENTORY_PATH}`. This inventory records current provider fact reads,")?;
    writeln!(output, "> ownership assumptions, and duplicate interpretation seams. It does not")?;
    writeln!(output, "> change provider behavior, promote a producer, or authorize edits.")?;
    writeln!(output)?;
    writeln!(
        output,
    ...
    for provider in &inventory.required_providers {
        writeln!(
            ...
            counts.get(provider.as_str()).copied().unwrap_or_default()
        )?;
    }
