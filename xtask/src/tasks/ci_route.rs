...
/// Workspace root, anchored at compile time so target discovery does not depend
/// on the caller's working directory.
fn workspace_root_for_targets() -> PathBuf {
...
#[derive(Debug, Clone, PartialEq, Eq)]
struct PackageTestTargets {
    package_name: String,
    has_lib: bool,
    binaries: Vec<BinaryTestTarget>,
}

fn normalized_target_path(value: &str) -> String {
    value.replace('\\', "/")
}

/// Resolve the source Cargo would pick for a pathless `[[bin]]`.
fn inferred_bin_path(crate_root: &Path, name: &str, package_name: &str) -> Option<String> {
    let mut candidates = vec![format!("src/bin/{name}.rs"), format!("src/bin/{name}/main.rs")];
    if name == package_name {
        candidates.push("src/main.rs".to_string());
    }
    candidates.into_iter().find(|candidate| crate_root.join(candidate).is_file())
}

/// Resolve a package edition, following `edition.workspace = true`.
///
/// Returns `None` when the edition cannot be established.  Callers must not
/// infer edition-2015 auto-discovery from an unresolved edition; this workspace
/// is 2024 and guessing 2015 would silently drop real binary targets.
fn resolved_edition(package: &toml::Value, repo_root: &Path) -> Option<String> {
    let raw = package.get("edition")?;
    if let Some(edition) = raw.as_str() {
        return Some(edition.to_string());
    }
    if raw.get("workspace").and_then(toml::Value::as_bool) != Some(true) {
        return None;
    }
    let contents = fs::read_to_string(repo_root.join("Cargo.toml")).ok()?;
    let workspace: toml::Value = toml::from_str(&contents).ok()?;
    workspace.get("workspace")?.get("package")?.get("edition")?.as_str().map(str::to_string)
}

/// Cargo's effective `autobins` for one package.
///
/// Cargo only defaults `autobins` to true unconditionally from edition 2018 on.
/// A 2015 package that declares any target manually — an explicit `[lib]`
/// counts, not only a `[[bin]]` — disables binary auto-discovery, so assuming
/// discovery would invent targets `cargo test --bin` then rejects.
fn autobins_enabled(
    package: &toml::Value,
    edition: Option<&str>,
    has_explicit_bins: bool,
    has_explicit_lib: bool,
) -> bool {
    if let Some(explicit) = package.get("autobins").and_then(toml::Value::as_bool) {
        return explicit;
    }
    !(edition == Some("2015") && (has_explicit_bins || has_explicit_lib))
}

fn required_features_of(target: &toml::Value) -> Vec<String> {
    let mut features: Vec<String> = target
...
fn target_name_from_path(path_value: &str, package_name: &str) -> String {
    let normalized = normalized_target_path(path_value);
    let path = Path::new(&normalized);
    if path.file_name().and_then(|name| name.to_str()) == Some("main.rs") {
        return match path.parent().and_then(Path::file_name).and_then(|name| name.to_str()) {
            Some("src") | None => package_name.to_string(),
            Some(parent) => parent.to_string(),
        };
    }
    path.file_stem()
        .and_then(|stem| stem.to_str())
        .map_or_else(|| package_name.to_string(), str::to_string)
}

/// Derive testable lib/bin targets for one local Cargo package.
///
/// Returns `None` when the crate directory carries no readable manifest, so a
/// changed path outside a real package is skipped instead of routing an
/// invented target.  Mirrors `scripts/ci/route-codecov-packs.py`, which serves
/// the nightly workflow while this serves `just coverage-proof-routed`.
fn package_test_targets(crate_name: &str, repo_root: &Path) -> Option<PackageTestTargets> {
    let crate_root = repo_root.join("crates").join(crate_name);
    let contents = fs::read_to_string(crate_root.join("Cargo.toml")).ok()?;
    let manifest: toml::Value = toml::from_str(&contents).ok()?;
    let package = manifest.get("package")?;
    let package_name = package.get("name").and_then(toml::Value::as_str)?.to_string();
    let edition = resolved_edition(package, repo_root);

    // Source files claimed by an explicit target are never auto-discovered again.
    let mut occupied: BTreeSet<String> = BTreeSet::new();

    let explicit_lib = manifest.get("lib");
    let has_lib = match explicit_lib {
        Some(lib) => {
            let path = lib.get("path").and_then(toml::Value::as_str).unwrap_or("src/lib.rs");
            occupied.insert(normalized_target_path(path));
            lib.get("test").and_then(toml::Value::as_bool) != Some(false)
        }
        None => {
            package.get("autolib").and_then(toml::Value::as_bool) != Some(false)
                && crate_root.join("src/lib.rs").is_file()
        }
    };

    let explicit_bins: Vec<&toml::Value> = manifest
        .get("bin")
        .and_then(toml::Value::as_array)
        .map(|bins| bins.iter().collect())
        .unwrap_or_default();
    let mut binaries: Vec<BinaryTestTarget> = Vec::new();
    for target in &explicit_bins {
        let raw_path = target.get("path").and_then(toml::Value::as_str);
        let name = match target.get("name").and_then(toml::Value::as_str) {
            Some(name) if !name.is_empty() => name.to_string(),
            _ => target_name_from_path(raw_path.unwrap_or("src/main.rs"), &package_name),
        };
        if let Some(path) = raw_path {
            occupied.insert(normalized_target_path(path));
        } else if let Some(path) = inferred_bin_path(&crate_root, &name, &package_name) {
            occupied.insert(path);
        }
        // `test = false` still occupies its source; it just is not run.
        if target.get("test").and_then(toml::Value::as_bool) == Some(false) {
            continue;
        }
        if !binaries.iter().any(|existing| existing.name == name) {
            binaries
                .push(BinaryTestTarget { name, required_features: required_features_of(target) });
        }
    }
    ...
    if autobins_enabled(
        package,
        edition.as_deref(),
        !explicit_bins.is_empty(),
        explicit_lib.is_some(),
    ) {
        let mut discovered: Vec<(String, String)> = Vec::new();
        if crate_root.join("src/main.rs").is_file() {
            discovered.push((package_name.clone(), "src/main.rs".to_string()));
        }
        let bin_root = crate_root.join("src/bin");
        if bin_root.is_dir() {
            let mut entries: Vec<PathBuf> = fs::read_dir(&bin_root)
                .into_iter()
                .flatten()
            ...
            for entry in entries {
                ...
                if entry.is_file() && entry.extension().and_then(|ext| ext.to_str()) == Some("rs") {
                    let file_name = entry
                        .file_name()
                        .and_then(|name| name.to_str())
                        .unwrap_or(stem)
                        .to_string();
                    discovered.push((stem.to_string(), format!("src/bin/{file_name}")));
                } else if entry.is_dir() && entry.join("main.rs").is_file() {
                    discovered.push((stem.to_string(), format!("src/bin/{stem}/main.rs")));
                }
            }
        }
        for (name, relative_path) in discovered {
            if occupied.contains(&relative_path) {
                continue;
            }
            if !binaries.iter().any(|existing| existing.name == name) {
                binaries.push(BinaryTestTarget { name, required_features: Vec::new() });
            }
        }
    }

    binaries.sort_by(|left, right| left.name.cmp(&right.name));
    Some(PackageTestTargets { package_name, has_lib, binaries })
}

fn binary_target_command(package_name: &str, target: &BinaryTestTarget) -> String {
    let feature_arg = if target.required_features.is_empty() {
        String::new()
    } else {
        format!(" --features {}", target.required_features.join(","))
    };
    format!(
        "cargo llvm-cov test --no-report -p {package_name}{feature_arg} --bin {} --profile agent --locked",
        target.name
    )
}
...
fn augment_rust_focused_commands(
    ...
    augment_rust_focused_commands_at(base_commands, changed_files, &workspace_root_for_targets())
}

fn augment_rust_focused_commands_at(
    base_commands: &[String],
    changed_files: &[String],
    repo_root: &Path,
) -> Vec<String> {
    ...
    for crate_name in changed_crates(changed_files) {
        // Fall back to the crate directory when no manifest is readable, so a
        // changed path outside a real package keeps its previous routing.
        let targets = package_test_targets(&crate_name, repo_root);
        let package_name = targets
            .as_ref()
            .map_or_else(|| crate_name.clone(), |targets| targets.package_name.clone());
        if targets.as_ref().is_none_or(|targets| targets.has_lib) {
            push_unique_command(
                &mut commands,
                format!(
                    "cargo llvm-cov test --no-report -p {package_name} --lib --profile agent --locked"
                ),
            );
        }
        if let Some(targets) = targets.as_ref() {
            for binary in &targets.binaries {
                push_unique_command(&mut commands, binary_target_command(&package_name, binary));
            }
        }
        if let Some(test_targets) = test_targets_by_crate.get(&crate_name) {
            for target in test_targets {
                push_unique_command(&mut commands, targeted_test_command(&package_name, target));
            }
        } else {
            push_unique_command(
                &mut commands,
                format!(
                    "cargo llvm-cov test --no-report -p {package_name} --tests --profile agent --locked -- --test-threads=1"
                ),
            );
        }
    }
...
mod tests {
    ...
    /// Fixture package under a temporary workspace root.
    fn write_package(
        root: &TempDir,
        directory: &str,
        manifest: &str,
        ...
        let crate_root = root.path().join("crates").join(directory);
        ...
        for relative in files {
            ...
            fs::write(path, "// target fixture\n")?;
        }
        Ok(())
    }

    fn routed_commands(root: &TempDir, directory: &str) -> Vec<String> {
        let changed = vec![format!("crates/{directory}/src/changed.rs")];
        augment_rust_focused_commands_at(&[], &changed, root.path())
    }

    #[test]
    fn routed_coverage_registers_real_binary_unit_targets() -> Result<()> {
        // The `just coverage-proof-routed` path must register binary unit
        // tests, not only a library: #13061's perl-ci-hygiene witness.
        let commands = augment_rust_focused_commands_at(
            &[],
            &["crates/perl-ci-hygiene/src/cli.rs".to_string()],
            &workspace_root_for_targets(),
        );
        ...
        assert!(
            ...
            "routed coverage must register the binary unit target: {commands:?}"
        );
        Ok(())
    }

    #[test]
    fn routed_coverage_uses_package_name_not_crate_directory() -> Result<()> {
        ...
        let commands = routed_commands(&root, "tool-directory");

        assert!(commands.iter().any(|command| command
            == "cargo llvm-cov test --no-report -p renamed-tool --bin renamed-tool --profile agent --locked"));
        assert!(
            !commands.iter().any(|command| command.contains(" --lib ")),
            "a binary-only package must not receive an invented --lib target: {commands:?}"
        );
        assert!(commands.iter().all(|command| !command.contains("-p tool-directory")));
        Ok(())
    }

    #[test]
    fn routed_coverage_does_not_invent_a_binary_for_a_library_only_package() -> Result<()> {
        let root = TempDir::new()?;
        write_package(
            &root,
            "library-directory",
        ...
        let commands = routed_commands(&root, "library-directory");

        assert!(commands.iter().any(|command| command
            == "cargo llvm-cov test --no-report -p library-package --lib --profile agent --locked"));
        assert!(!commands.iter().any(|command| command.contains(" --bin ")));
        Ok(())
    }

    #[test]
    fn routed_coverage_honors_explicit_bin_source_occupancy() -> Result<()> {
        let root = TempDir::new()?;
        write_package(
            &root,
            "renamed-main",
        ...
        let commands = routed_commands(&root, "renamed-main");

        assert!(commands.iter().any(|command| command
            == "cargo llvm-cov test --no-report -p renamed-main --bin cli --profile agent --locked"));
        assert!(
            !commands.iter().any(|command| command.contains("--bin renamed-main")),
            "src/main.rs is claimed by `cli`; Cargo has no --bin renamed-main: {commands:?}"
        );
        Ok(())
    }

    #[test]
    fn routed_coverage_still_discovers_unclaimed_bin_sources() -> Result<()> {
        let root = TempDir::new()?;
        write_package(
            &root,
            "mixed-bins",
            "[package]\nname = \"mixed-bins\"\nversion = \"0.0.0\"\nedition = \"2024\"\n\n[[bin]]\nname = \"cli\"\npath = \"src/main.rs\"\n",
            &["src/main.rs", "src/bin/helper.rs", "src/changed.rs"],
        )?;

        let commands = routed_commands(&root, "mixed-bins");

        assert!(commands.iter().any(|command| command
            == "cargo llvm-cov test --no-report -p mixed-bins --bin cli --profile agent --locked"));
        assert!(
            commands.iter().any(|command| command
                == "cargo llvm-cov test --no-report -p mixed-bins --bin helper --profile agent --locked"),
            "occupancy must not suppress unrelated targets: {commands:?}"
        );
        Ok(())
    }

    #[test]
    fn routed_coverage_disables_autodiscovery_for_manual_2015_targets() -> Result<()> {
        let root = TempDir::new()?;
        write_package(
            &root,
            "edition-directory",
            "[package]\nname = \"edition-package\"\nversion = \"0.0.0\"\nedition = \"2015\"\n\n[[bin]]\nname = \"declared\"\npath = \"src/declared.rs\"\n",
            &["src/main.rs", "src/declared.rs", "src/changed.rs"],
        )?;

        let commands = routed_commands(&root, "edition-directory");

        assert!(commands.iter().any(|command| command
            == "cargo llvm-cov test --no-report -p edition-package --bin declared --profile agent --locked"));
        assert!(!commands.iter().any(|command| command.contains("--bin edition-package")));
        Ok(())
    }

    #[test]
    fn routed_coverage_keeps_autodiscovery_for_2024_manual_targets() -> Result<()> {
        let root = TempDir::new()?;
        write_package(
            &root,
            "edition-2024",
        ...
        let commands = routed_commands(&root, "edition-2024");

        assert!(commands.iter().any(|command| command.contains("--bin declared")));
        assert!(commands.iter().any(|command| command.contains("--bin edition-2024")));
        Ok(())
    }

    #[test]
    fn routed_coverage_preserves_required_features_and_skips_untested_bins() -> Result<()> {
        let root = TempDir::new()?;
        write_package(
            &root,
            "feature-bins",
            "[package]\nname = \"feature-bins\"\nversion = \"0.0.0\"\nedition = \"2024\"\n\n[[bin]]\nname = \"gated\"\npath = \"src/gated.rs\"\nrequired-features = [\"cli\"]\n\n[[bin]]\nname = \"untested\"\npath = \"src/untested.rs\"\ntest = false\n",
            &["src/gated.rs", "src/untested.rs", "src/changed.rs"],
        )?;

        let commands = routed_commands(&root, "feature-bins");

        assert!(commands.iter().any(|command| command
            == "cargo llvm-cov test --no-report -p feature-bins --features cli --bin gated --profile agent --locked"));
        assert!(!commands.iter().any(|command| command.contains("--bin untested")));
        Ok(())
    }

    #[test]
    fn routed_coverage_falls_back_when_no_manifest_is_readable() -> Result<()> {
        let root = TempDir::new()?;
        // No crates/ghost/Cargo.toml exists at all.
        let commands = routed_commands(&root, "ghost");

        assert!(commands.iter().any(|command| command
            == "cargo llvm-cov test --no-report -p ghost --lib --profile agent --locked"));
        assert!(!commands.iter().any(|command| command.contains(" --bin ")));
        Ok(())
    }
