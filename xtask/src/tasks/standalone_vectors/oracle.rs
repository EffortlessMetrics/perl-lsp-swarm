...
pub fn validate_vector(vector: &Vector) -> Result<(), OracleError> {
    ...
    if let Some(retry) = &vector.retry {
        if !vector.stage_graph.iter().any(|stage| stage.stage_id == retry.after_stage) {
            return Err(OracleError::CorpusRule(format!(
...
pub fn derive_packet(vector: &Vector, deviation: Deviation) -> Result<SemanticPacket, OracleError> {
    ...
    if deviation == Deviation::ErasePriorAttempt && packet.attempts.len() > 1 {
        if let Some(last) = packet.attempts.last().cloned() {
...
fn fold_stop(
    ...
    if retryable {
        if let Some(plan) = &vector.retry {
            if plan.after_stage == stage_id && !retried_stages.contains(&stage_id) {
...
fn run_attempt(
    ...
    for stage_id in &sequence {
            ...
            && chain.iter().any(|(stage, _)| *stage == StageId::ChecksumIntegrity)
        {
            ...
            let predecessors = predecessors_of(&chain, spec);
        ...
        if call.result != PortResult::Success {
                ...
                && packet.effects.iter().any(|effect| effect.kind == "path_persisted")
            {
                ...
                return fail_step(packet, attempt_id, branch_id, *stage_id, reason);
            }
        }
        ...
        if *stage_id == StageId::ExecutableObservation {
            if let Some(executables) = &call.executables {
                let unsatisfied: Vec<String> = required_executables
                    .iter()
                    .filter(|role| executables.get(*role) != Some(&ExecutableObservation::Ok))
                    .cloned()
                    .collect();
    ...
    {
        Some(stage_id) => AttemptStep::Complete { stage_id },
        None => AttemptStep::Stopped {
            stage_id: StageId::ResolveSubject,
...
fn predecessors_of(chain: &[(StageId, String)], spec: &StageSpec) -> Vec<String> {
    spec.predecessors
        .iter()
        .filter_map(|predecessor| {
            chain.iter().find(|(stage, _)| stage == predecessor).map(|(_, digest)| digest.clone())
        })
        .collect()
}
...
mod tests {
    ...
    fn corpus_covers_the_full_platform_neutral_denominator() {
        let vectors = corpus();
        assert_eq!(vectors.len(), 21, "corpus denominator");
