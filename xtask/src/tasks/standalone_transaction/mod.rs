...
pub fn redaction_finding(text: &str) -> Option<&'static str> {
    const PATTERNS: [(&str, &str); 28] = [
        ...
        ("token=", "inline token"),
        // Credential shapes are anchored to assignment/header/JSON-key forms
        // (`word=`, `word:`, `word":`) so benign lookalike identifiers such as
        // `passwordless-helper` never fail the boundary.
