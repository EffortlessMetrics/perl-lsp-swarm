...
pub fn prepare(config: PrepareConfig) -> Result<()> {
    ...
    println!("perl-core-harness: prepare {:?} for {}", receipt.status, receipt.requested_ref);
...
pub fn smoke(config: SmokeConfig) -> Result<()> {
    ...
    for mode in &modes {
        ...
        if let Err(err) = &run_result {
            ...
            println!("perl-core-harness: {mode} smoke preserved bucketed failure report: {err}");
        }
    ...
    println!("wrote {}", smoke_path.display());
