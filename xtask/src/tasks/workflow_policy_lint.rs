...
pub fn run(config: WorkflowPolicyLintConfig) -> Result<()> {
    ...
    if let Some(fixture) = config.fixture {
        ...
        // Unconditional for the same reason: starting a publisher twice per cut
        // is a release-integrity property, not a lane-economics one (#15454).
        check_release_dispatch_single_entry(&root.join(".github").join("workflows"), &mut issues)?;
    }
...
/// One release cut must start each downstream publisher exactly once (#15454,
/// under #6059 / #4145 criterion 7).
///
/// `release.yml` publishes a non-draft GitHub Release and *then* dispatches its
/// downstream publishers. Publishing emits `release: published`, so any
/// dispatched workflow that also declares a `release:` trigger is started twice
/// per cut, from two entry points that carry different `github.ref` values —
/// which also splits `concurrency.group` keys built from `github.ref`, so the
/// duplicate runs are concurrent rather than serialized.
///
/// The governed set is derived from `release.yml` itself rather than a literal
/// list here, so adding a publisher to that dispatch list brings it under the
/// rule automatically and the guard cannot silently under-cover.
fn check_release_dispatch_single_entry(
    workflows_dir: &Path,
    issues: &mut Vec<LintIssue>,
) -> Result<()> {
    let release_path = workflows_dir.join(RELEASE_DISPATCH_SOURCE);
    ...
    let release: Value = serde_yaml_ng::from_str(&raw)
        .with_context(|| format!("parsing workflow YAML {}", release_path.display()))?;

    let dispatched = dispatched_workflows(&release);
    if dispatched.is_empty() {
        // Reaching zero means the dispatch shapes this guard recognizes were
        // refactored away. Failing here is the point: a silently empty governed
        // set would make every clause below vacuous.
        issues.push(LintIssue {
            level: "error",
            code: "RELEASE_DISPATCH_SET_UNREADABLE",
            workflow: RELEASE_DISPATCH_SOURCE.to_string(),
            message: format!(
                "no downstream publisher could be derived from {RELEASE_DISPATCH_SOURCE}; the \
                 single-entry guard would cover nothing. Keep dispatches as `workflows=( ... )` \
                 entries or `/actions/workflows/<file>/dispatches` paths, or update \
    ...
    for workflow_file in dispatched {
        let path = workflows_dir.join(&workflow_file);
        if !path.exists() {
            issues.push(LintIssue {
                level: "error",
                code: "RELEASE_DISPATCH_TARGET_MISSING",
                workflow: RELEASE_DISPATCH_SOURCE.to_string(),
                message: format!(
                    "{RELEASE_DISPATCH_SOURCE} dispatches `{workflow_file}`, which does not exist"
                ),
            });
            continue;
        }

        let target_raw = fs::read_to_string(&path)
            .with_context(|| format!("reading workflow file {}", path.display()))?;
        let target: Value = serde_yaml_ng::from_str(&target_raw)
            .with_context(|| format!("parsing workflow YAML {}", path.display()))?;
        let target_triggers = triggers(&target);

        if target_triggers.iter().any(|trigger| trigger == "release") {
            issues.push(LintIssue {
                level: "error",
                code: "RELEASE_EVENT_FEEDBACK_PATH",
                workflow: workflow_file.clone(),
                message: format!(
                    "workflow is dispatched by {RELEASE_DISPATCH_SOURCE} and also declares a \
                     `release:` trigger, so one release cut starts it twice; remove the \
                     `release:` trigger and let the orchestrated dispatch be the single entry"
                ),
            });
        }

        if !target_triggers.iter().any(|trigger| trigger == "workflow_dispatch") {
            issues.push(LintIssue {
                level: "error",
                code: "RELEASE_DISPATCH_TARGET_UNDISPATCHABLE",
                workflow: workflow_file.clone(),
                message: format!(
                    "{RELEASE_DISPATCH_SOURCE} dispatches this workflow, but it declares no \
                     `workflow_dispatch` trigger, so the dispatch can never start it"
                ),
            });
        }
    }
...
/// The release workflow whose dispatch steps define the governed publisher set.
const RELEASE_DISPATCH_SOURCE: &str = "release.yml";

/// Workflow files `release.yml` starts, collected from both dispatch shapes it
/// uses: a `workflows=( ... )` shell array fed to `gh workflow run`, and
/// `/actions/workflows/<file>/dispatches` REST paths.
fn dispatched_workflows(release: &Value) -> Vec<String> {
    let mut found: HashSet<String> = HashSet::new();
    for script in run_scripts(release) {
        collect_shell_array_dispatches(&script, &mut found);
        collect_rest_api_dispatches(&script, &mut found);
    }
    let mut dispatched: Vec<String> = found.into_iter().collect();
    dispatched.sort();
    dispatched
}
...
/// Entries of a `name=(` ... `)` shell array that name a workflow file.
fn collect_shell_array_dispatches(script: &str, found: &mut HashSet<String>) {
    ...
    while let Some(line) = lines.next() {
        ...
        for entry in lines.by_ref() {
            ...
            if is_workflow_file_token(token) {
...
/// Workflow files named by `/actions/workflows/<file>/dispatches` REST paths.
fn collect_rest_api_dispatches(script: &str, found: &mut HashSet<String>) {
    const MARKER: &str = "/actions/workflows/";
    let mut rest = script;
    while let Some(index) = rest.find(MARKER) {
        ...
        let name: String = tail
            ...
            .collect();
        if is_workflow_file_token(&name) {
            found.insert(name);
        }
        rest = tail;
    }
...
/// A bare workflow filename. Interpolated or quoted tokens are rejected: this
/// guard reports only names it can resolve to a real file.
fn is_workflow_file_token(token: &str) -> bool {
    (token.ends_with(".yml") || token.ends_with(".yaml"))
        && !token.contains('$')
        && !token.contains('"')
        && !token.contains('\'')
...
"##,
    ...
    fn release_dispatch_fixture(case: &str) -> Result<PathBuf> {
        let root = project_root()?;
        Ok(root.join("xtask/tests/fixtures/release-dispatch").join(case))
    }

    fn release_dispatch_issues(case: &str) -> Result<Vec<LintIssue>> {
        let mut issues = Vec::new();
        check_release_dispatch_single_entry(&release_dispatch_fixture(case)?, &mut issues)?;
        Ok(issues)
    }

    /// The claim #15454 makes, asserted against the shipped workflows: every
    /// publisher `release.yml` dispatches must have exactly one entry point.
    ///
    /// Before the repair this fails on `brew-bump.yml`, `chocolatey-bump.yml`,
    /// `scoop-bump.yml`, and `winget-bump.yml`, each of which also declared
    /// `on: release: types: [published]`.
    #[test]
    fn shipped_release_publishers_have_one_entry_point() -> Result<()> {
        let root = project_root()?;
        let mut issues = Vec::new();
        check_release_dispatch_single_entry(&root.join(".github").join("workflows"), &mut issues)?;
        assert!(
            issues.is_empty(),
            "one release cut must start each dispatched publisher exactly once: {issues:?}"
        );
        Ok(())
    }

    /// The governed set is derived, not declared, so this pins what the
    /// derivation actually sees on the shipped `release.yml`. A refactor that
    /// quietly drops a publisher from the dispatch shapes fails here rather
    /// than silently shrinking the rule's coverage.
    #[test]
    fn shipped_release_dispatch_set_covers_both_dispatch_shapes() -> Result<()> {
        let root = project_root()?;
        let raw = fs::read_to_string(root.join(".github/workflows/release.yml"))?;
        let release: Value = serde_yaml_ng::from_str(&raw)?;
        let dispatched = dispatched_workflows(&release);

        // REST-API shape.
        for publisher in ["publish-crates.yml", "publish-extension.yml", "docker-publish.yml"] {
            assert!(
                dispatched.iter().any(|found| found == publisher),
                "primary publisher `{publisher}` must be governed: {dispatched:?}"
            );
        }
        // Shell-array shape.
        for bump in ["brew-bump.yml", "chocolatey-bump.yml", "scoop-bump.yml", "winget-bump.yml"] {
            assert!(
                dispatched.iter().any(|found| found == bump),
                "package-channel publisher `{bump}` must be governed: {dispatched:?}"
            );
        }
        Ok(())
    }
    ...
    #[test]
    fn fixture_single_entry_release_fanout_passes() -> Result<()> {
        let issues = release_dispatch_issues("single_entry")?;
        assert!(issues.is_empty(), "single-entry fan-out is accepted: {issues:?}");
        Ok(())
    }

    /// Prose is not a dispatch. The fixture's release workflow names
    /// `demo-retired.yml` in a comment beside the real array; if comment lines
    /// widened the governed set, that name would enter it and — having no file
    /// — surface as `RELEASE_DISPATCH_TARGET_MISSING` above. Asserting the
    /// exact set keeps that claim explicit rather than incidental.
    #[test]
    fn fixture_dispatch_set_excludes_commented_workflow_names() -> Result<()> {
        let raw =
            fs::read_to_string(release_dispatch_fixture("single_entry")?.join("release.yml"))?;
        let release: Value = serde_yaml_ng::from_str(&raw)?;
        assert_eq!(
            dispatched_workflows(&release),
            vec!["demo-bump.yml".to_string(), "demo-publish.yml".to_string()],
            "only executed dispatch shapes define the governed set"
        );
        Ok(())
    }

    /// Negative control: a dispatched publisher that also keeps the
    /// `release: published` trigger is the duplicate-entry defect itself.
    #[test]
    fn fixture_release_event_feedback_path_fails() -> Result<()> {
        let issues = release_dispatch_issues("feedback_path")?;
        let reported: Vec<_> =
            issues.iter().filter(|issue| issue.code == "RELEASE_EVENT_FEEDBACK_PATH").collect();
        assert_eq!(reported.len(), 1, "exactly the dual-entry workflow is reported: {issues:?}");
        assert_eq!(reported[0].workflow, "demo-bump.yml");
        assert_eq!(reported[0].level, "error");
        Ok(())
    }

    /// Negative control: an empty governed set would make every other clause
    /// vacuously true, so it must fail rather than pass quietly.
    #[test]
    fn fixture_unreadable_dispatch_set_fails() -> Result<()> {
        let issues = release_dispatch_issues("unreadable")?;
        assert!(
            issues.iter().any(|issue| issue.code == "RELEASE_DISPATCH_SET_UNREADABLE"),
            "a guard that covers nothing must say so: {issues:?}"
        );
        Ok(())
    }

    /// Negative control on the opposite direction: removing the `release:`
    /// trigger must not leave the publisher unreachable by the dispatch that
    /// replaced it.
    #[test]
    fn fixture_undispatchable_target_fails() -> Result<()> {
        let issues = release_dispatch_issues("undispatchable")?;
        let reported: Vec<_> = issues
            .iter()
            .filter(|issue| issue.code == "RELEASE_DISPATCH_TARGET_UNDISPATCHABLE")
            .collect();
        assert_eq!(reported.len(), 1, "the unreachable publisher is reported: {issues:?}");
        assert_eq!(reported[0].workflow, "demo-bump.yml");
        Ok(())
    }
}
