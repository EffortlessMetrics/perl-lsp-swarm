...
use xtask::stack_increment::{
    STACK_ADVISORY_CONTEXT_NAME, STACK_INCREMENT_RESULT_SCHEMA, STACK_INCREMENT_SUBJECT_SCHEMA,
    StackEndpoint, StackIncrementResultV1, StackIncrementSubjectV1, StackPlanRequest,
    StackSubjectInput, TrustContext, compile_stack_plan, compile_subject, render_explanation,
    resolve_endpoint_live, stack_plan_digest, subject_digest, validate_subject,
};
...
fn gh_pr_view(repository: &Path, pr: u64) -> Result<PullRequestFacts> {
    let output = std::process::Command::new("gh")
        ...
        .arg("number,headRefName,headRefOid,baseRefOid,body")
...
fn assemble_live_subject_input(repository: &Path, pr: u64) -> Result<StackSubjectInput> {
    ...
    let (parent_sha, parent_tree) = resolve_endpoint_live(repository, &declared_parent_head)
    ...
    Ok(StackSubjectInput {
        ...
        parent: StackEndpoint {
            pr_number: edge.parent_pr_number,
            issue_node_id: String::new(),
            branch: format!("parent-of-{pr}"),
    ...
    match schema {
        ...
        STACK_INCREMENT_RESULT_SCHEMA => {
            ...
            if result.context_name != STACK_ADVISORY_CONTEXT_NAME {
                bail!("result identity fields do not reconcile");
            }
            if result.protected_main_state != xtask::stack_increment::PROTECTED_MAIN_NOT_EVALUATED {
                bail!(
                    "protected-main state {:?} is not permitted on a stack-local result",
