    ...
    match execution {
        Ok(execution) => {
            ...
            if !execution.test_execution_reached_attempts.is_empty() {
                metrics.get_or_insert_with(GateMetrics::default).test_execution_reached_attempts =
...
"#,
    ...
    fn plan_gates_threads_staged_tree_oid_into_non_commit_tiers() -> color_eyre::eyre::Result<()> {
        ...
        // #15413: `plan_gates` with `staged: true` shells `git write-tree`
        // against the live index, and parallel test threads can hold
        // `.git/index.lock` transiently. Wait (bounded) for the lock to
        // clear instead of flaking on contention; a persistent lock still
        // fails loudly rather than silently testing a stale tree.
        let index_lock = root.join(".git/index.lock");
        let deadline = std::time::Instant::now() + std::time::Duration::from_secs(30);
        while index_lock.exists() {
            if std::time::Instant::now() > deadline {
                color_eyre::eyre::bail!("timed out waiting for transient git index.lock to clear");
            }
    ...
    fn retried_cargo_test_gate_retains_each_attempt_reach_result() -> color_eyre::eyre::Result<()> {
        ...
        let command = format!(
            "if [ -f {marker_display} ]; then printf 'running 2 tests\\n'; exit 0; \
             else touch {marker_display}; sleep 3; fi; exit 0; true && cargo test --lib --locked"
        );
