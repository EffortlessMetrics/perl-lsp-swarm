...
fn run_gate_plan(
    ...
    for (idx, planned_gate) in plan.selected.iter().enumerate() {
        ...
        // Re-verify between gates only: a gate that ran may have dirtied
        // the tree, which would unbind the plan's execution subject. The
        // first gate is already covered by the pre-execution binding check
        // above -- nothing between the two can mutate tracked state.
        if idx > 0 && routed_gate_plan.is_some() {
            let status =
                cmd!("git", "status", "--porcelain=v1").dir(&root).read().map_err(|error| {
