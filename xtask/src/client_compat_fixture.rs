...
pub fn canonical_expectation_set_digest() -> Result<String> {
    expectation_set_digest(CANONICAL_EXPECTATION_SET_ID, CANONICAL_EXPECTATION_IDS)
}

/// Digest of one expectation set, bound to its id and its exact membership.
///
/// The digest is order-independent — callers may list ids in any order — but any
/// change to the set's id or membership changes it, so a receipt cannot claim an
/// expectation set it did not actually run.
pub fn expectation_set_digest(set_id: &str, ids: &[&str]) -> Result<String> {
    ensure!(is_reason_token(set_id), "expectation set id is not a stable reason token: {set_id}");
...
/// The stable-reason-token rule shared by every identity in these contracts.
