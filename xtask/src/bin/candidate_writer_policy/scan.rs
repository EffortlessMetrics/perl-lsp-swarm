...
/// Events that take the workflow definition from the candidate's own tree.
///
/// For `pull_request` GitHub reads the workflow from the pull request's merge
/// ref, and for `merge_group` from the queued merge branch. Both contain the
/// candidate's edits, so the steps of such a job are candidate-authored.
const CANDIDATE_SOURCED_EVENTS: &[&str] = &["pull_request", "merge_group"];

/// Events a candidate can reach whose definition comes from the base branch.
///
/// For `pull_request_target` GitHub reads the workflow from the base branch, so
/// the steps are maintainer-authored and a candidate cannot introduce one. The
/// hazard is the inverse: a base-authored write-capable job that checks out and
/// executes candidate-controlled code.
const BASE_SOURCED_CANDIDATE_EVENTS: &[&str] = &["pull_request_target"];

/// Expressions that resolve to candidate-controlled refs or repositories.
const CANDIDATE_REF_MARKERS: &[&str] = &[
...
pub(crate) fn scan_workflow_with_policy(
    ...
    let candidate_sourced =
        triggers.iter().any(|trigger| CANDIDATE_SOURCED_EVENTS.contains(&trigger.as_str()));
    let base_sourced =
        triggers.iter().any(|trigger| BASE_SOURCED_CANDIDATE_EVENTS.contains(&trigger.as_str()));
    if !candidate_sourced && !base_sourced {
        return Vec::new();
    }
    ...
    for (job_name, job_value) in jobs {
        ...
        if candidate_sourced && let Some(uses) = mapping_get(job, "uses").and_then(Value::as_str) {
        ...
        if candidate_sourced {
            findings.push(Finding {
                ...
                kind: FindingKind::CandidateDefinedWriter,
                detail:
                    "write-capable candidate job is defined by candidate-controlled workflow steps"
                        .into(),
            });
        } else {
            scan_candidate_code_execution(workflow_name, job_name, steps, &mut findings);
        }
        scan_self_modifying_steps(workflow_name, job_name, steps, &mut findings);
    }
    findings
}
...
/// Flag a base-authored write-capable job that checks out candidate code.
///
/// The steps themselves are trusted, so the escape is not authorship: it is
/// giving a write token to code the candidate controls.
fn scan_candidate_code_execution(
    workflow_name: &str,
    job_name: &str,
    steps: &[Value],
    findings: &mut Vec<Finding>,
    ...
    for (index, step) in steps.iter().enumerate() {
        ...
        let checks_out_candidate_ref = mapping_get(step, "uses")
            .and_then(Value::as_str)
            .is_some_and(|uses| uses.starts_with("actions/checkout"))
            && mapping_get(step, "with").and_then(Value::as_mapping).is_some_and(|with| {
                ["ref", "repository"].iter().any(|key| {
                    mapping_get(with, key)
                        .and_then(Value::as_str)
                        .is_some_and(references_candidate_ref)
                })
            });
        let checks_out_via_cli = mapping_get(step, "run")
            .and_then(Value::as_str)
            .is_some_and(|run| normalize_shell(run).contains("gh pr checkout"));
        if checks_out_candidate_ref || checks_out_via_cli {
            findings.push(Finding {
                workflow: workflow_name.into(),
                job: job_name.into(),
                kind: FindingKind::CandidateCodeExecutionWriter,
                detail: format!(
                    "step {} gives a write-capable base-defined job a candidate-controlled tree",
                    index + 1
                ),
            });
        }
...
fn references_candidate_ref(value: &str) -> bool {
    CANDIDATE_REF_MARKERS.iter().any(|marker| value.contains(marker))
}

fn scan_self_modifying_steps(
    workflow_name: &str,
    job_name: &str,
    steps: &[Value],
    findings: &mut Vec<Finding>,
