...
impl ConceptLedger {
    ...
    fn render_markdown(&self) -> Result<String> {
        ...
        line(
            &mut output,
            &format!("- Schema: `{}`", normalized.schema_version),
        )?;
        line(
            &mut output,
            &format!("- Ledger: `{}`", normalized.ledger_id),
        )?;
        line(
            &mut output,
...
impl ConceptRow {
    fn validate(&self) -> Result<()> {
        ...
        if self.evidence.is_empty() {
            bail!(
                "compiler concept {} must name at least one evidence item",
                self.concept_id
            );
        }
    ...
    fn stage_states(&self) -> [(&'static str, &'static str); 11] {
        [
            ...
            (
                "Effects / world",
                self.compile_effects_world.stable_name(),
            ),
            ...
            (
                "Provider eligibility",
                self.provider_eligibility.stable_name(),
            ),
