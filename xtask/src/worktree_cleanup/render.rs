use super::model::{ObservationState, WorktreeClassification, WorktreeCleanupPlan};
...
pub fn render_human(plan: &WorktreeCleanupPlan) -> String {
    ...
    push_line(&mut output, format!("  repository: {}", subject.repository_root.display()));
    push_line(&mut output, format!("  common dir: {}", subject.common_dir.display()));
    push_line(&mut output, format!("  observed: {}", plan.observed_at));
    push_line(&mut output, format!("  plan digest: {}", plan.plan_digest));
    push_line(&mut output, format!("  aggregate: {}", plan.aggregate_classification.as_str()));
    ...
    for entry in &plan.entries {
        ...
        if !entry.reason_tokens.is_empty() {
            push_line(&mut output, format!("  reasons: {}", entry.reason_tokens.join(", ")));
        }
        if let Some(action) = &entry.proposed_action {
            push_line(
                ...
                format!(
                    ...
                    if action.targetable { "targetable" } else { "review only" }
                ),
            );
        }
...
fn render_not_proven_details(output: &mut String, entry: &super::model::WorktreePlanEntry) {
    let observations = [
        ...
        (&entry.facts.administrative_path.state, entry.facts.administrative_path.detail.as_deref()),
        (&entry.facts.dirty.state, entry.facts.dirty.detail.as_deref()),
        (&entry.facts.untracked.state, entry.facts.untracked.detail.as_deref()),
        (&entry.facts.open_pr.state, entry.facts.open_pr.detail.as_deref()),
        (&entry.facts.merged_pr.state, entry.facts.merged_pr.detail.as_deref()),
        (&entry.facts.unpushed_commits.state, entry.facts.unpushed_commits.detail.as_deref()),
    ];
