...
/// The supervisor deadline for the timeout/interruption session: long enough
/// that the session's own bootstrap and product observation complete on the
/// slowest supported host before the deliberate indefinite barrier takes
/// over, and the forced-kill path stays bounded without burning the journey
/// budget (the kill lands this many milliseconds after launch at the latest).
pub const TIMEOUT_SESSION_TIMEOUT_MS: u64 = 150_000;
...
pub fn host_lifecycle_run(
    ...
    let sessions = match variant {
        ...
        LifecycleFixtureVariant::ServerRestartRelabel => {
            vec![run_host_session(
                repo_root,
                run,
                &driver,
                &fixture_root,
...
/// aggregate cleanup, the prefixed per-host artifacts, and the
/// designed-boundary driver-completeness.
///
/// The aggregate cleanup is the journey's owned-resource proposition, judged
/// per host: a leaking host fails the journey; an orderly host carries the
/// substrate's pass; a designed forced-failure host carries the substrate's
/// conservative `not_proven` label for the orderly-shutdown dimension it
/// deliberately skipped, but its owned-resource observation (retained ledger
/// with an available probe, zero survivors, and a clean settled probe) is
/// exactly the #11387 `forced_failure` cleanup kind — proven, not missing.
/// The per-host conservative labels stay in the receipt limitations either
/// way, so the aggregate never hides them.
pub fn aggregate_journey_observation(sessions: &[HostSessionRecord]) -> ProcessObservation {
    ...
    for session in sessions {
        let designed_failure_clean = matches!(
            session.role.as_str(),
            "assertion_failure_session" | "timeout_interruption_session"
        ) && session.observation.cleanup != CleanupResult::Fail
            && session.ledger_probe_available()
            && session.ledger_survivors() == Some(0)
            && session.settled_probe_clean == Some(true);
        let host_cleanup =
            if designed_failure_clean { CleanupResult::Pass } else { session.observation.cleanup };
        if host_cleanup == CleanupResult::Fail || cleanup == CleanupResult::Fail {
            cleanup = CleanupResult::Fail;
        } else if host_cleanup == CleanupResult::NotProven || cleanup == CleanupResult::NotProven {
    ...
    for session in sessions {
        for artifact in &session.observation.artifacts {
...
pub fn evaluate_lifecycle_observation(
    ...
    let negative_control = variant.expected_negative_reason().is_some();
    let hard_fail = any_leak
        || !designed_boundaries
        || actionable_cells.iter().any(|cell| cells.get(*cell) == Some(&ObservationResult::Fail));
    let result = if negative_control {
        // A negative control never passes: reaching the complete positive
        // shape would be an oracle violation, and every evidence shortfall
        // is its typed failure — either way the run reports Fail.
        ObservationResult::Fail
