...
fn config_string(
    ...
    // different string key; stable Cargo ignores it. A present non-table
    // `build` is rejected by Cargo, so it is unresolvable rather than absent.
    match value.get(table) {
        None => Ok(None),
        Some(nested) if nested.as_table().is_none() => Err(CARGO_CONFIG_WRAPPER_NOT_RESOLVED),
...
mod tests {
    ...
    #[test]
    fn scalar_build_section_is_unresolvable_not_absent() {
        let (root, home, ws, _) = fixture();
        write_config(&ws.join(".cargo/config.toml"), "build = \"not-a-table\"\n");
        let resolved = resolve_tree(&ws, root.path(), &home);
        assert!(resolved.rustc_wrapper().is_none());
        assert!(!resolved.is_complete());
        assert!(
    ...
    #[test]
    fn array_build_section_is_unresolvable_not_absent() {
        let (root, home, ws, _) = fixture();
        write_config(&ws.join(".cargo/config.toml"), "build = [\"not-a-table\"]\n");
        let resolved = resolve_tree(&ws, root.path(), &home);
        assert!(resolved.rustc_wrapper().is_none());
