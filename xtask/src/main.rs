...
enum Commands {
    ...
    /// Emit a coverage baseline receipt from an existing LCOV snapshot.
    CoverageBaseline {
        /// LCOV input generated by cargo-llvm-cov.
        #[arg(long, default_value = "target/lcov.info")]
        lcov: PathBuf,
        /// Local coverage ratchet policy.
        #[arg(long, default_value = ".ci/coverage-baseline.txt")]
        baseline: PathBuf,
        /// Codecov policy file.
    ...
    /// Aggregate coverage and RIPR proof into one quality-gate receipt.
    QualityGate {
        /// Gate mode. Advisory never blocks; enforcement modes fail on missing proof.
        #[arg(long, value_enum, default_value = "advisory")]
        mode: QualityGateCliMode,
        /// RIPR+ receipt input.
        #[arg(long, default_value = "target/receipts/quality/ripr-plus.json")]
        ripr_receipt: PathBuf,
        /// Diff-scoped RIPR PR receipt input.
        #[arg(long, default_value = "target/ripr/pr/repo-exposure.json")]
        ripr_pr_receipt: PathBuf,
        /// RIPR review guidance receipt input.
        #[arg(long, default_value = "target/ripr/review/comments.json")]
        review_receipt: PathBuf,
        /// Coverage baseline receipt input.
        #[arg(long, default_value = "target/receipts/quality/coverage-baseline.json")]
        coverage_receipt: PathBuf,
        /// Codecov policy file checked by the aggregate gate.
        #[arg(long, default_value = "codecov.yml")]
        codecov: PathBuf,
        /// Optional Codecov patch coverage percentage for this PR.
        #[arg(long)]
        patch_coverage: Option<f64>,
        /// External source that enforces the Codecov patch status when the numeric value is unavailable locally.
        #[arg(long, value_enum)]
        patch_status_source: Option<QualityGatePatchStatusSource>,
        /// Temporary burn-down exception policy input.
        #[arg(long, default_value = "policy/quality-gate-exceptions.toml")]
        exceptions: PathBuf,
        /// Aggregated quality-gate receipt output.
        #[arg(long, default_value = "target/receipts/quality/quality-gate.json")]
        receipt: PathBuf,
        /// Human-readable quality-gate summary output.
        #[arg(long, default_value = "target/receipts/quality/quality-gate.md")]
        summary: PathBuf,
        /// Validate the existing quality-gate receipt instead of rewriting it.
        #[arg(long)]
        check: bool,
    },
...
impl From<QualityGateCliMode> for quality_gate::QualityGateMode {
    fn from(mode: QualityGateCliMode) -> Self {
        match mode {
            QualityGateCliMode::Advisory => Self::Advisory,
            QualityGateCliMode::EnforceNewRipr => Self::EnforceNewRipr,
            QualityGateCliMode::EnforcePatchCoverage => Self::EnforcePatchCoverage,
            QualityGateCliMode::Enforce => Self::Enforce,
        }
    }
}

#[derive(Clone, Copy, Debug, Eq, PartialEq, ValueEnum)]
enum QualityGatePatchStatusSource {
    Codecov,
}

impl From<QualityGatePatchStatusSource> for quality_gate::PatchStatusSource {
    fn from(source: QualityGatePatchStatusSource) -> Self {
        match source {
            QualityGatePatchStatusSource::Codecov => Self::Codecov,
        }
    }
}
...
fn main() -> Result<()> {
    ...
    match cli.command {
        ...
        Commands::RiprPlus { root, receipt, check } => {
            quality_baseline::ripr_plus(&root, &receipt, check)
        }
        Commands::CoverageBaseline { lcov, baseline, codecov, receipt, check } => {
            quality_baseline::coverage_baseline(&lcov, &baseline, &codecov, &receipt, check)
        }
        Commands::QualityGate {
            ...
            summary,
            check,
        } => quality_gate::run(quality_gate::QualityGateConfig {
            mode: mode.into(),
            ripr_receipt: &ripr_receipt,
            ripr_pr_receipt: &ripr_pr_receipt,
            review_receipt: &review_receipt,
            coverage_receipt: &coverage_receipt,
            codecov: &codecov,
            patch_coverage,
            patch_status_source: patch_status_source.map(Into::into),
