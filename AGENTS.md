...
This file is Codex's route map. `.agents/skills/` contains executable provider-native
procedures. Shared docs define invariants and GitHub surface ownership; they do not
replace a named `$skill`.

GitHub owns durable live transaction state. Agent topology, liveness, retries, temporary
plans, queue order, and runtime frontier remain runtime-local.
...
`NOT_PROVEN` boundary—not at research, a plan, one agent return, or green checks that do
not complete the claim.
...
The normal claim owner for the open-PR campaign is `.codex/agents/pr-lane.toml`.
Dispatch one persistent lane per PR or coherent claim. It invokes `$deliver-pr` and
keeps its thread, loaded source context, and worktree across review, repair, proof,
...
Do not create a new agent merely because the next skill changes. Equally, do not forbid
role-specialized agents when they improve evidence. A dedicated review context is useful
when independent attention matters, and `.codex/agents/pr-reviewer.toml` may retain one
PR across `$review-pr`, `$review-candidate`, `$review-tests`, external-oracle work, and
re-review without re-ingesting the candidate for every angle.

A PR lane or dedicated reviewer that finds a bounded candidate-owned defect may fix it
in the same context when the parent grants mutation/publication authority and no other
writer owns the candidate. The repair still returns through affected proof and review.
If that reviewer becomes the writer, final review must retain a genuinely different
oracle, method, threat model, environment, or review context where substantive
...
agent per skill or review lens merely to repeat the same PR ingestion.
...
```text
multi-PR campaign
→ dispatch useful disjoint PR lanes and role-specialized review contexts
...
Keep review breadth wider than mutation breadth. PR lanes and dedicated reviewers may
...
intermediate review packet merely so another agent can rediscover the PR. Do not spawn a
new reviewer for every review angle when one reviewer can reliably consume the required
skills in the same context.
...
surface—not merely a different agent identity or stage label.
...
stale agent handle, or unavailable cleanup operation is not by itself a hard stop.
...
- a persistent PR lane or reviewer may retain its worktree across review, repair, and
