...
| `perl/minimal_ptkdb_peer.pl`    | dual-use core-Perl fixture: executable synthetic reference peer, or version-pinned experimental `.ptkdbrc` mirror plugin — see [the integration target](../../docs/reference/PTKDB_PEER_INTEGRATION_TARGET.md#repository-implementation-substrate) |
...
rendezvous is present; for the pinned `Devel::ptkdb 1.1091` seam it preserves
`set_file`, reports real `DB::DB` stop locations, and emits bounded termination.
That headless conformance is partner-seam evidence, not a live Tk compatibility
receipt.
