...
The family stays `unadvertised-until-r04`: it appears in no capability, and no reload mechanism backs it yet (#10098 owns the mechanism, #10102 the composition). Since R03 (#10102) the request has exactly one route — the preview/test profile that binds the negotiated family into the debug session lifecycle with generation-safe reconciliation; the request name stays absent from the standard command authority (`SUPPORTED_COMMANDS`), no production construction enables the profile, and outside the profile the request receives the adapter's ordinary unknown-command response. The response's reconciliation sub-object (`loaded_source_refresh`/`inspection_invalidation`/`breakpoint_reconciliation`) carries the R03 dispositions (`not_applicable` for non-mutating outcomes; `invalidated`/`pending`/`unavailable` for terminal mutation outcomes — fail-closed about what cannot be reacquired). Registration is not behavior: ordinary standard DAP remains usable when the family is absent or incompatible, and handler registration or family negotiation is not capability proof.
