...
Invoke `/coding-standards` for the full set of banned constructs and required patterns.

**Hard blockers** (must fix or fail):
- Banned constructs in production (see `/coding-standards` for full list)
...
### 6. Create PR with Labels (if merge-ready)
...
### 7. Enable auto-merge for small PRs
...
Append metrics to `.ops-perl-lsp/swarm-metrics.jsonl` with: agent name, branches reviewed, PRs created, blockers found, timestamp.
