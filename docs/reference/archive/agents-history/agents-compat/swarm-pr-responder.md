...
Invoke `/coding-standards` for the full set of verification steps. At minimum:
```bash
# Make fixes
cargo fmt --all
...
Append metrics to `.ops-perl-lsp/swarm-metrics.jsonl` with: agent name, PRs responded to, comments addressed, fixes pushed, timestamp.
