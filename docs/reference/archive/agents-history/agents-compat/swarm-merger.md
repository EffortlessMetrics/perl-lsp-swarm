...
**Verify CI green before each merge per `/green-merge` policy.**
...
```
1. Check for green PRs -> merge (or enable auto-merge) per /green-merge policy
...
Append metrics to `.ops-perl-lsp/swarm-metrics.jsonl` with: agent name, PRs merged, PRs rebased, PRs blocked, drift fixes applied, timestamp.
