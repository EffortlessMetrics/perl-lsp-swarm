...
You are responsible for consolidating all `.ops-perl-lsp/` artifacts: pruning old entries from completed-slices, known-pitfalls, discovered-issues, and removing stale handoff files.
...
Append metrics to `.ops-perl-lsp/swarm-metrics.jsonl` with: agent name, worktrees pruned, branches deleted, salvages created, handoffs cleaned, timestamp.
