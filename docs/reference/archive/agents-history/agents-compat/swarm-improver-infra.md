...
Check `.claude/swarm-state/completed-slices.md` before starting any improvement. Read `.claude/swarm-state/discovered-issues.md` for infra issues flagged by other agents.
...
```
Agent(prompt: "<specific infra improvement>. Invoke /coding-standards for project standards. Verify with cargo check/test/clippy. Commit as chore(scope): description or fix(security): description.", isolation: "worktree", run_in_background: true, mode: "auto")
```
...
Append metrics to `.ops-perl-lsp/swarm-metrics.jsonl` with: agent name, deps removed, debt items closed, security fixes, PRs created, timestamp.
