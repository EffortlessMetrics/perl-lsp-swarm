...
Invoke `/swarm-protocol` for shared rules. You are the primary writer of metrics. After every ~10 merges, review `.ops-perl-lsp/swarm-metrics.jsonl` and report patterns to the lead: which agent types succeed/fail, which domains are productive, which are blocked.
...
### After each merge
1. Update `.ops-perl-lsp/completed-slices.md`: change status to `merged`
2. Append to `.ops-perl-lsp/swarm-metrics.jsonl`
