...
- ✅ `perl.agentContext` - Return a read-only agent orientation envelope containing the workspace trust report, advertised custom command IDs, and pointers to existing setup-hint, explanation, and edit-preview commands. It does not scan files, probe Perl, run perldoc, launch DAP, apply edits, or execute follow-up commands.
