...
| `gh pr view <n> --json reviewThreads` | `mcp__github__pull_request_read(method:"get_review_comments", pullNumber:n)` | Returns threads with `isResolved`/`isOutdated` flags, the `PRRT_…` thread node ID, and inline comments (each with its `#discussion_r…` numeric ID) |
| `gh api .../pulls/comments/<id>/replies` | `mcp__github__add_reply_to_pull_request_comment(pullNumber:n, commentId, body)` | `commentId` is the **numeric** ID from the `#discussion_r…` anchor, **not** the `PRRT_…` thread node ID |
