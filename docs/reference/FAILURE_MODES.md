...
### gh pr checks Masks Aggregator Failure

**Symptom**: `gh pr checks <PR>` reports "Passed: N, Failed: 0" but the PR is actually blocked. The CI Gate (Merge-Blocking) aggregator is FAILURE on the latest SHA.
...
**Mitigation**: Never trust `gh pr checks` for promote/merge decisions. Use raw rollup query above. A PR with full sign-off chain + `UNSTABLE` mergeStateStatus + reporting "0 failed" should always trigger a raw rollup re-check.

**Prevention**: Ops agents must use direct rollup queries, not summaries, for merge-readiness decisions. Document this in the ops prompt: is for human-readable summaries, not merge gates.

**Memory**: `feedback_rtk_pr_checks_masks_failures.md`

**Example PRs**: #7016 (reported "Passed: 14, Failed: 0"; raw rollup showed `CI Gate FAILURE` on latest SHA)
