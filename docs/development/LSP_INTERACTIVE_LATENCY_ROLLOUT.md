...
Current code computes diagnostics first and checks `client_supports_pull_diags`
only at the end.
...
`didOpen` currently calls `publish_diagnostics(uri)` in successful open paths.
...
The current default is hardcoded as 250 ms.
...
The current initialization path starts workspace indexing after the server is
initialized.
...
The current capability layer advertises full semantic-token delta, while routing
implements `full` and `range`; the full handler returns token `data` without
`resultId`.
