...
**Updated**: 2026-06-20
**Release payload SHA**: `89a1225e48852583f6662da43257dae6a651a96b`
**Current `origin/main` SHA at refresh**: `89a1225e48852583f6662da43257dae6a651a96b`
...
Note: the release payload SHA is the latest audited merged closeout commit that changed release-facing product, CI, or changelog scope. It includes #1839, which surfaces quoted special-character hash keys in completion; #1840, which escapes markdown metacharacters in hover docs; #1889, which serializes `filterText` in completion items; #1811, which adds DAP `totalVariables`; #1807, which adds logpoint interpolation substrate; #1903, which removes invalid Perl syntax from gold corpus fixtures; #1904, which completes hash-last synthetic facts in `FileSemanticBundle`; and #1894, which repaired the #1839 fmt/Patch-95 gate fallout with double-quoted hash-key coverage. The previous readiness bundle merge SHA records the doc-only staging bundle that captured the closeout claims through #1759/#1888. Verify the current `origin/main` SHA again at dispatch time; later doc-only clarification commits do not expand the product claim.
...
Latest merged release-scope product/gate PR checked: [#1894](https://github.com/EffortlessMetrics/perl-lsp-swarm/pull/1894), merge commit `89a1225e48852583f6662da43257dae6a651a96b`, merged 2026-06-20T15:41:38Z. It follows hash-key completion special-character support [#1839](https://github.com/EffortlessMetrics/perl-lsp-swarm/pull/1839), merge commit `b43cbff19e1682fa9f42fb9b12a0cccfbbade51f`; sigil-aware completion regression coverage [#1842](https://github.com/EffortlessMetrics/perl-lsp-swarm/pull/1842), merge commit `2df14f4c3eb1deca08f468982375d540b838364f`; hover markdown escaping [#1840](https://github.com/EffortlessMetrics/perl-lsp-swarm/pull/1840), merge commit `4cb6609ba2d52e24d1810cdbcf49704b0bb9e72f`; completion `filterText` serialization [#1889](https://github.com/EffortlessMetrics/perl-lsp-swarm/pull/1889), merge commit `24c00c6b04512f966106d689af5881c4d3ef0832`; DAP `totalVariables` [#1811](https://github.com/EffortlessMetrics/perl-lsp-swarm/pull/1811), merge commit `b08de9857d7dc9b248b4cfa931285f2b8ec06688`; corpus gold-fixture hygiene [#1903](https://github.com/EffortlessMetrics/perl-lsp-swarm/pull/1903), merge commit `ff00391a638b8db08ee31cc9d87810335779156b`; DAP logpoint interpolation substrate [#1807](https://github.com/EffortlessMetrics/perl-lsp-swarm/pull/1807), merge commit `3dd52238488c466e01393d29c1dec2de2a241765`; and FileSemanticBundle hash-last synthetic facts [#1904](https://github.com/EffortlessMetrics/perl-lsp-swarm/pull/1904), merge commit `d91878d045fa66589eb80de2f4b797c06d399b25`.

The earlier #1759 stale-branch PR-fast caveat is superseded for release-gate purposes by #1894's fresh `PR Smoke (Fast Feedback)` and `CI Gate (Merge-Blocking)` passes on current payload `89a1225e48852583f6662da43257dae6a651a96b`. The targeted prior-current DAP stack-trace receipt remains listed below as supporting evidence for the original stale-branch diagnosis.
...
Advisory/non-required state observed during this refresh: #1894 routed Rust Small, RIPR, UB review, UX, and droid review successfully. CX43/CX53 self-hosted variants that were not selected by routing are recorded as skipped, not failed. Required merge gates are green on payload `89a1225e48852583f6662da43257dae6a651a96b`.
...
| Semantic identity substrate | #1876, #1904 | File-scoped semantic fact IDs include `FileId`, preventing identical source in different files from colliding in anchors, entities, occurrences, and file-scoped edges while preserving the file-neutral reference-source sentinel. `FileSemanticBundle` category hashes now include generated-member and eval-sub synthetic entities/anchors before hash computation, and shards carry an explicit producer schema version. |
...
| Measurement substrate | #1482, #1520, #1528, #1530, #1539, #1549, #1576, #1581, #1586, #1688, #1689, #1816, #1817, #1833, #1878, #1881, #1886, #1842, #1903, #1894 | Coverage/test semantics, CPAN bounded ratchet mode, runner disk failover, workflow privilege analysis, routed-suite expectations, draft-ripr neutrality, trigger-safe docs/assets-only CI skipping, Windows-worktree coverage-baseline recovery, native-tooling git-context recovery, RIPR evidence git-context recovery, self-hosted workspace-target cleanup, sigil-aware completion regression coverage, gold-fixture corpus hygiene, and the #1839 fmt/Patch-95 gate repair are current. |
...
- [x] Changelog refreshed from merged scope only through payload `89a1225e48852583f6662da43257dae6a651a96b`.
...
Generated from current repo/GitHub state through release payload `89a1225e48852583f6662da43257dae6a651a96b` and local smoke receipts on 2026-06-20. Claims above are limited to the commands and receipts named in this file. Verify the current `origin/main` SHA again before dispatch.
