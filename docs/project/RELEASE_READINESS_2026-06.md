...
Latest audited `origin/main` payload at refresh: [#1939](https://github.com/EffortlessMetrics/perl-lsp-swarm/pull/1939), merge commit `e2201ebd7300ff36b9785a9853324294aff36542`, merged 2026-06-20T17:54:38Z. That PR carries tracking issue [#1942](https://github.com/EffortlessMetrics/perl-lsp-swarm/issues/1942) in its commit title. The latest product-safety receipt after the #1894 closeout payload is [#1926](https://github.com/EffortlessMetrics/perl-lsp-swarm/pull/1926), merge commit `9a51859fdf2a359012bd45fefa1f23b9c5098fe1`, merged 2026-06-20T17:42:27Z. It follows hash-key completion special-character support [#1839](https://github.com/EffortlessMetrics/perl-lsp-swarm/pull/1839), sigil-aware completion regression coverage [#1842](https://github.com/EffortlessMetrics/perl-lsp-swarm/pull/1842), hover markdown escaping [#1840](https://github.com/EffortlessMetrics/perl-lsp-swarm/pull/1840), completion `filterText` serialization [#1889](https://github.com/EffortlessMetrics/perl-lsp-swarm/pull/1889), DAP `totalVariables` [#1811](https://github.com/EffortlessMetrics/perl-lsp-swarm/pull/1811), corpus gold-fixture hygiene [#1903](https://github.com/EffortlessMetrics/perl-lsp-swarm/pull/1903), DAP logpoint interpolation substrate [#1807](https://github.com/EffortlessMetrics/perl-lsp-swarm/pull/1807), FileSemanticBundle hash-last synthetic facts [#1904](https://github.com/EffortlessMetrics/perl-lsp-swarm/pull/1904), and the #1927 readiness-bundle refresh [#1927](https://github.com/EffortlessMetrics/perl-lsp-swarm/pull/1927).
...
The earlier #1759 stale-branch PR-fast caveat is superseded for release-gate purposes by #1894's fresh `PR Smoke (Fast Feedback)` and `CI Gate (Merge-Blocking)` passes on payload `89a1225e48852583f6662da43257dae6a651a96b`, plus fresh #1926 and #1939 required-gate passes on the current closeout payload. The targeted prior-current DAP stack-trace receipt remains listed below as supporting evidence for the original stale-branch diagnosis. This release-staging branch is the required branch-gate receipt for refreshing this bundle through `e2201ebd7300ff36b9785a9853324294aff36542`.

| Required / decision gate | Status | Notes |
|--------------------------|-----------------|-------|
| `Perl LSP Rust Small Result` | PASS | #1939 aggregate completed 2026-06-20T17:34:31Z after CX43 Rust Small passed. |
| `ripr+ New Gap Gate` | PASS | #1939 aggregate completed 2026-06-20T17:51:03Z after CX43 `ripr+` passed. |
| `Codecov / Patch 95` | PASS | #1939 GitHub check completed 2026-06-20T17:32:39Z with 100% patch coverage for the test-only diff. |
| `Workflow Trigger Lint` | PASS | #1939 GitHub check completed 2026-06-20T17:36:16Z. |
| `PR Smoke (Fast Feedback)` | PASS | #1939 GitHub check completed 2026-06-20T17:50:11Z. |
| `CI Gate (Merge-Blocking)` | PASS | #1939 aggregate completed 2026-06-20T17:50:17Z; all CI Gate shards passed. |
| `UX Regression Tests` | PASS | #1939 GitHub check completed 2026-06-20T17:41:07Z. |
| `droid-review` | PASS | #1939 advisory check completed successfully after a 6m42s run. |
| `LSP Memory Smoke` | PASS | #1939 GitHub check completed 2026-06-20T17:38:22Z. |
| `UB Review Advisory on GitHub Hosted` | PASS | #1939 advisory check completed 2026-06-20T17:56:58Z. Advisory proof, not product-smoke proof. |

Advisory/non-required state observed during the #1939 closeout refresh: routed Rust Small, RIPR, UB review, UX, and droid review completed successfully. CX43/CX53/GitHub-hosted variants that were not selected by routing are recorded as skipped, not failed. Required merge gates were green on payload `e2201ebd7300ff36b9785a9853324294aff36542`; this branch carries the current refreshed-bundle gate before merge.
...
| Deterministic completion value | #1532, #1573, #1579, #1585, #1875, #1839, #1889, #1926 | Try::Tiny, Mojolicious, DBI, and indexed package receiver completions are offered only with supporting evidence; context-specific completion families keep stable semantic sort tiers; quoted special-character hash keys are surfaced in hash-key completion; completion items serialize `filterText` for client-side matching; multiline inline-completion candidates are full-document parse-checked and fail closed when ranges cannot be reconstructed. |
...
- [x] Required gate state recorded from latest merged release-scope PR, including fresh #1939 PR Smoke and CI Gate aggregate passes.
...
- [x] Changelog refreshed from merged scope only through payload `e2201ebd7300ff36b9785a9853324294aff36542`.
...
Generated from current repo/GitHub state through release payload `e2201ebd7300ff36b9785a9853324294aff36542` and local smoke receipts on 2026-06-20. Claims above are limited to the commands and receipts named in this file. Verify the current `origin/main` SHA again before dispatch.
