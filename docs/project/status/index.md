...
- **Release posture**: `v0.17.0` is the current workspace version and shipped public-beta release (2026-06-28); `v0.18.0` is the next public-beta train, not a maturity promotion or version bump in this tree. The published crate surface is 34 crates. See [release.md](release.md) for channel receipts.
...
- **LSP server**: `features.toml` is the canonical capability catalog; evidence-backed feature state is owned by the generated [lsp.md](lsp.md) — defer to it for current numbers
...
|-----------|------|-------|-------------|
| LSP status (evidence-owned) | [lsp.md](lsp.md) | Generator | Every LSP-touching merge |
...
- Keep the top-level README, status docs, and release runbooks aligned with the actual `perllsp` asset line, the `perl-lsp-rs` extension package, and the 34-crate published surface
