...
| Type definition | Direct source-backed class receiver | `safe subset` | Provider-decision receipt plus project-shaped receiver/data-flow blockers before broader type-definition expansion | Cursor target is a direct package/class identifier or constructor method whose source-backed class receiver resolves to an open-document package definition | Variable receivers, chained method results, function-call results, missing package definitions, and unsupported receiver shapes return no type-definition result instead of inferred data-flow claims | Generated/no-source receiver, dynamic boundary, stale fact, low confidence, ambiguous identity, missing fact, or unsupported receiver fact class |
