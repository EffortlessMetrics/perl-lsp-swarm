...
| Completion | Source-backed generated/framework member | `narrow live generated-member pilot` | Additional labeled generated-member rank/noise receipts before broader completion expansion | Fresh source-backed generated member has an explicit label, framework declaration anchor, bounded Medium confidence, and is consumed only through the generated-member completion surface | Missing anchor/label, dynamic, or stale candidates remain fallback-only | Generated/no-source, low-confidence, ambiguous, fallback-policy, or unsupported candidates |
| References | PIR-A initialized same-file lexical reference | `narrow live gated` | Keep risky-shape receipts fresh before broadening beyond the selected lexical slice | Fresh high-confidence source-backed PIR-A same-file initialized lexical reference with `includeDeclaration=false`, exact P5/P6/P7 proof, and the runtime rollback gate preserved | Legacy references remain fallback when the rollback gate is off, proof is incomplete, or the request shape is outside the selected slice | Generated/no-source, dynamic boundary, stale fact, low confidence, ambiguous identity, declaration-including request, typeglob/AUTOLOAD/symbolic/dynamic-require boundary, unsupported fact class, or fallback-policy blocker |
| Type definition | Direct source-backed class receiver | `safe subset` | Additional broader project-shaped blocker receipts before broader type-definition expansion | Cursor target is a direct package/class identifier or constructor method whose source-backed class receiver resolves to exactly one open-document package definition | Variable receivers, chained method results, function-call results, missing package definitions, generated/no-source framework-method shapes, dynamic method names, dynamic type constraints, dynamic bless package arguments, stale request versions, unscannable target sources, low-confidence unknown receivers, ambiguous package identities, and unsupported receiver shapes return no type-definition result instead of inferred data-flow or arbitrary identity claims | Generated/no-source receiver, dynamic boundary, stale fact, low confidence, ambiguous identity, missing fact, or unsupported receiver fact class |
...
| Semantic tokens | Our variable declaration token | `partial live trace slice` | Another scoped compiler-token class proof before broader compiler-token promotion | Fresh source-backed `token:our_variable_declaration:` span matches exactly one existing live `variable` token, proves `didChange` freshness, and emits no new token output | Parser/HIR token output remains live for unproven our-variable declaration token shapes | Broader `token:variable:` identity, generated/no-source, dynamic, stale, low-confidence, or unmatched span |
| Semantic tokens | State variable declaration token | `partial live trace slice` | Another scoped compiler-token class proof before broader compiler-token promotion | Fresh source-backed `token:state_variable_declaration:` span matches exactly one existing live `variable` token, proves `didChange` freshness, and emits no new token output | Parser/HIR token output remains live for unproven state-variable declaration token shapes | Broader `token:variable:` identity, generated/no-source, dynamic, stale, low-confidence, or unmatched span |
