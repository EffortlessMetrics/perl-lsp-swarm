...
| References | PIR-A initialized same-file lexical reference | `promotion-prep gated shadow` | P8 gated live flip after maintainer sign-off and project-shaped risky-shape receipts | Fresh high-confidence source-backed PIR-A same-file initialized lexical reference with `includeDeclaration=false`, exact P5/P6/P7 proof, and default rollback gate preserved | Legacy references remain fallback when the gate is off, shadow-only, proof is incomplete, or the request shape is outside the selected slice | Generated/no-source, dynamic boundary, stale fact, low confidence, ambiguous identity, typeglob/AUTOLOAD/symbolic/dynamic-require boundary, unsupported fact class, or fallback-policy blocker |
