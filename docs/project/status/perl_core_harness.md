...
| Compile mode | Scaffolded | `cargo xtask perl-core-harness run --mode compile --perl-tree <prepared-perl5> --host-perl <perl> --profile base` parses clean files, lowers HIR, projects compile effects, fails on compile-effect dynamic boundaries, and writes a JSON report |
...
receipt path for compiler workstream gaps; it does not claim full compiler or
runtime conformance yet. Execute mode remains fail-closed until the runtime
slice lands.
