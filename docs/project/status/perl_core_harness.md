...
| Execute-base | Ratcheted selected subset | `perl-core-harness run --mode execute --profile base --test base/if.t --test base/cond.t --test base/num.t --test base/pat.t --test base/while.t` runs explicit allowlisted `base` files and writes `target/perl-core/reports/base-execute.json`; `.ci/perl-core-harness/base-execute-baseline.json` ratchets 5/5 files and 68/68 TAP assertions, while profile-wide execute remains unsupported |
