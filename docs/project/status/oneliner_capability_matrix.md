...
| -a | `supported` | — | `lane_first_autosplit_field`; `lane_join_autosplit_fields` | — | `cargo test -p perl-parser-core --test command_line_oneliners` | Autosplit inserts no source text, so `@F` bodies are ordinary Perl to the parser. Whether `@F` is actually populated is layer 4. |
| -l | `supported` | — | `lane_first_autosplit_field`; `lane_join_autosplit_fields` | — | `cargo test -p perl-parser-core --test command_line_oneliners` | Record-separator handling inserts no source text. The body parses; the output-record behavior is layer 4. |
