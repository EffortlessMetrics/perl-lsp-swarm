...
| Tooling PIR | `fixture-backed` | [#8196](https://github.com/EffortlessMetrics/perl-lsp/issues/8196) | `crates/perl-parser-core/src/pir/`, `crates/perl-parser-core/tests/pir_tests.rs`, `crates/perl-parser-core/tests/pir_a_branch_body_test.rs`, `crates/perl-parser-core/tests/pir_a_loop_body_test.rs`, `crates/perl-parser-core/tests/pir_a_return_body_test.rs`, `crates/perl-parser-core/tests/pir_a_ternary_body_test.rs` | PIR v0 data model + HIR lowering for data-access, call, and dynamic-boundary families with source anchors, dynamic-boundary links, visible unknown context, conservative fallthrough/dynamic-exit CFG, and lowering receipts. The canonical body path (`lower_hir_bodies`) now also emits first-class control-flow: Branch (`if`/`unless` statements and ternary `?:`), Loop (`while`/`until`/C-style `for`/`foreach`), and Return nodes, with condition links, per-arm `Branch` edges, loop back-edges, and terminal `Return` exit edges — ternary consumers stay reachability-correct across terminal arms (issues #4795/#4815/#4856/#4859 → PRs #4797/#4817/#4858/#4864; note the merge-commit titles carry the issue numbers per the repo `(#N)` title convention). Provider cutover and precise per-arm value-merge (phi) edges remain out (gated by [#8197](https://github.com/EffortlessMetrics/perl-lsp/issues/8197)) |
