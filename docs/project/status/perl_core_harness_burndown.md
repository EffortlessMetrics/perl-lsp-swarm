...
| H14 | First bucket burn-down | Green / advisory | #3428, #3429, #3474, #3481, [run 28730071077](https://github.com/EffortlessMetrics/perl-lsp-swarm/actions/runs/28730071077) | None | `base` `parse_recovery` reduced to 0 in the accepted compile ratchet; `base/term.t` now compiles cleanly; `base/lex.t` and `base/rs.t` remain `compile_effect` |
...
| `target/perl-core/smoke/base/compile.json` | 7/9 passed, 2 `compile_effect` | Focused compile-runner proof in #3481; accepted by `.ci/perl-core-harness/upstream-base-compile-baseline.json`; failures: `base/lex.t`, `base/rs.t` |
| `target/perl-core/smoke/base/gap-map.json` | 16/18 mode-file entries passed; buckets: 2 `compile_effect` | Focused runner records confirm `base/term.t` moved from `compile_effect` to compile pass; next advisory workflow refresh should publish the full gap-map receipt |
...
| `.ci/perl-core-harness/upstream-base-compile-baseline.json` | Ratchets 7/9 compile pass state; buckets: 2 `compile_effect` | Accepted from #3481 focused compile-runner proof; separate from generated fixture baseline |
...
`base/lex.t` and `base/rs.t` remain `compile_effect` until
...
9. Burn down receipt-backed compiler buckets; the remaining `base` failures are now `compile_effect`.
