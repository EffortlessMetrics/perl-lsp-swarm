...
| `target/perl-core/smoke/run/compile.json` | 6/28 passed, 10 `parse_recovery`, 12 `compile_effect` | Focused runner proofs for #3506 move `run/switch-I-and-M.t` and `run/switchM.t` to compile pass; focused runner proofs for #3508 move `run/switch0.t`, `run/switchF2.t`, and `run/switcht.t` to compile pass; prior advisory workflow [run 28726563803](https://github.com/EffortlessMetrics/perl-lsp-swarm/actions/runs/28726563803) remains the latest full smoke artifact |
| `target/perl-core/smoke/run/gap-map.json` | 24/56 mode-file entries passed; buckets: 20 `parse_recovery`, 12 `compile_effect` | Focused runner proofs for #3506 and #3508 update the accepted compile ratchet; next advisory workflow refresh should publish the full gap-map receipt |
...
| `.ci/perl-core-harness/upstream-run-compile-baseline.json` | Ratchets 6/28 compile pass state; buckets: 10 `parse_recovery`, 12 `compile_effect` | Updated by focused #3506 proofs for `run/switch-I-and-M.t` and `run/switchM.t`, plus focused #3508 proofs for `run/switch0.t`, `run/switchF2.t`, and `run/switcht.t`; separate from generated fixture baseline |
