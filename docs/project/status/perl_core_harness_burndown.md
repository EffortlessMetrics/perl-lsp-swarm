...
| H14 | First bucket burn-down | Green / advisory | #3428, #3429, #3474, #3481, #3483, #3485, #3487, #3490, #3492, #3494, #3496, #3498, #3500, #3502, #3506, #3508, #3510, #3512, #3514, #3516, #3518, #3520, #3522, #3524, #3526, #3528, #3530, #3532, #3534, #3536, #3538, #3540, #3543, #3545, #3548, #3551, #3553, #3555, #3557, #3560, #3562, #3565, [run 28730071077](https://github.com/EffortlessMetrics/perl-lsp-swarm/actions/runs/28730071077) | None | `base` `parse_recovery` reduced to 0 in the accepted compile ratchet; all nine upstream `base/*.t` files now compile cleanly; all 25 upstream `comp/*.t` files now compile cleanly, including `comp/hints.t`; `comp/require.t` moved from compile-mode `parse_recovery` to `compile_effect` and then to compile pass; `comp/hints.t` moved from `compile_effect` to compile pass; `run/switch-I-and-M.t`, `run/switchM.t`, `run/switch0.t`, `run/switchF2.t`, `run/switcht.t`, `run/switcha.t`, `run/switchF.t`, `run/noswitch.t`, `run/switchp.t`, `run/switchx.t`, `run/switchI.t`, `run/switchd-78586.t`, `run/cloexec.t`, `run/runenv_hashseed.t`, `run/switchDx.t`, `run/fresh_perl.t`, and `run/script.t` now compile cleanly |
...
| `target/perl-core/smoke/comp/compile.json` | 25/25 passed, 0 `parse_recovery`, 0 `compile_effect` | Focused runner proofs for #3485, #3487, #3490, #3492, #3494, #3498, #3500, and #3502 move `comp/decl.t` and `comp/our.t` to compile pass and `comp/require.t`, `comp/use.t`, `comp/proto.t`, `comp/final_line_num.t`, and `comp/parser.t` to `compile_effect`; focused runner proofs for #3530, #3532, #3534, #3536, #3538, #3540, #3543, #3545, #3548, #3551, #3553, and #3555 move `comp/line_debug.t`, `comp/filter_exception.t`, `comp/redef.t`, `comp/multiline.t`, `comp/fold.t`, `comp/utf.t`, `comp/parser_run.t`, `comp/retainedlines.t`, `comp/proto.t`, `comp/use.t`, `comp/parser.t`, and `comp/final_line_num.t` to compile pass; focused #3557 proof moves `comp/form_scope.t` to compile pass; focused #3560 proof moves `comp/require.t` to compile pass; focused #3562 proof moves `comp/hints.t` to compile pass; `comp` compile-effect failures are cleared |
| `target/perl-core/smoke/comp/gap-map.json` | 43/50 mode-file entries passed; buckets: 7 `parse_recovery` | Focused runner proofs for #3485, #3487, #3490, #3492, #3494, #3498, #3500, #3502, #3530, #3532, #3534, #3536, #3538, #3540, #3543, #3545, #3548, #3551, #3553, #3555, #3557, #3560, and #3562 update the accepted compile ratchet; parse-mode receipt from [run 28711942840](https://github.com/EffortlessMetrics/perl-lsp-swarm/actions/runs/28711942840) still contributes 7 `parse_recovery` entries |
...
| `target/perl-core/smoke/run/parse.json` | 19/28 passed, 9 `parse_recovery` | [run 28726563803](https://github.com/EffortlessMetrics/perl-lsp-swarm/actions/runs/28726563803) plus focused #3565 proof; remaining failures: `run/dtrace.t`, `run/exit.t`, `run/locale.t`, `run/runenv.t`, `run/runenv_randseed.t`, `run/switchC.t`, `run/switchd.t`, `run/switches.t`, `run/todo.t` |
| `target/perl-core/smoke/run/compile.json` | 19/28 passed, 9 `parse_recovery`, no `compile_effect` | Focused runner proofs for #3506 move `run/switch-I-and-M.t` and `run/switchM.t` to compile pass; focused runner proofs for #3508 move `run/switch0.t`, `run/switchF2.t`, and `run/switcht.t` to compile pass; focused runner proofs for #3510 move `run/switcha.t`, `run/switchF.t`, and `run/noswitch.t` to compile pass; focused runner proof for #3512 moves `run/switchn.t` to compile pass; focused runner proof for #3514 moves `run/switchp.t` to compile pass; focused runner proof for #3516 moves `run/switchx.t` to compile pass; focused runner proof for #3518 moves `run/switchI.t` to compile pass; focused runner proof for #3520 moves `run/switchd-78586.t` to compile pass; focused runner proof for #3522 moves `run/cloexec.t` to compile pass; focused runner proof for #3524 moves `run/runenv_hashseed.t` to compile pass; focused runner proof for #3526 moves `run/switchDx.t` to compile pass; focused runner proof for #3528 moves `run/fresh_perl.t` to compile pass; focused #3565 proof moves `run/script.t` to compile pass; prior advisory workflow [run 28726563803](https://github.com/EffortlessMetrics/perl-lsp-swarm/actions/runs/28726563803) remains the latest full smoke artifact |
| `target/perl-core/smoke/run/gap-map.json` | 38/56 mode-file entries passed; buckets: 18 `parse_recovery` | Focused runner proofs for #3506, #3508, #3510, #3512, #3514, #3516, #3518, #3520, #3522, #3524, #3526, #3528, and #3565 update the accepted compile ratchet; next advisory workflow refresh should publish the full gap-map receipt |
...
| `.ci/perl-core-harness/upstream-comp-compile-baseline.json` | Ratchets 25/25 compile pass state; buckets: none | Updated by focused #3485, #3487, #3490, #3492, #3494, #3498, #3500, and #3502 proofs for `comp/decl.t`, `comp/final_line_num.t`, `comp/parser.t`, `comp/require.t`, `comp/use.t`, `comp/proto.t`, and `comp/our.t`, plus focused #3530, #3532, #3534, #3536, #3538, #3540, #3543, #3545, #3548, #3551, #3553, #3555, #3557, #3560, and #3562 proofs for `comp/line_debug.t`, `comp/filter_exception.t`, `comp/redef.t`, `comp/multiline.t`, `comp/fold.t`, `comp/utf.t`, `comp/parser_run.t`, `comp/retainedlines.t`, `comp/proto.t`, `comp/use.t`, `comp/parser.t`, `comp/final_line_num.t`, `comp/form_scope.t`, `comp/require.t`, and `comp/hints.t`; separate from generated fixture baseline |
| `.ci/perl-core-harness/upstream-run-compile-baseline.json` | Ratchets 19/28 compile pass state; buckets: 9 `parse_recovery` | Updated by focused #3506 proofs for `run/switch-I-and-M.t` and `run/switchM.t`, focused #3508 proofs for `run/switch0.t`, `run/switchF2.t`, and `run/switcht.t`, focused #3510 proofs for `run/switcha.t`, `run/switchF.t`, and `run/noswitch.t`, focused #3512 proof for `run/switchn.t`, focused #3514 proof for `run/switchp.t`, focused #3516 proof for `run/switchx.t`, focused #3518 proof for `run/switchI.t`, focused #3520 proof for `run/switchd-78586.t`, focused #3522 proof for `run/cloexec.t`, focused #3524 proof for `run/runenv_hashseed.t`, focused #3526 proof for `run/switchDx.t`, focused #3528 proof for `run/fresh_perl.t`, plus focused #3565 proof for `run/script.t`; separate from generated fixture baseline |
