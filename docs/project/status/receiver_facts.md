...
| `hash_slot_receiver` | `partial` | `receiver_facts` module tests `hash_slot_receiver_uses_known_slot_fact` and `source_derived_hash_of_arrays_receiver_resolves_element_fact`; `receiver_expression_facts` tests for plain hash literals and slot assignment | Works when `TypeEnvironment` contains a `HashShape`; source-derived plain `%hash` literals and `$hash{key}` assignments can populate that shape, and a static hash slot can now remain classified through a following array index. Broader chained method-return source inference remains pending. |
| `hashref_slot_receiver` | `partial` | `receiver_facts` module tests `hashref_slot_receiver_preserves_hashref_kind` and `source_derived_hashref_of_arrays_receiver_resolves_element_fact`; `receiver_expression_facts` tests for hashref literals and slot assignment | Works when the base fact already has a hash shape; source-derived `$hashref->{key}` facts are proven for hashref literals and slot assignment, including a static hashref slot followed by an array index. Broader chained method-return source inference remains pending. |
...
| `array_index_receiver` | `partial` | `receiver_facts` module tests for static and dynamic array indexes, `source_derived_hash_of_arrays_receiver_resolves_element_fact`, `source_derived_hashref_of_arrays_receiver_resolves_element_fact`, and `dynamic_hash_key_inside_array_chain_preserves_dynamic_boundary`; PR [#9468](https://github.com/EffortlessMetrics/perl-lsp/pull/9468) | Static indexes can use existing `ArrayShape` facts and can traverse source-backed plain-hash or hashref slots whose values carry an array shape. A dynamic intermediate hash key preserves `DynamicHashKey`; dynamic indexes remain non-exact. Array-index completion remains on the existing fallback path until a separate provider receipt authorizes cutover. |
| `dynamic_key_boundary` | `landed` | `receiver_facts` module tests `dynamic_hash_key_marks_dynamic_boundary` and `dynamic_hash_key_inside_array_chain_preserves_dynamic_boundary`; `TypeFact::dynamic` test in `type_facts.rs`; `receiver_expression_facts` dynamic plain-hash-key test; completion provider test `dynamic_hash_key_receiver_preserves_imported_fallback` | Proven for direct receiver extraction, a dynamic intermediate key in a hash-of-arrays chain, plain hash expression facts, and the first completion-provider boundary receipt. Additional provider boundary receipts remain pending for other receiver forms. |
...
| `receiver_fact_api` | `landed` | `crates/perl-semantic-analyzer/src/analysis/receiver_facts.rs`; receiver-facts module tests for source-derived plain-hash and hashref arrays; PR [#9468](https://github.com/effortlessmetrics/perl-lsp/pull/9468) | API extracts facts from existing AST and supplied environment facts, including nested static hash/hashref and array access paths. Dynamic intermediate hash keys fail closed while retaining their boundary and traversed access evidence through the covered later static aggregate lookups; malformed, unknown, or unsupported method-call containers remain generic fallback. Broader method-call chains remain unknown until explicit rules land. |
