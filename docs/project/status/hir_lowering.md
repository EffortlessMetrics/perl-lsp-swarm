...
| `Diamond` | `lowered` | `ReadlineExpr` | `<>`/`<<>>` lowered as a readline shell reading the `@ARGV` diamond source. |
| `Ellipsis` | `not_yet_modeled` | - | No first-slice HIR shell yet. |
| `Undef` | `lowered` | `LiteralExpr` | Lowered as undef literal shell. |
| `Readline` | `lowered` | `ReadlineExpr` | Lowered as readline shell classified as a named or scalar filehandle. |
| `Glob` | `lowered` | `GlobExpr` | Angle-bracket glob lowered as a glob shell recording whether the pattern interpolates. |
...
| `Heredoc` | `lowered` | `HeredocExpr` | Lowered as heredoc shell with delimiter, interpolation, indent, and command facts; the body stays addressable by range. |
