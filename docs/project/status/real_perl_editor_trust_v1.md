...
live `variable` token and `didChange` freshness boundary. The our-variable
declaration proof now authorizes only the scoped
`token:our_variable_declaration:` identity class when its source-backed
package-scoped `our` declaration span already matches an existing live
parser/HIR `variable` token and `didChange` freshness is proven. The
state-variable declaration proof now authorizes only the scoped
`token:state_variable_declaration:` identity class when its source-backed
lexical `state` declaration span already matches an existing live parser/HIR
`variable` token and `didChange` freshness is proven.
...
lexical-variable declaration, lexical-variable use, our-variable declaration, and state-variable declaration trace slices that emit no
new token output.
...
lexical-variable declaration, lexical-variable use, our-variable declaration, and state-variable declaration proofs stay scoped,
output-neutral, and
...
declaration, lexical-variable use, our-variable declaration, and state-variable declaration rows so the class
registry, human ledger, and machine policy share the same fallback and blocker
boundary. The next
