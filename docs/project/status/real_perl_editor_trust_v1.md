...
| Type definition | `safe subset + explanation receipt` | Live request receipts now record acted provider-decision traces for existing source-backed direct package/class and constructor receiver results, no-result fallback traces for unproven variable/data-flow receivers, a project-shaped blocker receipt where open-package variable receivers, function-call results, and chained method results stay fallback, an ambiguous package-identity blocker receipt for duplicate open-document package definitions, a generated/dynamic/low-confidence blocker receipt for generated/no-source framework-method, dynamic-method-name, and unknown-receiver request shapes, a dynamic type-constraint blocker receipt for `isa => $type` shapes, and a stale request-version blocker receipt | Type-definition behavior is narrowed for ambiguous package identities and stale request versions; variable receivers, chained method results, function-call results, missing packages, duplicate package definitions, generated/no-source facts, dynamic boundaries, stale facts, and low-confidence facts remain fallback, blocked, or unpromoted; dynamic method names and dynamic type constraints are labeled as `dynamic_boundary`, while stale request versions are labeled as `stale_fact` before content-modified is returned | Additional broader project-shaped blocker receipts before broader type-definition expansion |
...
constraints and labels that fallback as `dynamic_boundary`. The stale
request-version receipt now proves a request older than the current document
version records a `stale_fact` provider decision with stale freshness before
returning content-modified, rather than answering from current source under an
old request version.
