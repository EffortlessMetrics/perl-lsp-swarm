...
| Type definition | `safe subset + explanation receipt` | Live request receipts now record acted provider-decision traces for existing source-backed direct package/class and constructor receiver results, plus no-result fallback traces for unproven variable/data-flow receivers | Existing type-definition behavior is unchanged; variable receivers, chained method results, function-call results, missing packages, generated/no-source facts, dynamic boundaries, stale facts, low-confidence facts, and ambiguous identities remain fallback, blocked, or unpromoted | Project-shaped receiver/data-flow blockers before broader type-definition expansion |
