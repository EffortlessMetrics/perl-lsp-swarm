...
- References now have narrow live source-backed runtime slices: fresh,
  high-confidence, source-backed `ExactAst`, `ImportExportInference`, or
  `LiteralRequireImport` occurrence references can drive
  `textDocument/references` when declaration inclusion is off, and the selected
  PIR-A initialized same-file lexical slice can now enter the same live tier
  when `includeDeclaration=false`; generated/no-source, dynamic, stale,
  low-confidence, ambiguous, and declaration-including requests retain legacy
  fallback.
...
- The our-variable declaration scoped cutover proof proves a source-backed
  `our $shared` compiler span matches exactly one existing live `variable`
  token, allows only the `token:our_variable_declaration:` compiler identity
  class, refreshes after `didChange`, and emits no new semantic-token output. It
  does not approve broader `token:variable:` candidates or other compiler-token
  classes. The our-variable declaration proof extends the existing
  `my`/`state` lexical-variable declaration class to the package-scoped `our`
  declaration form while preserving the same output-neutral, fail-closed
  boundary.
- The state-variable declaration scoped cutover proof proves a source-backed
  `state $count` compiler span matches exactly one existing live `variable`
  token, allows only the `token:state_variable_declaration:` compiler identity
  class, refreshes after `didChange`, and emits no new semantic-token output. It
  does not approve broader `token:variable:` candidates or other compiler-token
  classes. It completes the lexical-declaration trio (`my`/`our`/`state`) under
  the same output-neutral, fail-closed boundary.
...
| `references_exact_live` | `partial live` | [#8828](https://github.com/EffortlessMetrics/perl-lsp/issues/8828), [#8462](https://github.com/EffortlessMetrics/perl-lsp/issues/8462), `FindReferences` release-readiness receipts; `handle_references_lexical_variable_without_declaration_uses_source_backed_tier` | Fresh, high-confidence, source-backed exact occurrences and the selected PIR-A initialized same-file lexical slice may answer live when declaration inclusion is off. |
...
| References | `partial live / ranked-shadowed` | Fresh, high-confidence, source-backed `ExactAst`, `ImportExportInference`, or `LiteralRequireImport` occurrence references can drive live `textDocument/references` when `includeDeclaration=false`; the selected PIR-A initialized same-file lexical slice can also answer live through the semantic source-backed tier for declaration-excluding requests. Generated/no-source, dynamic-boundary, low-confidence, ambiguous, stale, declaration-including, coderef, and typeglob requests remain traced as fallback/shadow proof. Mojolicious scenario 30 records exact-local, imported-symbol, and declaration-including boundary reference probes without behavior changes. | Broader references migration requires precision/recall receipts for generated, coderef, typeglob, and declaration-including cases |
...
- Semantic-token note, 2026-06-20: `our_variable_declaration` joins the scoped
  output-neutral provider traces/proofs for the `token:our_variable_declaration:`
  identity only. The proof matches a source-backed `our $shared` span to exactly
  one existing live `variable` token, refreshes after `didChange`, and emits no
  new token output. It does not move broader `token:variable:` or other
  compiler-token classes into live output; broader compiler-backed semantic
  tokens still require class-specific cutover proof.

- Semantic-token note, 2026-06-20: `state_variable_declaration` joins the scoped
  output-neutral provider traces/proofs for the `token:state_variable_declaration:`
  identity only. The proof matches a source-backed `state $count` span to exactly
  one existing live `variable` token, refreshes after `didChange`, and emits no
  new token output. It completes the `my`/`our`/`state` lexical-declaration trio
  without moving broader `token:variable:` or other compiler-token classes into
  live output; broader compiler-backed semantic tokens still require
  class-specific cutover proof.
