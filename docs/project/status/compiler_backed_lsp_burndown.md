...
| `run` parse/compile | `parse_recovery` on run-script and switch tests; `compile_effect` remains on switch and fresh-perl tests after `run/switch-I-and-M.t` and `run/switchM.t` moved to compile pass | Continue with a tight switch or fresh-perl compile-effect cluster |
