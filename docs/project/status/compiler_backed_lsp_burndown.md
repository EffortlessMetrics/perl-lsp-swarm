...
| `comp` parse/compile | compile-mode `parse_recovery` on `comp/parser.t`; `compile_effect` on fifteen files including `comp/final_line_num.t`, `comp/line_debug.t`, `comp/require.t`, `comp/use.t`, and `comp/proto.t` | Continue with the remaining tight `comp/parser.t` parse gap, or reduce `comp/line_debug.t`, `comp/require.t`, `comp/our.t`, or `comp/hints.t` compile effects |
