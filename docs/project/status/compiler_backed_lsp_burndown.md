...
| `comp` parse/compile | compile-mode `parse_recovery` is cleared; `compile_effect` remains on five files including `comp/parser.t`, `comp/final_line_num.t`, `comp/require.t`, `comp/form_scope.t`, and `comp/hints.t` | Reduce `comp/parser.t`, `comp/require.t`, or another tight comp compile-effect cluster |
