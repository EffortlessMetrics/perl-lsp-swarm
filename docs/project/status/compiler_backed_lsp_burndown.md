...
| `comp` parse/compile | compile-mode `parse_recovery` is cleared; `compile_effect` remains on four files: `comp/final_line_num.t`, `comp/form_scope.t`, `comp/hints.t`, and `comp/require.t` | Reduce `comp/form_scope.t`, `comp/require.t`, or another tight comp compile-effect cluster |
