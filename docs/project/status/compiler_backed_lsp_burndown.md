...
| `comp` parse/compile | compile-mode `parse_recovery` is cleared; `compile_effect` remains on two files: `comp/hints.t` and `comp/require.t`; #3555 moves `comp/final_line_num.t` from `compile_effect` to compile pass; #3557 moves `comp/form_scope.t` from `compile_effect` to compile pass | Reduce `comp/hints.t`, `comp/require.t`, or another tight comp compile-effect cluster |
