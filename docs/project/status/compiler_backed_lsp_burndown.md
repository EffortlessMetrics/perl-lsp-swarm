...
| P3 | Compile bucket burn-down | Yellow / ongoing | #3474 focused runner proof moves `base/lex.t` from `parse_recovery` to `compile_effect`; #3481 focused proof moves `base/term.t` from `compile_effect` to compile pass; current real upstream receipts still show `parse_recovery` in `comp`/`run` and `compile_effect` in `base`/`comp`/`run` | Reduce one named bucket or tight cluster | One compile bucket shrinks without new `unknown` or unbucketed failures |
...
|---|---|---|
| `base` parse/compile | `base/lex.t` and `base/rs.t` remain `compile_effect`; `base/term.t` is compile-clean but not selected execute | Classify or model `base/lex.t` / `base/rs.t`; defer `base/term.t` execute until the runtime lane intentionally pulls IO/backtick behavior |
