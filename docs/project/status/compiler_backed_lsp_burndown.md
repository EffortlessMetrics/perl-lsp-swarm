...
| `run` parse/compile | `parse_recovery` on run-script and switch tests; `compile_effect` remains on the fresh-perl test after `run/switch-I-and-M.t`, `run/switchM.t`, `run/switch0.t`, `run/switchF2.t`, `run/switcht.t`, `run/switcha.t`, `run/switchF.t`, `run/noswitch.t`, `run/switchn.t`, `run/switchp.t`, `run/switchx.t`, `run/switchI.t`, `run/switchd-78586.t`, `run/cloexec.t`, `run/runenv_hashseed.t`, and `run/switchDx.t` moved to compile pass | Continue with a tight fresh-perl compile-effect cluster or return to comp compile-effect |
