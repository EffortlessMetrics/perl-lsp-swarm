...
| `comp` parse/compile | compile-mode `parse_recovery` is cleared; `compile_effect` remains on three files: `comp/form_scope.t`, `comp/hints.t`, and `comp/require.t`; #3555 moves `comp/final_line_num.t` from `compile_effect` to compile pass | Reduce `comp/form_scope.t`, `comp/require.t`, or another tight comp compile-effect cluster |
