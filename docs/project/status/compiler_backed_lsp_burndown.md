...
| `comp` parse/compile | compile-mode `parse_recovery` is cleared; `compile_effect` remains on sixteen files including `comp/parser.t`, `comp/final_line_num.t`, `comp/line_debug.t`, `comp/require.t`, `comp/use.t`, and `comp/proto.t` | Reduce `comp/parser.t`, `comp/line_debug.t`, `comp/require.t`, `comp/our.t`, or `comp/hints.t` compile effects |
