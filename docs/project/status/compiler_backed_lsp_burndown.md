...
| `comp` parse/compile | `parse_recovery` on `comp/final_line_num.t`, `comp/line_debug.t`, `comp/parser.t`; `compile_effect` on thirteen files including `comp/require.t`, `comp/use.t`, and `comp/proto.t` | Continue with a tight `comp` parser gap such as `comp/final_line_num.t` / `comp/line_debug.t`, or reduce `comp/require.t`, `comp/our.t`, or `comp/hints.t` compile effects |
