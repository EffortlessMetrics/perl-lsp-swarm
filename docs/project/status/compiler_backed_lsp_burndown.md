...
| `comp` parse/compile | `parse_recovery` on `comp/decl.t`, `comp/final_line_num.t`, `comp/line_debug.t`, `comp/parser.t`, `comp/proto.t`, `comp/use.t`; `compile_effect` on eleven files including `comp/require.t` | Start with `comp/use.t` parser gaps, then `comp/require.t`, `comp/our.t`, or `comp/hints.t` compile effects |
