...
| `run` parse/compile | `parse_recovery` on run-script and switch tests; `compile_effect` remains on switch and fresh-perl tests after `run/switch-I-and-M.t`, `run/switchM.t`, `run/switch0.t`, `run/switchF2.t`, `run/switcht.t`, `run/switcha.t`, `run/switchF.t`, `run/noswitch.t`, `run/switchn.t`, `run/switchp.t`, `run/switchx.t`, `run/switchI.t`, `run/switchd-78586.t`, `run/cloexec.t`, and `run/runenv_hashseed.t` moved to compile pass | Continue with a tight switch or fresh-perl compile-effect cluster |
