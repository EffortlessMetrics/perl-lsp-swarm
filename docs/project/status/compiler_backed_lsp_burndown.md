...
| `comp` parse/compile | compile-mode `parse_recovery` on `comp/final_line_num.t` and `comp/parser.t`; `compile_effect` on fourteen files including `comp/line_debug.t`, `comp/require.t`, `comp/use.t`, and `comp/proto.t` | Continue with a tight `comp` parser gap such as `comp/final_line_num.t`, or reduce `comp/line_debug.t`, `comp/require.t`, `comp/our.t`, or `comp/hints.t` compile effects |
