...
| `comp` parse/compile | compile-mode `parse_recovery` is cleared; `compile_effect` remains on seven files including `comp/parser.t`, `comp/final_line_num.t`, `comp/require.t`, `comp/use.t`, and `comp/proto.t` | Reduce `comp/parser.t`, `comp/require.t`, `comp/hints.t`, or another tight comp compile-effect cluster |
