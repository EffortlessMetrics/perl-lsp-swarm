...
- **A filtered `gh pr checks` summary** masked aggregator failures — `UNSTABLE` mergeStateStatus + raw `statusCheckRollup` filter (excluding tokmd advisory) was the only reliable green-check predicate.
