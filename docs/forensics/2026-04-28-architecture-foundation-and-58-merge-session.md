...
### 7. Filtered check output masks aggregator failures

During ops merge preparation, a filtered `gh pr checks` summary showed all checks green for a PR that the raw GitHub API reported as having failing checks. The command wrapper was filtering the output to show only the most recent check result per check name, but it was applying that filter in a way that could show a stale passing result over a current failing one.
