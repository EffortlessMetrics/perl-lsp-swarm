...
- The **dogfood test suite** (`crates/perl-lsp-ux-tests/tests/ux_scenario_*`) is ground truth: 213 tests where a working provider behavior is a hard `assert!` regression guard and a broken one is `#[ignore]` + a filed issue (e.g. `#[ignore = "real gap — incomingCalls returns empty for OO method callers; tracking #3093"]`). The negative space is first-class: you cannot have a broken behavior the suite is silent about, because "broken" is an explicit, compiled, listed state. Note this doc states **no frozen count of gaps**: the live set is derived by grepping the suite (`grep -rE '#\[ignore' crates/perl-lsp-ux-tests/tests/`), never hand-maintained, so it cannot drift — a fixed number written here would itself be exactly the stale, hand-counted scoreboard the pattern warns against. (At the time of writing it is a single open gap, #3093; the count shrinks as gaps are fixed and un-ignored.)
