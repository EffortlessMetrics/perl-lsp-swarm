...
| `handle.row.get` | `DBIx::QuickORM::Handle` | `row` | handle | zero_arg_getter | single_optional_row | zero_or_one | preserved_from_receiver | sync_async_aside_forked | croaks | exact | — | `lib/DBIx/QuickORM/Handle.pm:1308` |
...
- `handle.row.get`: Zero-argument form returns the bound row object, if any. `_check_row` admits a bound row only when its source shares the handle's `source_orm_name` (Handle.pm:291-304), so the returned row's class is the receiver's source row class rather than an untyped scalar.
