...
- multi-range formatting is withdrawn from live routes;
  `documentRangeFormattingProvider.rangesSupport` and
  `documentRangesFormattingProvider` are not advertised
...
| Multi-range formatting | `documentRangeFormattingProvider.rangesSupport`, `textDocument/rangesFormatting` | Withdrawn from live routes until the atomic composition contract is proven; the capability is absent and well-formed requests return `MethodNotFound`. Malformed requests may be rejected earlier as `InvalidParams`. The non-spec plural capability is absent. | `lsp_caps_contract_shapes`, `lsp_disabled_features_tests`, `lsp_formatting_e2e`, `lsp_capabilities_snapshot`, `lsp_cap_snap` |
| Dynamic text document content | `workspace.textDocumentContent.schemes`, `workspace/textDocumentContent` | `perldoc` scheme is advertised; local workspace POD returns extracted sections plus sorted related `perldoc://` links for bare or labeled module references, POD section references, and supported core pragmas; invalid params and malformed URIs return `InvalidParams`; unsupported schemes return deterministic unavailable errors. | `lsp_text_document_content_tests`, `lsp_cap_snap` |
...
| Command tooltips | `Command.tooltip` on reachable command objects | Every reachable LSP Command producer uses the canonical 3.18 Command type and a required plain-text tooltip; title, identifier, and arguments stay unchanged. Completion, inline-completion, and document-link surfaces do not produce Command objects. CodeAction.documentation Command objects are included. | `lsp_command_tooltip_tests`, `lsp_318_negative_claims`, `check-lsp-318-claims` |
...
| CodeAction.documentation (`initialize` `codeActionProvider.documentation`, runtime + effective-surface writers) | `Command::presented` via shared `code_action_documentation_entries` | populate useful plain-text tooltip |
