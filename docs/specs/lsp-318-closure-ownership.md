...
| relative-pattern-document-selector | `RelativePattern` watcher registrations | accepted-disposition | n/a | protocol-identity correction owner #8897 | matrix identity split in #8897; protocol types #11803 / PR #12096 | Watcher RelativePattern is an implemented LSP 3.17 surface, not proof for LSP 3.18 document filters. The current text-document filter remains negative-gated because no producer needs it, and notebook-filter support remains not applicable without a notebook model. |
...
| notebook-318-additions | Notebook 3.18 additions | accepted-disposition | n/a | claim authority #6731; bounded selector ruling #8897 | a concrete Perl notebook document model and editor need | The server has no notebook document model; #8897 must not silently expand into a full notebook implementation program. |
