...
| relative-pattern-document-selector | Document-filter `relative pattern` (text-document filters) | accepted-disposition | n/a | protocol-identity correction owner #8897 | a producer that needs relative document filters; protocol types #11803 / PR #12096 | The current text-document filter remains negative-gated because no producer needs it. The implemented watcher RelativePattern surface in the conformance matrix is not proof for this distinct LSP 3.18 document-filter surface. |
| relative-pattern-notebook-selector | Notebook document-filter `relative pattern` | accepted-disposition | n/a | protocol-identity correction owner #8897 | a concrete notebook document model and editor need | The preview profile may advertise a plain-language notebook selector; notebook-filter `RelativePattern` support remains absent and is independently gated from text-document filters. |
